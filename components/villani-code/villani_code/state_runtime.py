...
def _collect_changed_python_files(runner: Any) -> list[str]:
    current_changed = set(runner._git_changed_files())
    ...
    runner._last_patch_sanity_paths = {
        "raw": observed,
        "canonical": canonical,
        "relevant": relevant,
    }
...
def _repo_relative_modified_path(runner: Any, raw_path: Any) -> str:
    ...
    if ":/" in candidate:
        ...
        for i in range(len(cand_parts)):
            tail = cand_parts[i:]
            if len(tail) >= len(repo_parts) and [p.lower() for p in tail[: len(repo_parts)]] == [
                p.lower() for p in repo_parts
            ]:
...
def _run_patch_sanity_check(runner: Any) -> dict[str, Any]:
    ...
    path_telemetry = getattr(
        runner, "_last_patch_sanity_paths", {"raw": [], "canonical": [], "relevant": []}
    )
    telemetry = {
