const obj = (value) => value && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
const text = (...values) => values.find((v) => typeof v === "string" && v.length > 0);
...
const eventName = (event) => text(event.name, obj(event.raw).event_type, event.type, event.title) ??
    "unknown";
const failed = (event) => /fail|error|cancel/.test(`${event.status ?? ""} ${eventName(event)}`.toLowerCase()) || (event.exit_code ?? 0) !== 0;
const isTest = (event) => /test|pytest|vitest|jest|npm test/.test(`${event.kind ?? ""} ${eventName(event)} ${event.command ?? ""}`.toLowerCase());
const isCommand = (event) => /command|tool|shell|bash/.test(`${event.kind ?? ""} ${event.type ?? ""} ${eventName(event)}`.toLowerCase()) || Boolean(event.command);
const isFile = (event) => /file|patch|diff|materializ/.test(`${event.kind ?? ""} ${event.type ?? ""} ${eventName(event)}`.toLowerCase());
export function deriveRunStatus(events, controllerState) {
    const terminal = controllerState?.toUpperCase();
    const commands = events.filter(isCommand);
    const tests = events.filter(isTest);
    const lifecycleKey = (event) => {
        const value = data(event);
        return text(value.tool_use_id, value.tool_call_id, value.call_id, value.command_id, event.command, event.event_id, event.id);
    };
    ...
    const base = {
        ...
        fileEdits: edits,
        hasFinalAnswer: events.some((e) => /run_(completed|failed|exhausted)|session_end/.test(eventName(e))),
    };
    ...
    const running = events.some((e) => /running|started|queued/.test(`${e.status ?? ""} ${eventName(e)}`.toLowerCase()));
    if (running)
        return {
            ...base,
            status: "partial",
...
function attemptsFrom(detail, events) {
...
export function deriveRun(detail, events) {
    ...
    const candidates = attempts.map((attempt) => {
        const related = events
            ...
            .map(data);
        const outcome = detail.outcomes.find((value) => value.attempt_id === attempt.id) ?? {};
        const verification = related.find((value) => "acceptance_eligible" in value) ?? outcome;
        return {
            attemptId: attempt.id,
            status: text(verification.outcome, outcome.status, attempt.status) ?? "unknown",
            eligible: verification.acceptance_eligible === true || outcome.accepted === true,
            selected: attempt.id === selected,
            requirementResults: list(verification.requirement_results),
            evidenceGrades: strings(verification.evidence_grades).concat(strings(verification.success_evidence).map((item) => item)),
            risks: strings(verification.risk_flags).concat(strings(verification.risks)),
            patchDigest: text(verification.patch_digest, outcome.patch_digest),
            explanation: text(verification.selection_explanation, verification.reason, outcome.reason),
    ...
    for (const event of events) {
        ...
        if (/patch|diff/.test(`${event.kind ?? ""} ${eventName(event)}`.toLowerCase()))
            evolution.push({
    ...
    return {
        ...
        failure: failureEvent
            ? {
                ...
                evidence: strings(failureData?.evidence).concat(text(failureData?.message) ? [text(failureData?.message)] : []),
                nextSafeAction: text(failureData?.next_safe_action, failureData?.recommended_action) ?? "Review the evidence before retrying.",
