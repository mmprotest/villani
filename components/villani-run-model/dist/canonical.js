...
const records = (value) => Array.isArray(value) ? value.map(object) : [];
...
function attemptSnapshot(attemptId, candidate, attemptStatus, selectedAttemptId) {
    ...
    return {
        ...
        verification_authority: text(verification.authority_source ?? object(verification.metadata).authority_source),
 ...
 */
export function canonicalRunSnapshot(detail) {
    const projection = object(detail.canonical_projection);
    const selectedAttemptId = text(detail.selected_attempt_id ?? projection.selected_attempt_id);
    const candidateOutcomes = object(detail.candidate_outcomes ?? projection.candidate_outcomes);
    ...
    const attempts = attemptIds.map((attemptId) => attemptSnapshot(attemptId, object(candidateOutcomes[attemptId]), statuses.get(attemptId), selectedAttemptId));
    const selected = attempts.find((attempt) => attempt.selected);
    const candidateEligibility = Object.fromEntries(attempts.map((attempt) => [attempt.attempt_id, attempt.eligible]));
    const attemptChangedFiles = Object.fromEntries(attempts.map((attempt) => [attempt.attempt_id, attempt.changed_files]));
    return {
        run_id: detail.id,
        ...
        raw_classification: detail.raw_classification ??
            (projection.raw_classification ? object(projection.raw_classification) : null),
        effective_classification: detail.effective_classification ??
            (projection.effective_classification
                ? object(projection.effective_classification)
                : null),
        classification_confidence: finiteNumber(detail.classification_confidence ?? projection.classification_confidence),
        classification_adjustments: records(detail.classification_adjustments ?? projection.classification_adjustments),
        ...
        attempts,
        escalation_count: finiteNumber(detail.escalation_count ?? projection.escalation_count),
        ...
        coding_cost_usd: finiteNumber(detail.coding_cost_usd ?? projection.coding_cost_usd),
        verifier_cost_usd: finiteNumber(detail.verifier_cost_usd ?? projection.verifier_cost_usd),
        ...
        verification_outcome: normalizedStatus(detail.verification_status ?? projection.verification_status),
        verification_authority: text(detail.verification_authority ?? projection.verification_authority),
        verifier_identity: selected?.verifier_identity ?? null,
        candidate_eligibility: candidateEligibility,
        candidate_rankings: records(detail.selection_rankings ?? projection.selection_rankings),
        selection_reason: text(detail.selection_reason ?? projection.selection_reason),
        file_write_count: finiteNumber(detail.file_write_count ?? projection.file_write_count),
        attempt_changed_files: attemptChangedFiles,
        selected_materialized_files: strings(detail.changed_files ?? projection.changed_files),
        materialization_status: normalizedStatus(detail.materialization_status ?? projection.materialization_status),
        ...
        redacted_field_count: finiteNumber(detail.redacted_field_count ?? projection.redacted_field_count),
        withheld_artifact_count: finiteNumber(detail.withheld_artifact_count ?? projection.withheld_artifact_count),
        withheld_artifact_categories: detail.withheld_artifact_categories !== undefined ||
            Array.isArray(projection.withheld_artifact_categories)
            ? strings(detail.withheld_artifact_categories ??
