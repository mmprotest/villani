...
def test_adaptive_single_candidate_still_uses_single_task(tmp_path):
    ...
    execute_tool_with_policy(
        s, "ops_select_execution_path", {"path": "single_task", "reason": "go"}, "x", c
    )
...
def test_adaptive_invalid_execution_path_is_repaired_to_tournament(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        {"path": "decomposed_subtasks", "reason": "bad"},
        "x",
        c,
    )
    assert not res.is_error
    ...
    assert s.subtasks == []
    assert any(
        "adaptive_orchestrator_forced_tournament_execution_path" in w
        for w in s.warnings
    )
...
def test_tournament_candidate_prompt_is_clean_pass_through(tmp_path):
    s = state(tmp_path)
    s.task = "fix the bug"
    s.success_criteria = "tests pass"
    s.execution_path = "candidate_tournament"
    s.attempt_observations = []
    s.reviews = [{"summary": "looks good"}]
...
def test_adaptive_tournament_launches_parallel_and_writes_ranking(tmp_path):
    s = state(tmp_path)
    c = ctx(tmp_path)
    c.backend.max_parallel = 2
    c.coding_backend.max_parallel = 2
    s.orchestrator = "adaptive"
    s.investigation = {"summary": "i"}
    execute_tool_with_policy(
        s,
    ...
    execute_tool_with_policy(
        s,
        "ops_select_execution_path",
        {"path": "candidate_tournament", "reason": "go"},
        "sel",
        c,
    )
    res = execute_tool_with_policy(
        ...
        {"attempts": 3, "reason": "parallel independent"},
        "launch",
        c,
    )
    ...
    assert (
        tmp_path / "run" / "candidates" / "candidate_001" / "evidence.json"
    ).exists()
    ev = execute_tool_with_policy(
        s, "ops_evaluate_tournament", {"reason": "evaluate saved candidates"}, "eval", c
    )
...
def _material_candidate(tmp_path, s, aid, changed="a.py", status="completed"):
    wt = tmp_path / "run" / "attempts" / aid / "worktree"
    wt.mkdir(parents=True, exist_ok=True)
    patch = tmp_path / "run" / "attempts" / aid / "patch.diff"
    ...
    a = CandidateAttemptState(
        attempt_id=aid,
        status=status,
        scope="candidate",
        worktree_path=str(wt),
        artifacts_dir=str(patch.parent),
    ...
    return TournamentRanking(
        ranked_candidates=[
            RankedCandidate(
                ...
                hidden_test_risk_score=0.2,
                pairwise_wins=0,
                pairwise_losses=0,
                validation_status="not_run",
                materiality_notes="patch",
            )
...
def test_hydrate_tournament_state_from_artifacts_restores_evidence_ranking_selection(
    tmp_path,
):
    from villani_ops.agentic.tools import (
        ...
        _write_tournament_artifacts,
    )

    s = state(tmp_path)
    s.execution_path = "candidate_tournament"
    s.phase = "running_candidates"
    ...
    fresh.execution_path = "candidate_tournament"
    fresh.phase = "running_candidates"
...
def test_low_time_evaluation_uses_fallback_reviews_and_best_effort_selection(tmp_path):
    ...
    c = ctx(tmp_path)
    s.investigation = {"summary": "i"}
    s.plan = {"strategy": "parallel_candidates"}
    s.execution_path = "candidate_tournament"
    s.tournament_evaluation_deadline_seconds = 1
    s.reserve_finalization_seconds = 60
    a = _material_candidate(tmp_path, s, "candidate_001")
    from villani_ops.agentic.tools import _record_completed_tournament_candidate

    _record_completed_tournament_candidate(s, a, c)
    res = execute_tool_with_policy(
        s, "ops_evaluate_tournament", {"reason": "low time"}, "eval", c
    )
    assert not res.is_error, res.content
    assert (
        s.candidate_risk_reviews["candidate_001"].review_quality
...
def test_tolerant_structured_parsing_repairs_extra_and_missing_fields():
    ...
    from villani_ops.agentic.state import (
        CandidateRiskReview,
        PairwiseCandidateComparison,
    )

    review = _coerce_structured_payload(
        CandidateRiskReview,
        {"candidate_id": "candidate_001", "summary": "ok", "extra": "ignored"},
        quality="model_minimal",
        changed_files=["a.py"],
    )
    ...
    cmp = _coerce_structured_payload(
        PairwiseCandidateComparison,
        {"winner": "candidate_a", "extra": "ignored"},
        quality="model_minimal",
        candidate_id=("a", "b"),
    )
...
def test_finalize_defensively_commits_tournament_selection(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        "ops_finalize_run",
        {
            "decision": "accepted",
            "summary": "done",
            "selected_attempt_id": "candidate_001",
        },
        "fin",
        c,
    )
...
def test_recovery_commits_tournament_selection_and_never_recommends_blocked_review(
    tmp_path,
):
...
def test_best_effort_selection_when_ranking_missing_but_candidate_materializable(
    tmp_path,
):
    from villani_ops.agentic.tools import commit_tournament_selection
...
def test_no_materializable_tournament_candidates_fails_clearly(tmp_path):
    ...
    s.candidates.append(
        CandidateAttemptState(
            attempt_id="candidate_001", status="completed", scope="candidate"
        )
    )
    s.tournament_ranking = _ranking("candidate_001")
...
def test_agreement_same_file_different_diff_not_same_patch(tmp_path):
    from villani_ops.agentic.tools import (
        build_candidate_evidence_packet,
        build_candidate_agreement_summary,
    )
    ...
    pathlib.Path(a2.patch_path).write_text(
        "diff --git a/run.py b/run.py\n--- a/run.py\n+++ b/run.py\n@@\n+while ready:\n+    retry()\n"
    )
    p1 = build_candidate_evidence_packet(s, a1)
    p2 = build_candidate_evidence_packet(s, a2)
...
def test_command_evidence_from_nested_debug_jsonl(tmp_path):
    ...
    (adir / "events.jsonl").write_text(
        '{"tool":"shell","input":{"command":"make test"},"exit_code":0,"stdout":"ok"}\n{"tool_call":{"name":"run_command","arguments":{"cmd":"lint"}},"returncode":1,"stderr":"bad"}\n'
    )
    a = CandidateAttemptState(
        attempt_id="c", status="completed", scope="candidate", artifacts_dir=str(adir)
    )
...
def test_fallback_pairwise_uses_signature_not_only_changed_files(tmp_path):
    from villani_ops.agentic.tools import (
        build_candidate_evidence_packet,
        _risk_review_from_summary,
        _candidate_summary_from_attempt,
        _compare_pair,
    )
    ...
    p1 = build_candidate_evidence_packet(s, a1)
    p2 = build_candidate_evidence_packet(s, a2)
    r1 = _risk_review_from_summary(_candidate_summary_from_attempt(a1), p1)
    r2 = _risk_review_from_summary(_candidate_summary_from_attempt(a2), p2)
...
def test_all_fallback_all_tie_best_effort_low_confidence(tmp_path):
    ...
    def rv(cid):
        return CandidateRiskReview(
            ...
            hidden_test_risk_score=0.5,
            recommendation="uncertain",
            rationale="fallback",
        )

    s.candidate_risk_reviews = {
        "candidate_001": rv("candidate_001"),
        "candidate_002": rv("candidate_002"),
    }
    s.pairwise_comparisons = [
        PairwiseCandidateComparison(
            candidate_a="candidate_001",
            candidate_b="candidate_002",
            winner="tie",
            confidence=0.5,
            rationale="tie",
        )
    ...
    assert rank.selection_confidence <= 0.35
...
def test_default_timeout_constant_is_1500_and_policy_uses_it():
    ...
    assert (
        OpsRunRequest(repo_path=".", task="x", timeout_seconds=42).timeout_seconds == 42
    )
...
def test_default_budget_policy_does_not_protect_pairwise_before_reviews(tmp_path):
    from villani_ops.agentic.tools import (
        _can_spend_candidate_review_budget,
        _tournament_budget_plan,
    )
    import time

    s = state(tmp_path)
    s.reserve_finalization_seconds = 90
    s.reserve_pairwise_seconds = 180
    s.reserve_ranking_seconds = 30
    s.max_candidate_review_seconds = 240
    s.per_candidate_review_timeout_seconds = 30
...
def test_default_budget_policy_does_not_exhaust_pairwise_budget(tmp_path):
    ...
    s.reserve_finalization_seconds = 90
    s.reserve_ranking_seconds = 30
    s.per_pairwise_comparison_timeout_seconds = 30
...
def test_finalization_guard_is_only_default_time_based_skip(tmp_path):
    from villani_ops.agentic.tools import (
        _can_spend_candidate_review_budget,
        _can_spend_pairwise_budget,
    )
    import time

    s = state(tmp_path)
    s.finalization_guard_seconds = 60
    review_ok, review_reason = _can_spend_candidate_review_budget(
        s, time.time() + 60, 0
    )
...
def test_planned_tournament_budget_policy_retains_protected_reserves(tmp_path):
    ...
    s = state(tmp_path)
    s.tournament_budget_policy = "planned"
    s.reserve_finalization_seconds = 90
    s.reserve_pairwise_seconds = 180
    s.reserve_ranking_seconds = 30
    s.max_candidate_review_seconds = 240
    ...
    assert not ok
    assert (
        reason == "candidate_review_skipped_protected_pairwise_or_finalization_reserve"
    )
    ok, reason = _can_spend_pairwise_budget(s, time.time() + 149)
    assert not ok
...
def test_pairwise_priority_starts_with_top_two_then_top_three():
    ...
    assert _pairwise_pairs_by_priority(["a", "b", "c", "d"])[:3] == [
        ("a", "b"),
        ("a", "c"),
        ("b", "c"),
    ]
...
def test_select_execution_path_rejected_during_planning(tmp_path):
    ...
    res = execute_tool_with_policy(
        s,
        "ops_select_execution_path",
        {"path": "candidate_tournament", "reason": "too early"},
        "sel",
        c,
    )
    assert res.is_error
...
def test_recovery_commits_stale_plan_path_and_launch_emits_recovered_event(tmp_path):
    ...
    s.execution_path = "unknown"
    s.phase = "failed"
    s.tournament_phase = "not_started"
    res = execute_tool_with_policy(
        s,
        "ops_launch_tournament_candidates",
        {"attempts": 3, "reason": "recover stale planned path"},
        "launch",
        c,
    )
    assert not res.is_error, res.content
...
def test_prompt_guidance_does_not_select_path_during_planning():
    ...
    assert (
        "select execution_path=candidate_tournament when candidate_attempts > 1"
        not in guidance
    )
