...
def obs(aid, outcome, **kw):
    data = dict(
        ...
        next_attempt_directives=["focus tests"],
        validation_snapshot_id="passed:1",
        review_snapshot_id="failed:1:True",
    )
    data.update(kw)
...
def evidenced_attempt(aid="candidate_001"):
    a = attempt(aid)
    a.validation = {"passed": False, "status": "failed", "commands": []}
...
def test_missing_completed_attempt_without_patch_observation_is_created_before_retry(
    tmp_path,
):
    s = state(tmp_path)
    a = attempt()
...
def test_validation_failed_observation_runs_focused_retry(tmp_path):
    s = state(tmp_path)
    s.candidates = [evidenced_attempt()]
    s.attempt_observations = [
        obs(
            "candidate_001",
            "validation_failed",
            should_repair=True,
            validation_snapshot_id="failed:1",
            review_snapshot_id="failed:0:True",
        )
    ]
    rec = recommend_next_agentic_action(s)
...
def test_no_patch_observation_adds_inspect_edit_directive(tmp_path):
    ...
    s.attempt_observations = [
        obs(
            ...
            "no_patch",
            changed_files=[],
            next_attempt_directives=["inspect source files first"],
            validation_snapshot_id="not_run:0",
            review_snapshot_id="not_run:0:False",
        )
    ]
    rec = recommend_next_agentic_action(s)
    assert rec.tool_name == "ops_run_next_candidate_attempt"
...
def test_allowed_next_actions_omits_next_attempt_when_budget_exhausted(tmp_path):
    ...
    a.validation = {"passed": False, "status": "failed", "commands": []}
    a.validation_status = "failed"
    a.review = {"decision": "fail", "recommended_action": "retry", "blockers": ["b"]}
    a.review_status = "failed"
    s.candidates = [a]
    s.attempt_observations = [
        obs(
            "candidate_001",
            "validation_failed",
            validation_snapshot_id="failed:0",
            review_snapshot_id="failed:0:True",
        )
    ]
...
def test_recovery_reviews_before_creating_missing_observation(tmp_path):
    s = state(tmp_path)
    a = attempt()
    a.validation = {"passed": True, "status": "passed", "commands": []}
