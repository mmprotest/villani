...
def engine(tmp_path):
    return OrchestrationEngine(
        backends={"b": Backend(name="b", provider="local", model="m")},
        execution_policy=_P(),
        runner_adapter=_R(),
        workspace=tmp_path,
    )
...
def test_quoted_command_parses_correctly(tmp_path):
    e = engine(tmp_path)
    argv, shell = e._validation_argv(
        {"cmd": 'python -c "import sys; print(sys.argv[1])" "checkout flow"'}
    )
...
def test_explicit_argv_and_shell_recorded(tmp_path):
    e = engine(tmp_path)
    assert e._validation_argv({"argv": ["python", "-c", "print(1)"]}) == (
        ["python", "-c", "print(1)"],
        False,
    )
...
def test_validation_plan_required_optional_and_artifacts(tmp_path):
    e = engine(tmp_path)
    idir = tmp_path / "integration"
    idir.mkdir()
    ctx = SimpleNamespace(
        run_dir=tmp_path,
        integration={"worktree_path": str(tmp_path)},
...
def test_required_command_failure_fails_aggregate(tmp_path):
    ...
    ctx = SimpleNamespace(
        ...
        task_context=SimpleNamespace(
            investigation={
                "validation_plan": {
                    "commands": [{"cmd": 'python -c "import sys; sys.exit(2)"'}]
                }
            }
        ),
    )
    assert not e._run_validation_plan(ctx, "validation_initial", str(tmp_path))[
        "passed"
    ]
