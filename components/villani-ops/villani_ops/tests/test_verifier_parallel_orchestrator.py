...
from villani_ops.orchestrator.verifier_parallel import (
    ...
    build_verifier_parallel_candidate_task,
)
...
def test_resolve_verifier_debug_dir_direct(tmp_path):
    ...
    _touch(debug / "session_meta.json")
    assert resolve_verifier_debug_dir(debug) == debug


def test_resolve_verifier_debug_dir_nested_timestamp(tmp_path):
    root = tmp_path / "villani_code_debug"
    nested = root / "20260705T004006_753583Z"
    _touch(nested / "session_meta.json")
    _touch(nested / "final_summary.json")
...
def test_resolve_verifier_debug_dir_prefers_valid_resolved_trace_dir(tmp_path):
    ...
    old = root / "old"
    new = root / "new"
    _touch(old / "session_meta.json", 1)
    _touch(new / "session_meta.json", 2)
    assert resolve_verifier_debug_dir(root, old) == old


def test_resolve_verifier_debug_dir_invalid_resolved_trace_falls_back_to_child(
    tmp_path,
):
    root = tmp_path / "debug_root"
    ...
    _touch(valid / "session_meta.json")
...
def test_selection_single_success_wins():
    s = select_winner(
        [{"candidateId": "a", "result": 0}, {"candidateId": "b", "result": 1}], 1
    )
...
def test_selection_seed_reproducible_and_pool_recorded():
    ...
    assert (
        select_winner(cs, 42).winnerCandidateId
        == select_winner(cs, 42).winnerCandidateId
    )
    s = select_winner(cs, 42)
    assert s.tieBreak and s.candidatePool == [f"c{i}" for i in range(5)]
...
def test_selection_failures_ignored_when_success_exists():
    assert (
        select_winner(
            [
                {"candidateId": "z", "result": 0, "confidence": 1},
                {"candidateId": "s", "result": 1, "confidence": 0},
            ],
            2,
        ).winnerCandidateId
        == "s"
...
def test_selection_all_fail_fail_no_winner():
    s = select_winner([{"candidateId": "a", "result": 0}], 1, "fail")
    assert s.winnerCandidateId is None


def test_selection_all_fail_random_and_errors_rank_below_zero():
    s = select_winner(
        [{"candidateId": "e", "result": None}, {"candidateId": "z", "result": 0}],
        3,
        "random",
    )
...
def test_selection_best_confidence():
    s = select_winner(
        [
            {"candidateId": "a", "result": 0, "confidence": 0.1},
            {"candidateId": "b", "result": 0, "confidence": 0.9},
        ],
        1,
        "best-confidence",
    )
...
def test_selection_all_errors_random():
    s = select_winner(
        [
            {"candidateId": "e1", "result": None},
            {"candidateId": "e2", "verdict": "error"},
        ],
        4,
        "random",
    )
...
class FakeRunner:
    ...
    def run_task(
        ...
        *,
        ...
        self.calls.append((repo_path, task, success_criteria, context))
        (Path(repo_path) / "a.txt").write_text(context["attempt_id"])
        dbg = artifacts_dir / "debug"
...
def test_verifier_parallel_config_stores_success_criteria():
    cfg = VerifierParallelConfig(
        repo=Path("."), task="task", success_criteria="must pass"
    )
...
def test_candidate_prompt_wrapper_preserves_task_and_criteria():
    task = "Line 1\nLine 2 exactly"
    wrapped = build_verifier_parallel_candidate_task(
        task, "criteria text", "candidate-007"
    )
...
def test_flow_creates_worktrees_verifies_integrates_and_writes_artifacts(tmp_path):
    ...
    def verifier(**kw):
        ver_calls.append(kw)
        return {
            "result": 1 if kw["repo_dir"].parent.name == "candidate-002" else 0,
            "verdict": "success",
            "confidence": 0.8,
    ...
    cfg = VerifierParallelConfig(
        repo=repo,
        task="do it",
        success_criteria="done criteria",
        candidates=3,
        parallelism=2,
        seed=1,
        workspace=ws,
    ...
    orch._backend_obj = lambda: Backend(
        name="b", provider="local", model="m", api_key="x"
    )
    out = orch.run()
    od = Path(out["orchestrationDir"])
    ...
    assert all(
        c["repo_dir"].name == "worktree"
        and c["repo_dir"].parent.name.startswith("candidate-")
        for c in ver_calls
    )
    ...
    assert (
        json.loads((od / "task.json").read_text())["success_criteria"]
        == "done criteria"
    )
...
def test_non_git_source_uses_copied_git_baseline_and_integrates(tmp_path):
    ...
    out = orch.run()
    od = Path(out["orchestrationDir"])
    cdir = od / "candidates" / "candidate-001"
    assert out["status"] == "completed"
    assert (cdir / "worktree" / ".git").exists()
    assert (cdir / "diff.patch").exists() and "a.txt" in (
        cdir / "diff.patch"
    ).read_text()
...
def test_empty_patch_candidate_is_recorded_and_verified(tmp_path):
    ...
    cfg = VerifierParallelConfig(
        repo=repo, task="noop", candidates=1, workspace=tmp_path / "ws", backend="b"
    )
...
def test_failure_no_debug_becomes_verifier_error_and_all_fail_does_not_integrate(
    tmp_path,
):
    repo = tmp_path / "repo"
    init_repo(repo)
    ...
    class BadRunner:
        def run_task(self, **kw):
            return RunnerResult(exit_code=1, stdout="", stderr="bad")

    cfg = VerifierParallelConfig(
        repo=repo, task="x", candidates=1, workspace=tmp_path / "ws", backend="b"
    )
    orch = VerifierParallelOrchestrator(cfg, runner=BadRunner())
    orch._backend_obj = lambda: Backend(
        name="b", provider="local", model="m", api_key="x"
    )
    out = orch.run()
    assert out["status"] == "failed" and out["winnerCandidateId"] is None
...
def test_cli_validation_and_json(monkeypatch, tmp_path):
    ...
    repo.mkdir()
    assert (
        r.invoke(app, ["orchestrate", "verifier-parallel", "--task", "x"]).exit_code
        != 0
    )
    assert (
        r.invoke(
...
def test_verifier_parallel_passes_runner_resolved_nested_trace_dir(tmp_path):
    ...
    class NestedRunner:
        def run_task(self, **kw):
            ...
            root = kw["artifacts_dir"] / "villani_code_debug"
            trace = root / "20260705T004006_753583Z"
            _touch(trace / "session_meta.json")
            _touch(trace / "final_summary.json")
            return RunnerResult(
                exit_code=0,
                stdout="",
                stderr="",
                debug_artifact_dir=str(root),
                resolved_trace_dir=str(trace),
                duration_ms=1,
            )
...
def test_verifier_parallel_finds_nested_trace_without_runner_resolved_trace_dir(
    tmp_path,
):
    repo = tmp_path / "plain"
    repo.mkdir()
...
def test_verifier_parallel_invalid_debug_is_candidate_error_and_other_candidate_can_win(
    tmp_path,
):
    repo = tmp_path / "plain"
    repo.mkdir()
    ...
    orch = VerifierParallelOrchestrator(
        cfg, runner=MixedDebugRunner(), verifier=verifier
    )
    orch._backend_obj = lambda: Backend(
        name="b", provider="local", model="m", api_key="x"
    )
    ...
    rows = [
        json.loads(line)
        for line in (od / "verifier-results.jsonl").read_text().splitlines()
    ]
...
def test_selection_quality_tiebreak_beats_random_for_result_one():
    candidates = [
        ...
        },
        {
            ...
            "missingEvidence": [],
            "successEvidence": [
                "Implementation appears correct from source inspection"
            ],
        },
        {
            ...
            "missingEvidence": [],
            "successEvidence": [
                "Behavioral validation test passed",
                "End-to-end runtime cleanup test passed",
            ],
            "toolsUsed": [{"tool": "read_command", "reason": "inspected validation"}],
        },
    ]
...
def test_selection_uses_stable_candidate_id_after_identical_quality_keys():
    ...
    assert (
        first.winnerCandidateId == select_winner(candidates, seed=0).winnerCandidateId
    )
    assert (
        first.winnerCandidateId == select_winner(candidates, seed=1).winnerCandidateId
    )
        ...
        [
            {
                ...
                "result": 1,
                "missingEvidence": [],
                "successEvidence": ["source inspection suggests correct"],
            },
            {
                "candidateId": "candidate-002",
                "result": 1,
                "missingEvidence": ["no final runtime validation"],
                "successEvidence": [
...
def test_selection_json_includes_quality_diagnostics(tmp_path):
    ...
    def verifier(**kw):
        ...
        return {
            ...
            "successEvidence": ["Behavioral validation test passed"]
            if cid == "candidate-002"
            else ["source inspection suggests correct"],
            "recommendedAction": "accept",
            "traceDir": str(kw["trace_dir"]),
        }
...
def test_llm_comparison_called_for_multiple_successes_and_recorded_advisory(
    ...
    def cmp(**kw):
        calls.append(kw)
        return {"selectedCandidateId": "candidate-002", "reason": "better evidence"}

    monkeypatch.setattr(
        "villani_ops.orchestrator.verifier_parallel.select_success_with_llm_comparison",
        cmp,
    )
    cfg = VerifierParallelConfig(
        repo=repo,
    ...
    orch = VerifierParallelOrchestrator(cfg, runner=runner, verifier=verifier)
    orch._backend_obj = lambda: Backend(
        name="b", provider="local", model="m", api_key="x", base_url="http://x"
    )
    out = orch.run()
    sel = json.loads((Path(out["orchestrationDir"]) / "selection.json").read_text())
...
def test_llm_comparison_cannot_override_evidence_ranked_winner(tmp_path, monkeypatch):
    ...
    def verifier(**kw):
        ...
        if cid == "candidate-001":
            return {
                ...
                "recommendedAction": "accept",
                "requirementResults": [
                    {"requirement": "core behavior", "status": "unsatisfied"}
                ],
        ...
        return {
            ...
            "requirementResults": [
                {"requirement": "core behavior", "status": "satisfied"}
            ],
            "successEvidence": [
                "pytest integration test passed validating core behavior"
            ],
            "traceDir": str(kw["trace_dir"]),
        }
    ...
    def cmp(**kw):
        ...
        return {
            ...
            "reason": "LLM preferred candidate A",
        }
    ...
    cfg = VerifierParallelConfig(
        repo=repo,
        task="task",
        success_criteria="core behavior",
        candidates=2,
        parallelism=1,
        seed=1,
        workspace=ws,
    ...
    assert (
        "LLM comparison recommended candidate-001, but evidence-ranked selector selected candidate-002"
        in report
    )
...
def test_llm_comparison_invalid_id_falls_back(tmp_path, monkeypatch):
    ...
    def verifier(**kw):
        ...
        return {
            ...
            "confidence": 0.5,
            "recommendedAction": "accept",
            "failureEvidence": ["x"] if cid == "candidate-002" else [],
            "traceDir": str(kw["trace_dir"]),
        }

    monkeypatch.setattr(
        "villani_ops.orchestrator.verifier_parallel.select_success_with_llm_comparison",
        lambda **kw: {"selectedCandidateId": "bad", "reason": "bad"},
    )
    cfg = VerifierParallelConfig(
        repo=repo,
        task="task",
...
def test_llm_comparison_not_called_for_one_or_zero_successes(tmp_path, monkeypatch):
    ...
    def verifier(**kw):
        return {
            "result": 1 if kw["repo_dir"].parent.name == "candidate-001" else 0,
            "verdict": "success",
            "confidence": 0.5,
            "recommendedAction": "accept",
            "traceDir": str(kw["trace_dir"]),
    ...
    orch2 = VerifierParallelOrchestrator(cfg2, runner=runner2, verifier=verifier0)
    orch2._backend_obj = lambda: Backend(
        name="b", provider="local", model="m", api_key="x", base_url="http://x"
    )
    out2 = orch2.run()
...
def test_evidence_ranking_prefers_cleanup_coverage_and_writes_reports(tmp_path):
    ...
    candidates = [
        {
            ...
            "confidence": 0.99,
            "successEvidence": [
                "implemented async concurrency behavior and smoke test passed"
            ],
            "requirementResults": [
                {"requirement": "support concurrent async work", "status": "satisfied"},
                {
                    "requirement": "handle interruption and cleanup resources on shutdown",
                    "status": "unsatisfied",
                },
            ],
            "missingEvidence": [
                "no explicit evidence for cleanup, cancellation, interruption, or shutdown"
            ],
            "riskFlags": [],
        ...
        },
        {
            ...
            "confidence": 0.80,
            "successEvidence": [
                "implemented async concurrency behavior and cleanup on cancellation was validated",
                "SIGINT shutdown closes resources",
            ],
            "requirementResults": [
                {"requirement": "support concurrent async work", "status": "satisfied"},
                {
    ...
    matrix = _finalize_evidence_reasons(
        build_candidate_evidence_matrix(candidates, "candidate-b"), "candidate-b"
    )
    assert (
        next(r for r in matrix if r["candidate_id"] == "candidate-b")[
            "selection_status"
        ]
        == "selected"
    )
    ...
    assert any(
        any(
            word in flag
            for word in ["cleanup", "cancellation", "interruption", "shutdown"]
        )
        for flag in row_a["missing_requirement_flags"]
    )
    matrix_path = write_candidate_evidence_matrix(
        tmp_path / "candidate_evidence_matrix.json", matrix
    )
    report_path = write_selection_report(
        tmp_path / "selection_report.md", matrix, "candidate-b"
    )
    report = report_path.read_text()
    assert (
        "candidate-b" in report
        and "Selected because" in report
        and "candidate-a" in report
    )
...
def test_verifier_parallel_evidence_matrix_uses_full_candidate_artifacts(tmp_path):
    ...
    class CommandRunner:
        def run_task(
            ...
            *,
            ...
            dbg.mkdir(parents=True)
            (dbg / "session_meta.json").write_text("{}")
            (dbg / "commands.jsonl").write_text(
                json.dumps({"command": "pytest tests/test_repo.py", "exitCode": 0})
                + "\n"
            )
            return RunnerResult(
                exit_code=0,
    ...
    def verifier(**kw):
        return {
            ...
            "recommendedAction": "accept",
            "traceDir": str(kw["trace_dir"]),
            "successEvidence": ["runtime validation observed"],
        }
...
def test_selection_json_uses_finalized_winner_reason(tmp_path):
        ...
        def run_task(
            ...
            *,
            ...
            if context["attempt_id"] == "candidate-001":
                (dbg / "commands.jsonl").write_text(
                    json.dumps(
                        {"command": "pytest tests/test_cleanup.py", "exitCode": 0}
                    )
                    + "\n"
                )
            else:
                (dbg / "commands.jsonl").write_text(
                    json.dumps({"command": 'python -c "assert True"', "exitCode": 0})
                    + "\n"
                )
            return RunnerResult(
                exit_code=0,
    ...
    assert (
        "SIGINT shutdown closes resources" in selection["reason"]
        or "pytest tests/test_cleanup.py" in selection["reason"]
    )
