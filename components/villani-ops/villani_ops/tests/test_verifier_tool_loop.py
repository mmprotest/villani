...
def test_read_debug_file_blocks_unsafe_paths():
    ...
    with pytest.raises(VerifierToolError):
        tools.read_debug_file("/etc/passwd")
    with pytest.raises(VerifierToolError):
        tools.read_debug_file("../session_meta.json")
...
def test_tool_loop_calls_search_commands(monkeypatch, tmp_path):
    ...
    class Resp:
        ...
        def json(self):
            ...
            assert (
                "Tool result for search_commands"
                in self.payload["messages"][-1]["content"]
            )
            return {
                "choices": [
                    {
                            ...
                            "content": json.dumps(
                                {
                                    ...
                                    "reason": "PASS evidence found",
                                    "criticalRequirement": "PASS behavior",
                                    "directEvidenceForCriticalRequirement": "PASS evidence",
                                    "criticalRequirementCovered": True,
                                    "requirementResults": [],
                                    "successEvidence": ["PASS evidence"],
                                    "failureEvidence": [],
                                    "recoveredFailures": [],
                                    "missingEvidence": [],
                                    "riskFlags": [],
...
def test_read_debug_file_blocks_symlink_escape(tmp_path):
    ...
    d = tmp_path / "debug"
    d.mkdir()
    (d / "session_meta.json").write_text('{"objective":"x"}')
    ...
    with pytest.raises(VerifierToolError):
        tools.read_debug_file("link.txt")
