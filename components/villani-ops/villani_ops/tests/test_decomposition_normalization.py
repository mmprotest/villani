...
from villani_ops.orchestration.planner import (
    normalize_decomposition_payload,
    DecompositionResult,
    Planner,
)
...
def test_strict_valid_decomposition_passes_without_normalization():
    payload = {
        ...
        "reason": "r",
        "subtasks": [{"id": "a", "title": "A", "objective": "Do A"}],
        "confidence": 0.7,
    }
...
def test_wrong_shaped_subtasks_description_files_normalizes():
    norm, notes = normalize_decomposition_payload(
        {
            "subtasks": [
                {
                    "id": "fix_pricing_logic",
                    "description": "Correct coupon/tax/shipping order and quantity validation.",
                    "files": ["src/signalshop/pricing.py"],
                }
            ]
        }
    )
    ...
    assert (
        norm["merge_strategy"]
        == "Apply coordinated changes in one candidate patch and validate full test suite."
    )
    st = norm["subtasks"][0]
    assert st["title"] == "Correct coupon/tax/shipping order and quantity validation."
    assert (
        st["objective"] == "Correct coupon/tax/shipping order and quantity validation."
    )
...
def test_missing_id_string_aliases_confidence_and_invalid_enums():
    norm, _ = normalize_decomposition_payload(
        {
            "tasks": [
                {"description": "Do work", "difficulty": "weird", "risk": "odd"},
                "Second task",
            ],
            "should_decompose": "yes",
            "confidence": "85%",
        }
    )
    assert norm["should_use_decomposition"] is True
...
def test_utf8_artifact_writes_and_json_preserves_unicode(tmp_path):
    ...
    assert "✅" in (tmp_path / "decomposition_normalized.json").read_text(
        encoding="utf-8"
    )
    assert (
        json.loads(
            (tmp_path / "decomposition_normalized.json").read_text(encoding="utf-8")
        )["msg"]
        == "ok ✅"
    )
...
def test_report_generation_with_unicode_and_decomposition(tmp_path):
        ...
        "subtasks": [
            {
                ...
                "title": "Correct ✅",
                "objective": "Correct ✅",
                "relevant_files": ["src/signalshop/pricing.py"],
            }
        ],
    ...
    decision = Decision(
        ...
        final_state="failed",
        final_action="fail",
        candidate_attempts_requested=1,
        decomposition=dec,
        acceptance_blockers=[],
        warnings=[],
    )
...
def test_decompose_live_path_normalizes_and_writes(tmp_path):
    raw = {
        "subtasks": [
            ...
            {
                "id": "implement_atomic_reservation",
                "description": "Make inventory reservation atomic and avoid partial stock mutation.",
                "files": ["src/signalshop/inventory.py", "src/signalshop/checkout.py"],
            },
        ]
    }
    ...
    class Client:
        def complete_json(self, *a, **k):
            return LLMCallResult(
                parsed_json=raw, raw_text=json.dumps(raw), backend_name="b", model="m"
            )

    planner = Planner(Client())
    plan = SimpleNamespace(
        model_dump=lambda mode="json": {"should_decompose": True}, should_decompose=True
    )
    task = Task(repo_path=str(tmp_path), objective="Fix checkout")
    dec, call = planner.decompose(
        task=task,
        plan=plan,
        investigation={},
        backend=Backend(name="b", provider="openai", model="m"),
        run_dir=tmp_path,
    )
...
def test_file_path_aliases_map_to_relevant_files():
    for key in ["file_paths", "affected_files", "relevant_file_paths"]:
        norm, _ = normalize_decomposition_payload(
            {"subtasks": [{"id": "a", "description": "Do A", key: ["a.py"]}]}
        )
...
def test_string_file_path_becomes_one_item_list():
    norm, _ = normalize_decomposition_payload(
        {"subtasks": [{"id": "a", "description": "Do A", "files": "a.py"}]}
    )
    assert norm["subtasks"][0]["relevant_files"] == ["a.py"]
...
def test_normalized_decomposition_artifact_preserves_relevant_files(tmp_path):
    ...
    write_json_utf8(
        tmp_path / "decomposition_normalized.json", {"normalized_payload": norm}
    )
...
def test_top_level_decomposition_list_normalizes_exact_failure_shape():
    raw = {
        "decomposition": [
            {
                "id": 1,
                "title": "Fix Cart Pricing Logic",
                "instruction": "Review and correct pricing calculations.",
                "relevant_files": ["src/signalshop/pricing.py"],
            }
        ]
    }
    ...
    assert st["relevant_files"] == ["src/signalshop/pricing.py"]
    assert (
        DecompositionResult.model_validate(norm).subtasks[0].id
        == "fix_cart_pricing_logic"
    )
...
def test_top_level_decomposition_object_extracts_subtasks():
    norm, _ = normalize_decomposition_payload(
        {"decomposition": {"subtasks": [{"title": "Fix A", "instruction": "Do A"}]}}
    )
    assert len(norm["subtasks"]) == 1
...
def test_numeric_id_without_title_and_duplicate_slugs():
    norm, _ = normalize_decomposition_payload(
        {
            "subtasks": [
                {"id": 1, "instruction": "Do one"},
                {"id": 2, "title": "Fix Pricing", "instruction": "A"},
                {"id": 3, "title": "Fix Pricing", "instruction": "B"},
            ]
        }
    )
    assert [s["id"] for s in norm["subtasks"]] == [
        "subtask_001",
        "fix_pricing",
        "fix_pricing_2",
    ]
...
def test_exact_run_failure_investigation_and_decomposition_regression():
    ...
    inv_raw = {
        "analysis": {
            "summary": "The checkout flow spans pricing, inventory, orders, and receipts.",
            "root_causes": [
                {
                    "file": "src/signalshop/pricing.py",
                    "issue": "tax before discount",
                    "fix": "apply coupon before tax",
                }
            ],
        },
        "files_to_modify": ["src/signalshop/pricing.py", "src/signalshop/inventory.py"],
        "implementation_steps": ["Fix pricing", "Fix inventory"],
    }
    inv_norm, _ = normalize_investigation_payload(inv_raw)
    assert isinstance(inv_norm["summary"], str)
    assert (
        "src/signalshop/pricing.py" in inv_norm["relevant_files"]
        and "src/signalshop/inventory.py" in inv_norm["relevant_files"]
    )
    ...
    assert (
        dec_norm["subtasks"][0]["objective"]
        == "Review and correct pricing calculations."
    )
