...
def make_ops(tmp_path, monkeypatch, reviews, strategy=None, provider=None, exit_code=0):
    ...
    monkeypatch.setattr(
        ...
        lambda self, task, backends, out_path=None: (
            TaskClassification(difficulty="easy", category="bugfix", risk="low"),
            LLMCallResult(
                parsed_json={}, raw_text="{}", backend_name="code", model="m"
            ),
        ),
    )
    strat = strategy or ExecutionStrategy(
        profile="balanced",
        attempts=[StrategyAttempt(backend="code", max_attempts=len(reviews))],
    )
    monkeypatch.setattr(
        "villani_ops.policy_engine.engine.PolicyEngine.generate",
        lambda self, cls, backends, profile, out_path=None: (
            strat,
            LLMCallResult(
                parsed_json={}, raw_text="{}", backend_name="code", model="m"
            ),
...
def transitions(run_dir):
    return [
        (x["state_before"], x["action"], x["state_after"])
        for x in map(
            json.loads, (run_dir / "controller_steps.jsonl").read_text().splitlines()
        )
    ]
...
def test_happy_path_full_lifecycle_and_report(tmp_path, monkeypatch):
    ...
    res = ops.run(
        repo, Task(repo_path=str(repo), objective="edit"), non_interactive=True
    )
    t = transitions(Path(res.run_dir))
    assert [
        ("planned", "classify", "classifying"),
        ("classifying", "generate_strategy", "planning"),
        ("planning", "run_attempt", "attempting"),
        ("attempting", "review_attempt", "reviewing"),
        ("reviewing", "decide", "deciding"),
        ("deciding", "accept", "accepted"),
    ] == t[:6]
    d = json.loads((Path(res.run_dir) / "decision.json").read_text())
    assert d["final_state"] == "accepted" and d["controller_steps"]
    report = (Path(res.run_dir) / "report.md").read_text()
    assert "| Step | Attempt | From | Action | To | Reason |" in report
    timeline = report.split("## Controller Timeline", 1)[1].split(
        "## Controller Decision Steps", 1
    )[0]
    assert "\\n" not in timeline
    assert (
        "| Step | Attempt | From | Action | To | Reason |\n| --- | --- | --- | --- | --- | --- |"
        in timeline
    )
...
def test_retry_lifecycle_records_retrying(tmp_path, monkeypatch):
    ...
    assert (
        ...
        and res.decision.retries_used == 1
        and ("deciding", "retry_same_backend", "retrying")
        in transitions(Path(res.run_dir))
    )
...
def test_escalation_lifecycle_records_escalating(tmp_path, monkeypatch):
    ...
    strat = ExecutionStrategy(
        profile="balanced",
        attempts=[StrategyAttempt(backend="code"), StrategyAttempt(backend="strong")],
    )
    ops, _ = make_ops(
        tmp_path,
        monkeypatch,
    ...
    assert (
        res.decision.accepted
        and res.decision.escalations_used == 1
        and ("deciding", "escalate", "escalating") in transitions(Path(res.run_dir))
    )
...
def test_human_accept_requested_by_reviewer_overrides_nonzero(tmp_path, monkeypatch):
    ...
    res = ops.run(
        repo, Task(repo_path=str(repo), objective="edit"), non_interactive=False
    )
...
def test_noninteractive_human_skip_does_not_override(tmp_path, monkeypatch):
    ...
    ops, _ = make_ops(
        ...
        provider=NonInteractiveHumanApprovalProvider(),
        exit_code=1,
    )
    res = ops.run(
        repo, Task(repo_path=str(repo), objective="edit"), non_interactive=True
...
def test_review_failure_goes_to_deciding_and_fails(tmp_path, monkeypatch):
    ...
    assert any(
        a == "decide" and to == "deciding"
        for _, a, to in transitions(Path(res.run_dir))
    )
...
def test_classification_failure_records_failed(tmp_path, monkeypatch):
    repo = tmp_path / "repo"
    git_repo(repo)
    ops, ws = make_ops(tmp_path, monkeypatch, [])
    monkeypatch.setattr(
        "villani_ops.classification.classifier.TaskClassifier.classify",
        lambda *a, **k: (_ for _ in ()).throw(RuntimeError("classify boom")),
    )
    res = ops.run(
        repo, Task(repo_path=str(repo), objective="edit"), non_interactive=True
    )
    assert not res.decision.accepted and transitions(Path(res.run_dir))[-1] == (
        "classifying",
        "fail",
        "failed",
    )
...
def test_policy_failure_records_failed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "villani_ops.policy_engine.engine.PolicyEngine.generate",
        lambda *a, **k: (_ for _ in ()).throw(RuntimeError("policy boom")),
    )
    res = ops.run(
        repo, Task(repo_path=str(repo), objective="edit"), non_interactive=True
    )
