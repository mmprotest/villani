...
class FakeRunner:
    def run_task(self, **kw):
        ...
        return RunnerResult(
            ...
            stdout="",
            stderr="",
            debug_artifact_dir=str(debug.parent),
            resolved_trace_dir=str(debug),
            duration_ms=1,
        )
...
def _verifier(results):
    def verifier(**kw):
        ...
        return {
            ...
            "confidence": 0.9,
            "recommendedAction": "accept" if result == 1 else "reject",
...
def _orch(
    tmp_path, mode="sequential", results=None, keep_worktrees=True, on_all_fail="fail"
):
    repo = tmp_path / "repo"
    init_repo(repo)
    ws = tmp_path / "ws"
    cls, cfgcls = (
        (VerifierSequentialOrchestrator, VerifierSequentialConfig)
        if mode == "sequential"
        else (VerifierParallelOrchestrator, VerifierParallelConfig)
    )
    kwargs = {
        "repo": repo,
        "task": "change file",
    ...
    orch = cls(
        cfgcls(**kwargs),
        runner=FakeRunner(),
        verifier=_verifier(results or {"candidate-001": 1}),
    )
...
def test_sequential_writes_materialization_artifact(tmp_path):
    ...
    mat = json.loads((od / "materialization.json").read_text())
...
def test_no_winner_records_no_winner_and_does_not_materialize(tmp_path):
    orch, repo, ws = _orch(
        tmp_path,
        "sequential",
        results={"candidate-001": 0, "candidate-002": 0},
        on_all_fail="fail",
    )
    out = orch.run()
    od = Path(out["orchestrationDir"])
