...
def test_capture_git_patch_includes_untracked_modified_deleted_and_excludes_internal(
    tmp_path,
):
    repo = _repo(tmp_path)
    ...
    (repo / ".villani").mkdir()
    (repo / ".villani" / "context_state.json").write_text("{}")
    (repo / ".villani_code").mkdir()
    (repo / ".villani_code" / "transcript.log").write_text("secret")
...
def test_generated_patch_passes_git_apply_check_against_clean_base(tmp_path):
    ...
    chk = subprocess.run(
        ["git", "apply", "--check", res.patch_path],
        cwd=clean,
        text=True,
        capture_output=True,
        timeout=10,
...
def test_invalid_custom_and_internal_patches_are_not_acceptance_eligible(tmp_path):
    ...
    bad.write_text(
        "Added file: .villani/context_state.json\n--- /dev/null\n+++ b/.villani/context_state.json\n@@ -0,0 +1 @@\n+{}\n"
    )
    attempt = {
        "status": "completed",
        "scope": "candidate",
        "exit_code": 0,
        "patch_path": str(bad),
        "changed_files": [".villani/context_state.json"],
        "review": {"decision": "pass", "recommended_action": "accept", "passed": True},
    }
