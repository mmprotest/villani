...
def test_windows_venv_python_path_preserved_by_validation_artifact(tmp_path):
    val = {
        "command": ["C:\\repo\\.venv\\Scripts\\python.exe", "-m", "pytest", "-q"],
        "python_executable": "C:\\repo\\.venv\\Scripts\\python.exe",
        "exit_code": 0,
        "passed": True,
    }
    p = tmp_path / "validation_initial.json"
...
def test_report_shows_initial_and_post_repair_validation_separately(tmp_path):
    ...
    dec = Decision(
        ...
        integration_validation_initial={
            "passed": False,
            "exit_code": 1,
            "command": [sys.executable, "-m", "pytest", "-q"],
        },
    ...
    text = write_performance_report(
        tmp_path, Task(repo_path=str(tmp_path), objective="Fix"), None, [], None, dec, 0
    ).read_text()
...
def test_repair_validation_context_records_full_required_and_optional_failures(
    ...
    idir = tmp_path / "run" / "integration"
    idir.mkdir(parents=True)
    (idir / "a.out").write_text("out1")
    (idir / "a.err").write_text("err1")
    (idir / "b.out").write_text("out2")
    (idir / "b.err").write_text("err2")
    validation = {
        "passed": False,
        "fallback": False,
        ...
        "commands": [
            {
                "cmd": 'npm test -- --grep "checkout flow"',
                "argv": ["npm", "test", "--", "--grep", "checkout flow"],
                "shell": False,
                "required": True,
                "reason": "required one",
                "returncode": 1,
                "passed": False,
                "stdout_path": str(idir / "a.out"),
                "stderr_path": str(idir / "a.err"),
            },
            {
                ...
                "returncode": 2,
                "passed": False,
                "stdout_path": str(idir / "b.out"),
                "stderr_path": str(idir / "b.err"),
            },
            {
                "cmd": "echo optional",
                "argv": ["echo", "optional"],
    ...
    monkeypatch.setattr(
        "villani_ops.orchestration.engine.capture_worktree",
        lambda worktree, out: {
            "patch_path": str(idir / "dummy.patch"),
            "changed_files": [],
        },
    )
    ctx = SimpleNamespace(
    ...
    node = ctx.graph.get("integration_repair")
    node.assigned_backend = "b"
    node.assigned_model = "m"
    e._run_validation_plan = lambda context, phase, worktree_path: {
        "passed": True,
        "commands": [],
        "fallback": False,
        "source": "test",
    ...
    assert data["required_failed_commands"][0]["argv"] == [
        "npm",
        "test",
        "--",
        "--grep",
    ...
    assert (
        "required one" in prompt and "required two" in prompt and "optional" in prompt
    )
