...
def deadlocked_state(tmp_path):
    ...
    s.phase = "running_subtasks"
    s.decomposition_accepted = True
    s.decomposition_validated = True
    a = CandidateAttemptState(
        attempt_id="a_attempt_001",
        status="failed",
        scope="subtask",
        subtask_id="a",
        failure_reason="boom",
    ...
    s.subtasks = [
        SubtaskState(
            subtask_id="a", title="A", objective="oa", status="failed", attempts=[a]
        ),
        SubtaskState(
            subtask_id="b",
            title="B",
    ...
    s.decomposed_execution_failed_subtasks = ["a"]
    s.decomposed_execution_blocked_subtasks = ["b", "c"]
    s.decomposed_execution_blockers = [
        "required_subtask_failed",
        "dependency_failed",
        "subtask_attempts_exhausted",
        "no_ready_subtasks_remaining",
        "blocked_dependents_exist",
        "decomposition_deadlocked",
    ]
    s.partial_progress = {
        "accepted_subtasks": [],
        "failed_subtasks": ["a"],
        "blocked_subtasks": ["b", "c"],
    }
    return s
...
def test_start_candidate_fallback_preserves_subtasks_and_enables_adaptive_candidate(
    tmp_path,
):
    s = deadlocked_state(tmp_path)
    c = ctx(tmp_path)
    ...
    assert (
        s.fallback_execution_path == "parallel_candidates_after_decomposition_deadlock"
    )
    assert [st.subtask_id for st in s.subtasks] == ["a", "b", "c"]
    assert s.candidates == []
    assert (
        "ops_run_next_fallback_candidate_attempt" in res.content["next_allowed_actions"]
    )
...
def test_launch_candidates_rejected_during_adaptive_fallback_by_default(tmp_path):
    s = deadlocked_state(tmp_path)
    c = ctx(tmp_path)
    assert not execute_tool_with_policy(
    ...
    blocked = execute_tool_with_policy(
        s, "ops_launch_candidates", {"attempts": 1, "reason": "legacy batch"}, "lc", c
    )
    assert blocked.is_error
    assert (
        "ops_launch_candidates is legacy/batch execution and is disabled during adaptive fallback"
        in str(blocked.content)
    )
    assert "ops_run_next_fallback_candidate_attempt" in str(blocked.content)
    assert s.candidates == []


def test_launch_candidates_direct_handler_rejects_adaptive_fallback_without_legacy_gate(
    tmp_path,
):
...
def test_launch_candidates_during_fallback_requires_explicit_legacy_gate(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        {"attempts": 1, "reason": "legacy compatibility"},
        "lc",
        c,
    )
    assert not res.is_error, res.content
...
def test_failed_finalize_clears_raw_subtask_selected_attempt(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        "ops_finalize_run",
        {
            "decision": "failed",
            "summary": "failed",
            "selected_attempt_id": "a_attempt_001",
            "blockers": ["required_subtask_failed"],
        },
        "fin",
        c,
    )
...
def test_legacy_launch_subtasks_gate_default_and_enabled(tmp_path):
    ...
    s = deadlocked_state(tmp_path)
    c = ctx(tmp_path)
    assert "ops_launch_subtasks" not in {
    ...
    blocked = execute_tool_with_policy(
        s,
        "ops_launch_subtasks",
        {"subtask_ids": ["a"], "attempts_per_subtask": 1, "reason": "legacy"},
        "legacy",
        c,
    )
    assert blocked.is_error
    ...
    allowed = execute_tool_with_policy(
        s,
        "ops_launch_subtasks",
        {"subtask_ids": ["a"], "attempts_per_subtask": 1, "reason": "legacy"},
        "legacy2",
...
def test_recovery_and_guidance_do_not_recommend_launch_candidates_during_fallback(
    tmp_path,
):
    from villani_ops.agentic.recovery import handle_no_tool_call
    ...
    s = deadlocked_state(tmp_path)
    assert not execute_tool_with_policy(
        s, "ops_start_candidate_fallback", {"reason": "deadlock"}, "fb", ctx(tmp_path)
    ).is_error
    rec = recommend_next_agentic_action(s)
