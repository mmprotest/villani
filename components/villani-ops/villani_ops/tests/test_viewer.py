...
def fake_run(tmp_path: Path):
    rd = tmp_path / "runs" / "20260626T000000Z-abc123"
                ...
                json.dumps(
                    {
                        ...
                        "payload": {"attempt_id": "candidate_001"},
                    }
                ),
                json.dumps(
                    {
                        "event_id": "3",
                        "timestamp": "2026-06-26T00:00:02+00:00",
...
def test_server_snapshot_and_path_safety(tmp_path):
    ...
    with pytest.raises(ValueError):
        safe_join_under(tmp_path, "../escape")
    srv = ViewerServer(tmp_path / "runs", port=18765).start(try_ports=1)
    try:
        assert srv.host == "127.0.0.1"
        data = json.loads(
            urllib.request.urlopen(
                srv.url(rd.name).replace("/runs/", "/api/runs/") + "/snapshot"
            ).read()
        )
...
def test_viewer_list_cli(tmp_path):
    ...
    result = CliRunner().invoke(
        app, ["viewer", "list", "--workspace", str(tmp_path / ".villani-ops")]
    )
        ...
        json.dumps(
            {
                "timestamp": "2026-06-26T00:00:00+00:00",
                "type": "run_started",
                "payload": {"model": "qwen35b"},
            }
        )
    )
    (rd / "cost_summary.json").write_text(
        json.dumps(
            {
                "input_tokens": 58342,
                "output_tokens": 27615,
                ...
                json.dumps(
                    {
                        ...
                        "payload": {"subtask_id": "st1"},
                    }
                ),
                json.dumps(
                    {
                        "timestamp": "2026-06-26T00:00:01+00:00",
                        "type": "subtask_attempt_started",
                        "payload": {
                            "subtask_id": "st1",
                            "attempt_id": "st1_attempt_001",
                        },
                    }
                ),
            ]
...
def test_viewer_graph_layout_humanized_grouped_and_non_overlapping(tmp_path):
    ...
    events = [
        ...
        {
            "timestamp": "2026-06-26T00:00:03+00:00",
            "type": "plan_submitted",
            "payload": {},
        },
        {
            "timestamp": "2026-06-26T00:00:04+00:00",
        ...
        {
            "timestamp": f"2026-06-26T00:00:0{i}+00:00",
            "type": "subtask_attempt_completed",
            "payload": {"subtask_id": f"st{i}"},
        }
        for i in range(1, 6)
    ]
    ...
    assert any(
        n["label"] == "Subtask 1" and n["status"] == "completed" for n in graph["nodes"]
    )
    assert (
        sum(
            1
            for e in graph["edges"]
            if e["source"] == "select_path" and e["target"].startswith("st")
        )
        == 0
    )
...
def test_offline_viewer_contains_required_ui_without_external_deps(tmp_path):
    ...
    assert (
        "https://" not in text
        and "cdn" not in text.lower()
        and "npm" not in text.lower()
    )


def _decision_run(tmp_path, state):
    rd = tmp_path / "runs" / state.get("run_id", "decision")
        ...
        json.dumps(
            {
                "timestamp": "2026-06-26T00:00:00+00:00",
                "type": "run_finalized",
                "payload": state.get("final_decision") or {},
            }
        )
    )
    return rd
...
def test_decision_summary_and_warnings_render_in_offline_html(tmp_path):
    rd = _decision_run(
        tmp_path,
        {
            "run_id": "decision",
            "status": "completed",
            "task": "A very long task objective that should remain fully present in the DOM",
            "runner": "villani-code-long-runner-name",
            "backend_model": "local/backend/model-name-long",
            "selection": {"selected_attempt_id": "candidate_003"},
            "selection_basis": "best_effort_tournament_selection",
            "candidates": [
                {
                    "attempt_id": "candidate_003",
                    "status": "completed",
                    "patch_path": "x.patch",
    ...
    assert (
        "A very long task objective that should remain fully present in the DOM" in html
    )
...
def test_decision_summary_states(tmp_path):
    base = {
        "run_id": "r",
        "selection": {"selected_attempt_id": "candidate_001"},
        "candidates": [
            {
                "attempt_id": "candidate_001",
    ...
    assert (
        build_viewer_snapshot(
            _decision_run(tmp_path / "a", {**base, "status": "completed"})
        )["decision"]["state"]
        == "accepted"
    )
    assert (
            ...
            _decision_run(
                tmp_path / "b",
                {
                    **base,
                    ...
                    "status": "completed",
                    "candidates": [
                        {"attempt_id": "candidate_001", "review_status": "passed"}
                    ],
                },
            )
        )["decision"]["state"]
        ...
        build_viewer_snapshot(
            _decision_run(
                tmp_path / "c",
                {
                    "run_id": "r3",
                    "status": "failed",
                    "failure_message": "Could not connect to backend http://127.0.0.1:9/v1",
                },
            )
        )["decision"]["state"]
        == "failed"
    ...
    assert (
        build_viewer_snapshot(
            _decision_run(
                tmp_path / "d",
                {"run_id": "r4", "status": "completed", "candidates": []},
            )
        )["decision"]["state"]
        == "incomplete"
    )
...
def test_graph_and_candidate_evidence_expose_winner_missing_validation_and_failure(
    tmp_path,
):
    rd = _decision_run(
        tmp_path,
        {
            ...
            "candidates": [
                ...
                {"attempt_id": "candidate_002", "status": "failed"},
            ],
        },
    )
    snap = build_viewer_snapshot(rd)
    ...
    assert any(
        n["type"] == "validation" and n["status"] == "missing"
        for n in snap["graph"]["nodes"]
    )
    assert any(
        n["id"] == "final_decision" and n["status"] == "failed"
        for n in snap["graph"]["nodes"]
    )
    assert {c["candidate_id"] for c in snap["candidate_evidence"]} == {
        "candidate_001",
        "candidate_002",
    }
...
def test_normalized_usage_statuses(tmp_path):
    rd = tmp_path / "runs" / "usage-normalized"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps({"run_id": "usage-normalized", "status": "completed"})
    )
    (rd / "runtime_events.jsonl").write_text("")
    (rd / "usage.json").write_text(
        json.dumps(
            {
                ...
                "total_tokens": 6300,
                "calls_count": 1,
                "unavailable_calls_count": 1,
            }
        )
    )
    u = build_viewer_snapshot(rd)["usage"]
    assert u["tokens"]["status"] == "available" and u["cost"]["status"] == "unavailable"
    (rd / "usage.json").write_text(
        json.dumps({"calls_count": 0, "unavailable_calls_count": 0})
    )
    u = build_viewer_snapshot(rd)["usage"]
    assert (
        u["tokens"]["status"] == "unavailable" and u["cost"]["status"] == "unavailable"
    )
    (rd / "usage.json").write_text(
        json.dumps(
            {
...
def test_provider_failure_graph_and_timeline_truthful(tmp_path):
    rd = tmp_path / "runs" / "backend-fail"
    rd.mkdir(parents=True)
    state = {
        "run_id": "backend-fail",
        "status": "failed",
        "failure_kind": "backend_connection_error",
        "failure_message": "Could not connect",
        "recoverable": True,
        "task": "Fix the failing calculator function",
        "runner": "villani-code",
    ...
    (rd / "runtime_events.jsonl").write_text(
        "\n".join(
            [
                ...
                json.dumps(
                    {
                        "timestamp": "2026-07-02T00:00:02+00:00",
                ...
                json.dumps(
                    {
                        "timestamp": "2026-07-02T00:00:03+00:00",
                        "type": "run_finalized",
                        "payload": {},
                    }
                ),
    ...
    snap = build_viewer_snapshot(rd)
    labels = " ".join(
        n["label"] + " " + n.get("subtitle", "") for n in snap["graph"]["nodes"]
    )
    assert snap["graph"]["kind"] == "provider_failure"
    assert (
        "Model request" in labels
        and "Provider failure" in labels
        and "Backend connection error" in labels
        and "backend_connection_error" not in labels
    )
    assert not any(
        x in labels for x in ["Candidates", "Validation", "Review", "Selection"]
    )
    tl = " ".join(e["title"] + " " + e.get("subtitle", "") for e in snap["timeline"])
    assert (
        "Model request started" in tl
        and "Provider failure" in tl
        and "Backend connection error" in tl
        and "Could not connect" in tl
        and "recoverable=true" in tl
    )
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
    assert (
        "Provider failure" in html
        and "Backend connection error" in html
        and "Candidates" not in json.dumps(snap["graph"])
    )
...
def test_header_uses_decision_and_cost_reasons_visible(tmp_path):
    rd = _decision_run(
        tmp_path,
        {
            "run_id": "20260702T004250Z-91ce53",
            "status": "completed",
            "task": "Fix the failing calculator function",
            "runner": "villani-code",
            "backend_model": "qwen3.6-27b",
    ...
    (rd / "usage.json").write_text(
        json.dumps({"total_tokens": 10, "calls_count": 1, "unavailable_calls_count": 1})
    )
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
    assert "Accepted with warnings" in html and "completed" in html
    assert (
        "Fix the failing calculator function" in html
        and "qwen3.6-27b" in html
        and "villani-code" in html
        and "20260702T004250Z-91ce53" in html
    )
...
def test_candidate_evidence_patch_aliases_and_changed_file_aliases(tmp_path):
    ...
    rd = _decision_run(
        tmp_path,
        {
            "run_id": "patch-aliases",
            "status": "completed",
            "candidates": [c for c, _, __ in cases],
        },
    )
...
def test_usage_normalizes_aliases_jsonl_state_runner_and_cost_reasons(tmp_path):
    rd = tmp_path / "runs" / "usage-aliases"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps(
            {
                "run_id": "usage-aliases",
                "status": "completed",
                "runner_telemetry": {
                    "tokens_in": 3,
                    "tokens_out": 4,
    ...
    (rd / "usage.jsonl").write_text(
        json.dumps({"prompt_tokens": 10, "completion_tokens": 5, "amount": 0.03}) + "\n"
    )
    u = build_viewer_snapshot(rd)["usage"]
    assert (
        u["input_tokens"] == 10 and u["output_tokens"] == 5 and u["total_tokens"] == 15
    )
...
def test_execution_graph_demoted_below_evidence_sections(tmp_path):
    ...
    assert (
        html.index("Live Event Timeline")
        < html.index("Candidate Evidence")
        < html.index("Execution Graph")
    )


def test_backend_failure_model_metadata_not_derived_from_v1_url(tmp_path):
    rd = tmp_path / "runs" / "backend-model"
    rd.mkdir(parents=True)
    state = {
        ...
        "backend_model": "qwen3.6-27b",
        "backend_name": "local",
        "backend_url": "http://127.0.0.1:9/v1",
    }
    (rd / "state.json").write_text(json.dumps(state))
    (rd / "runtime_events.jsonl").write_text(
        "\n".join(
...
def test_missing_model_renders_unknown_model(tmp_path):
    rd = tmp_path / "runs" / "missing-model"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps(
            {
...
def test_decision_summary_labels_are_humanized_and_raw_state_preserved(tmp_path):
    rd = _decision_run(
        tmp_path,
        {
            ...
            "status": "failed",
            "failure_kind": "backend_connection_error",
            "failure_message": "backend_connection_error",
            "selection_basis": "best_effort_tournament_selection",
            "final_decision": {"failure_kind": "backend_connection_error"},
        },
    )
    snap = build_viewer_snapshot(rd)
...
def test_usage_duplicate_summaries_are_not_double_counted(tmp_path):
    rd = tmp_path / "runs" / "dup-summary"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps({"run_id": "dup-summary", "status": "completed"})
    )
    (rd / "runtime_events.jsonl").write_text("")
    (rd / "usage.json").write_text(
        json.dumps(
    ...
    assert any(
        "duplicate summary ignored" in x.lower() for x in snap["usage"]["diagnostics"]
    )
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
...
def test_usage_jsonl_priority_and_call_id_dedupe(tmp_path):
    rd = tmp_path / "runs" / "jsonl-priority"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps({"run_id": "jsonl-priority", "status": "completed"})
    )
    (rd / "runtime_events.jsonl").write_text("")
    (rd / "usage.jsonl").write_text(
        "\n".join(
    ...
    (rd / "usage.json").write_text(
        json.dumps({"total_tokens": 999, "total_cost": 9.99, "calls_count": 1})
    )
    u = build_viewer_snapshot(rd)["usage"]
    assert (
        u["input_tokens"] == 30 and u["output_tokens"] == 12 and u["total_tokens"] == 42
    )
    assert round(u["total_cost"], 2) == 0.03
...
def test_candidate_lane_graph_contains_statuses_and_hooks(tmp_path):
    ...
    labels = " ".join(
        n["label"] + " " + n.get("subtitle", "") + " " + str(n.get("badge") or "")
        for n in graph["nodes"]
    )
    assert (
        "Candidate 001" in labels
        and "Candidate 002" in labels
        and "Candidate 003" in labels
    )
    assert (
        "Runner completed" in labels
        and "Review failed" in labels
        and "Validation not run" in labels
    )
    ...
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
    assert (
        "data-node-id" in html
        and "data-candidate-id" in html
        and "data-node-kind" in html
        and "data-detail-json" in html
    )
    assert (
        html.index("Live Event Timeline")
        < html.index("Candidate Evidence")
...
def test_timeline_includes_review_validation_selection_and_human_labels(tmp_path):
    rd = tmp_path / "runs" / "key-events"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps({"run_id": "key-events", "status": "completed"})
    )
    (rd / "runtime_events.jsonl").write_text(
        "\n".join(
            json.dumps(e)
            for e in [
                {
                    ...
                    "payload": {"attempt_id": "candidate_003", "result": "passed"},
                },
                {
                    "timestamp": "2026-07-02T00:00:02+00:00",
                    "type": "validation_completed",
                    "payload": {
                        "attempt_id": "candidate_003",
                        "result": "not_run",
                        "warning": "Validation did not run",
                    },
                },
                {
                    "timestamp": "2026-07-02T00:00:03+00:00",
                    ...
                    "payload": {
                        "selected_attempt_id": "candidate_003",
                        "selection_basis": "best_effort_tournament_selection",
                    },
                },
                {
                    "timestamp": "2026-07-02T00:00:04+00:00",
                    "type": "run_finalized",
                    "payload": {},
                },
            ]
    ...
    assert (
        "Review completed" in text
        and "Validation completed" in text
        and "Winner selected" in text
    )
    assert (
        "Candidate 003" in text
        and "Not run" in text
        and "Validation did not run" in text
    )
...
def test_graph_detail_card_readable_before_raw_json(tmp_path):
    ...
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
    assert (
        "graphDetailsCard" in html
        and "Raw node data" in html
        and "humanDetailLabel" in html
    )
...
def test_decision_economics_and_why_winner_for_candidate_run(tmp_path):
            ...
            "candidates": [
                {
                    ...
                    "usage": {"total_tokens": 1000, "total_cost": 0.01},
                },
                {
                    "attempt_id": "candidate_002",
                    "status": "failed",
                ...
                },
                {
                    ...
                    "validation_status": "not_run",
                    "acceptance_eligible": True,
                    "usage": {"total_tokens": 1300, "total_cost": 0.22},
                },
            ],
        },
    )
    (rd / "usage.json").write_text(
        json.dumps({"total_tokens": 2300, "total_cost": 0.23, "calls_count": 2})
    )
    snap = build_viewer_snapshot(rd)
    ...
    html = write_offline_viewer(rd).read_text(encoding="utf-8")
    assert (
        html.index("Decision Summary")
        < html.index("Decision Economics")
        < html.index("Live Event Timeline")
        < html.index("Candidate Evidence")
        < html.index("Execution Graph")
    )
    ...
    assert any(
        c["tokens"] == 1300 and c["cost"] == 0.22 and c["selected"]
        for c in snap["candidate_evidence"]
    )


def test_decision_economics_unavailable_for_no_candidate_failure_and_duration_reason(
    tmp_path,
):
    rd = tmp_path / "runs" / "short-fail"
    rd.mkdir(parents=True)
    (rd / "state.json").write_text(
        json.dumps(
            {
    ...
    (rd / "runtime_events.jsonl").write_text(
        "\n".join(
            [
                json.dumps(
                    {
                        "timestamp": "2026-07-02T00:00:00.000+00:00",
                        "type": "run_started",
                        "payload": {},
                    }
                ),
                json.dumps(
                    {
                        "timestamp": "2026-07-02T00:00:00.400+00:00",
                        "type": "provider_failure",
                        "payload": {
                            "failure_kind": "backend_connection_error",
                            "failure_message": "Could not connect",
    ...
    assert snap["usage"]["cost"]["reason"].startswith(
        "Backend failed before usage was available"
    )
...
def test_render_final_report_includes_decision_economics(tmp_path):
    ...
    (rd / "usage.json").write_text(
        json.dumps({"total_tokens": 100, "total_cost": 0.01, "calls_count": 1})
    )
