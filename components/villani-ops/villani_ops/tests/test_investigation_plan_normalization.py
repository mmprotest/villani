...
def test_investigation_findings_missing_confidence_and_no_summary_fallback_signal():
    ...
    assert notes
    bad, _ = normalize_investigation_payload({"confidence": 0.5})
...
def test_plan_aliases_and_coercions():
    norm, notes = normalize_plan_payload(
        ...
        requested_candidate_attempts=3,
    )
...
def test_plan_approach_strategy_maps_defaults_and_no_summary_fallback_signal():
    assert (
        normalize_plan_payload(
            {"approach": "do", "execution_strategy": "single"},
    ...
    assert (
        normalize_plan_payload(
            {"approach": "do", "execution_strategy": "decompose"},
            requested_candidate_attempts=2,
        )[0]["strategy"]
        == "decompose_then_execute"
    )
    norm, notes = normalize_plan_payload(
        {"approach": "do"}, requested_candidate_attempts=4
    )
    assert norm["candidate_attempts"] == 4
    bad, _ = normalize_plan_payload({"confidence": 0.8}, requested_candidate_attempts=3)
    assert not bad.get("summary")


def test_additional_investigation_aliases_and_root_cause_summary():
    norm, _ = normalize_investigation_payload(
        {"diagnosis": "diag", "cause": "cause", "file_paths": "a.py"}
    )
...
def test_additional_plan_aliases():
    norm, _ = normalize_plan_payload(
        {
            "steps": ["a", "b"],
            "strategy_name": "multiple_candidates",
            "candidate_count": "2",
        },
        requested_candidate_attempts=4,
    )
