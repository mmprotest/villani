...
def test_decomposed_graph_shape_has_subtask_and_integration_nodes():
    nodes = [
        ...
        OrchestrationNode(
            id="plan", kind="plan", objective="p", dependencies=["investigate"]
        ),
        OrchestrationNode(
            id="decompose", kind="decompose", objective="d", dependencies=["plan"]
        ),
        OrchestrationNode(
            id="subtask_fix_a_code",
            kind="code",
            objective="a",
            dependencies=["decompose"],
        ),
        OrchestrationNode(
            id="subtask_fix_a_review",
            kind="review",
            objective="ar",
            dependencies=["subtask_fix_a_code"],
        ),
        OrchestrationNode(
            id="subtask_fix_b_code",
            kind="code",
            objective="b",
            dependencies=["subtask_fix_a_review"],
        ),
        OrchestrationNode(
            id="subtask_fix_b_review",
            kind="review",
            objective="br",
            dependencies=["subtask_fix_b_code"],
        ),
        OrchestrationNode(
            id="integrate_subtasks",
            kind="integrate",
            objective="merge",
            dependencies=["subtask_fix_a_review", "subtask_fix_b_review"],
        ),
        OrchestrationNode(
...
def test_advisory_only_report_is_explicit(tmp_path):
    ...
    assert (
        "advisory-only and does not count as active decomposition" in path.read_text()
    )
...
def test_decomposition_zero_subtask_fallback_progress_is_explicit(capsys):
    ...
    assert (
        "Decomposition fallback to candidate path: no executable subtasks produced"
        in out
    )
...
def test_report_records_decomposition_fallback_reason(tmp_path):
    ...
    assert (
        "Planner requested decomposition but decomposition produced no executable subtasks."
        in text
    )
