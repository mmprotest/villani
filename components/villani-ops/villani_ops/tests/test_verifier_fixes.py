...
from villani_ops.verifier.deterministic import (
    _item,
...
from villani_ops.verifier.llm import (
    compact_verifier_packet,
    normalize_read_tool_call,
    deterministic_fallback_result,
    _parse,
    calibrate,
    prove_critical_requirement_coverage,
)


def cmd(command, stdout="", stderr="", exit=0):
    return CommandRecord(
        command=command, stdout=stdout, stderr=stderr, exitCode=exit, index=1
    )
...
def test_empty_output_validation_is_weak():
    run = DebugRun(
        debugDir="d",
        objective="Create /app/answer.txt",
        commands=[cmd("python check_something.py")],
    )
...
def test_explicit_pass_output_is_strong_when_deliverable_exercised():
    run = DebugRun(
        debugDir="d",
        objective="Create /app/answer.txt",
        commands=[cmd("pytest /app/answer.txt", "3 tests passed")],
    )
    spec = extract_deliverables(run.objective)
...
def test_crash_output_is_failure_even_when_exit_zero():
    run = DebugRun(
        debugDir="d",
        objective="Build the program",
        commands=[cmd('sh -c "./check || true"', "Segmentation fault (core dumped)")],
    )
    spec = extract_deliverables(run.objective)
...
def test_direct_test_like_helper_empty_output_not_strong():
    run = DebugRun(
        debugDir="d",
        objective="Make the filter reject alert payloads",
        commands=[cmd("python validation_helper.py")],
    )
    spec = extract_deliverables(run.objective)
...
def test_deliverable_extraction_ignores_source_content_paths():
    run = DebugRun(
        debugDir="d",
        objective="Create /app/output.json",
        toolCalls=[
            ToolCallRecord(
                toolName="write",
                status="ok",
                args={
                    ...
                    "content": 'open("/app/not_required.json", "w")',
                },
                index=1,
            )
        ],
...
def test_packet_compaction_caps_strings_and_lists_preserves_decisive_fields():
    ...
    packet = {
        "objective": long,
        "deliverableAssessment": {"missingDeliverables": ["/app/a"]},
        "deterministicChecks": {"activeFailureCount": 1},
        "evidence": {
            "activeFailures": [
                {"text": long + str(i), "kind": "failure_signal"} for i in range(20)
            ],
            "finalEndToEndValidation": [
                {"text": long + str(i), "validationStrength": "strong"}
                for i in range(20)
            ],
        },
        "candidateEvidence": {
            "candidateFailures": [{"text": long + str(i)} for i in range(20)]
        },
    }
...
def test_evidence_ids_registry_and_compaction_are_stable():
    run = DebugRun(
        ...
        status="completed",
        commands=[cmd("pytest", "3 tests passed")],
    )
    pkt1 = build_packet(run)
    pkt2 = build_packet(run)
    ids = [
        e["id"]
        for vals in pkt1["evidence"].values()
        for e in vals
    ...
    assert ids == [
        e["id"]
        for vals in pkt2["evidence"].values()
        for e in vals
        if isinstance(e, dict)
    ]
    ...
    compact_ids = [
        e["id"]
        for vals in compact["evidence"].values()
        for e in vals
...
def test_nested_tool_call_is_normalized():
    obj, err = normalize_read_tool_call(
        {"args": {"tool": "list_debug_files"}, "reason": "need files"}
    )
    assert err is None
...
def test_invalid_tool_call_rejected_without_null_tool():
    obj, err = normalize_read_tool_call(
        {"args": {}}, allowed_tools=["list_debug_files"]
    )
    assert obj is None
...
def test_exit_code_only_output_is_weak():
    ...
    assert (
        classify_validation_strength(
            cmd('python helper.py; echo "EXIT CODE: $?"', "EXIT CODE: 0"), spec
        )[0]
        == "weak"
    )
...
def test_transform_plus_display_is_weak():
    ...
    assert (
        classify_validation_strength(
            cmd(
                "python helper.py output.html && cat output.html",
                "<html>content</html>",
            ),
            spec,
        )[0]
...
def test_zero_tests_not_strong_and_zero_failed_not_failure():
    ...
    assert (
        classify_validation_strength(
            cmd("pytest", "collected 0 items\n0 failed"), spec
        )[0]
        != "strong"
    )
    assert (
        classify_validation_strength(cmd("pytest", "6 passed, 0 failed"), spec)[0]
        == "strong"
    )
...
def test_assertion_failure_is_strong_negative():
    ...
    assert (
        classify_validation_strength(
            cmd("python check.py", "AssertionError: mismatch", exit=0), spec
        )[0]
        == "failure"
    )
...
def _verdict(refs, result=1, action="accept", warnings=None):
    return {
        ...
        "reason": "ok",
        "criticalRequirement": "behavior",
        "directEvidenceForCriticalRequirement": "passed",
        "criticalRequirementCovered": True,
        "criticalRequirementEvidenceRefs": refs,
        "criticalRequirementEvidenceMatch": {
            r: {
                "matchesCriticalRequirement": True,
                "requirementCondition": "behavior",
                "evidenceCondition": "behavior",
                "whySameCondition": "same exercised behavior",
                "limitations": [],
            }
            for r in refs
        },
...
def test_parse_critical_refs_and_harden_requirement_results():
    obj = _parse(
        '{"type":"final_verdict","result":1,"verdict":"success","confidence":0.8,"recommendedAction":"accept","reason":"ok","criticalRequirementEvidenceRefs":["ev-0001"],"requirementResults":[{"id":"r","requirement":"x","status":"satisfied","evidence":[],"risks":[]},"bad"]}'
    )
    ...
    obj2 = _parse(
        '{"type":"final_verdict","result":1,"verdict":"success","recommendedAction":"accept","reason":"ok","requirementResults":"bad"}'
    )
...
def test_coverage_gate_accepts_only_concrete_evidence_and_preserves_warnings():
    for kind, prov in [
        ...
        ("runtime_observation", "tool_observation"),
        ("behavioral_check", "deterministic_analysis"),
        ("artifact_check", "file_content"),
    ]:
        out = calibrate(
    ...
    for kind, prov in [
        ("source_inspection", "source_diff"),
        ("mutation", "source_diff"),
        ("diagnostic", "command_output"),
    ]:
        out = calibrate(
            _det_with(kind, prov), _verdict(["ev-0001"], warnings=["existing"])
        )
        ...
        assert (
            "accept_downgraded_without_evidence_equivalent_critical_requirement_coverage"
            in out["warnings"]
        )
        assert "existing" in out["warnings"]
...
def test_deterministic_fallback_failure_for_active_crash():
    run = DebugRun(
        debugDir="d",
        objective="Fix project",
        commands=[cmd("./check", "Segmentation fault (core dumped)", exit=0)],
    )
...
def test_failed_validation_followed_only_by_inspection_remains_active():
    run = DebugRun(
        ...
        objective="Fix project",
        commands=[
            cmd("pytest", "AssertionError: bad", 0),
            CommandRecord(command="cat log.txt", stdout="log", exitCode=0, index=2),
        ],
    )
...
def test_failed_validation_before_mutation_and_later_strong_pass_can_recover():
    run = DebugRun(
        ...
        commands=[
            cmd("pytest", "AssertionError: bad", 0),
            CommandRecord(command="cat > fixed.txt", stdout="", exitCode=0, index=2),
            CommandRecord(
                command="pytest", stdout="3 tests passed", exitCode=0, index=3
            ),
        ],
    )
    pkt = build_packet(run)
    assert (
        pkt["deterministicChecks"]["recoveredFailureCount"] >= 1
        or pkt["deterministicChecks"]["activeFailureCount"] == 0
    )
...
def test_memory_diagnostics_still_reachable_not_failure():
    ...
    assert (
        classify_validation_strength(cmd("valgrind ./app", out), spec)[0] != "failure"
    )
...
def test_memory_diagnostics_lost_and_error_summary_are_failure():
    ...
    assert (
        classify_validation_strength(
            cmd("valgrind ./app", "definitely lost: 12 bytes in 1 blocks"), spec
        )[0]
        == "failure"
    )
    assert (
        classify_validation_strength(
            cmd("valgrind ./app", "possibly lost: 12 bytes in 1 blocks"), spec
        )[0]
        == "failure"
    )
    assert (
        classify_validation_strength(
            cmd("valgrind ./app", "ERROR SUMMARY: 3 errors from 3 contexts"), spec
        )[0]
        == "failure"
    )
    assert (
        classify_validation_strength(
            cmd("valgrind ./app", "ERROR SUMMARY: 0 errors from 0 contexts"), spec
        )[0]
        != "failure"
    )
...
def test_deliverable_extraction_ignores_command_args_flags_stdout_stderr():
    run = DebugRun(
        ...
        commands=[
            cmd(
                "g++ /app/main.cpp -L/usr/lib -I/include -o /app/release && /app/release",
                "wrote /app/from_stdout.txt",
                "error references /app/from_stderr.txt",
            )
        ],
        finalSummary={"changed_files": ["/app/main.cpp"]},
    )
    spec = extract_deliverables(run.objective, run)
...
def test_compact_tool_result_text_truncates_and_repeated_summary_short():
    ...
    assert _tool_cache_key("read_debug_file", {"path": "a"}) == _tool_cache_key(
        "read_debug_file", {"path": "a"}
    )
...
def test_evidence_equivalence_missing_false_and_material_limitations_downgrade():
    for match in [
        ...
        {
            "ev-0001": {
                "matchesCriticalRequirement": False,
                "requirementCondition": "edge",
                "evidenceCondition": "normal",
                "whySameCondition": "",
                "limitations": ["weaker nearby"],
            }
        },
        {
            "ev-0001": {
                "matchesCriticalRequirement": True,
                "requirementCondition": "edge",
                "evidenceCondition": "edge",
                "whySameCondition": "same",
                "limitations": ["partial only"],
            }
        },
    ]:
        v = _verdict(["ev-0001"])
...
def test_nearby_condition_concrete_validation_downgrades_but_same_condition_accepts():
    ...
    v["criticalRequirementEvidenceMatch"] = {
        "ev-0001": {
            "matchesCriticalRequirement": False,
            "requirementCondition": "abnormal edge condition",
            "evidenceCondition": "normal nearby condition",
            "whySameCondition": "",
            "limitations": ["tests weaker nearby condition"],
        }
    }
    out = calibrate(_det_with(), v)
    assert out["recommendedAction"] == "escalate"
    ...
    v["criticalRequirementEvidenceMatch"] = {
        "ev-0001": {
            "matchesCriticalRequirement": True,
            "requirementCondition": "abnormal edge condition",
            "evidenceCondition": "abnormal edge condition",
            "whySameCondition": "validation exercises the same edge condition",
            "limitations": [],
        }
    }
    out = calibrate(_det_with(), v)
...
def test_parser_hardens_critical_requirement_evidence_match_shapes():
    obj = _parse(
        '{"type":"final_verdict","result":1,"verdict":"success","recommendedAction":"accept","reason":"ok","criticalRequirementEvidenceMatch":"bad","requirementResults":[]}'
    )
    assert obj["criticalRequirementEvidenceMatch"] == {}
    assert "invalid_critical_requirement_evidence_match_shape" in obj["warnings"]
    obj = _parse(
        '{"type":"final_verdict","result":1,"verdict":"success","recommendedAction":"accept","reason":"ok","criticalRequirementEvidenceMatch":{"ev-1":"bad","ev-2":{"matchesCriticalRequirement":"true","requirementCondition":"r","evidenceCondition":"e","whySameCondition":"w","limitations":"partial"}},"requirementResults":[]}'
    )
    assert "ev-1" not in obj["criticalRequirementEvidenceMatch"]
    assert (
        obj["criticalRequirementEvidenceMatch"]["ev-2"]["matchesCriticalRequirement"]
        is False
    )
...
def test_packet_compaction_preserves_registry_ids_summaries_and_caps_raw():
            ...
            "testValidation": [
                {
                    "id": "ev-0001",
                    "text": long,
                    "summary": "edge condition validation",
                    "condition": "edge",
                }
            ]
        },
    ...
    assert (
        compact["evidenceRegistry"]["ev-0001"]["summary"] == "edge condition validation"
    )
...
def test_deterministic_fallback_inconclusive_for_diagnostic_and_tool_loop_only():
    det = {
        "evidenceByCategory": {
            "activeFailures": [
                {"id": "d", "kind": "diagnostic", "diagnosticOnly": True}
            ]
        },
    ...
    fb = deterministic_fallback_result(
        {"evidenceByCategory": {}, "deliverableAssessment": {}}, "tool loop failed"
    )
    assert fb["result"] is None
