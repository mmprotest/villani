...
def make_ops(tmp_path, monkeypatch, reviews, selection=None):
    ...
    def classify(self, task, backends, out_path=None, backend_override=None):
        ...
        if out_path:
            Path(out_path).write_text(c.model_dump_json(indent=2))
        return c, LLMCallResult(
    ...
    def investigate(self, task, cls, backend_name, backend, run_dir):
        ...
        ).InvestigationResult(summary="look")
        Path(run_dir / "investigation.raw.txt").write_text("{}")
        Path(run_dir / "investigation_normalized.json").write_text(
            '{"normalized": false, "notes": [], "payload": {"summary": "look"}}'
        )
        Path(run_dir / "investigation.json").write_text(inv.model_dump_json(indent=2))
        return inv, LLMCallResult(
            parsed_json={}, raw_text="{}", backend_name="code", model="m"
        )

    monkeypatch.setattr(
        "villani_ops.performance.investigator.Investigator.investigate", investigate
    )
    q = list(reviews)
...
def test_run_rejects_policy(tmp_path):
    res = runner.invoke(
        app, ["run", "--repo", str(tmp_path), "--task", "x", "--policy", "balanced"]
    )
...
def test_performance_artifacts_and_all_candidates(tmp_path, monkeypatch):
    ...
    for rel in [
        ...
        "classification.json",
        "investigation.json",
        "attempts/attempt_001/attempt.json",
        "attempts/attempt_001/review.json",
        "selection.json",
        "decision.json",
        "report.md",
    ]:
...
def test_selector_cannot_choose_ineligible_fallbacks(tmp_path, monkeypatch):
    ...
    ops, ws = make_ops(
        ...
        monkeypatch,
        [
            ReviewResult(decision="fail", recommended_action="fail", score=0.1),
            ReviewResult(
                passed=True, decision="pass", recommended_action="accept", score=0.8
            ),
        ],
        SelectionResult(decision="select", selected_attempt_id="attempt_001"),
    )
    res = ops.run(
        repo,
        Task(repo_path=str(repo), objective="edit"),
...
def test_no_enabled_backend_errors():
    ...
    try:
        select_performance_backend(
            {
                "x": Backend(
                    name="x", provider="openai-compatible", model="m", enabled=False
                )
            }
        )
...
def test_run_rejects_backend_human_and_unknown(tmp_path):
    cases = [
        (
            ["--backend", "local-qwen"],
            "Performance orchestration always uses the most capable enabled backend",
        ),
        (
            ["--human-approval"],
            "Human approval is not supported in performance orchestration",
        ),
...
def test_live_run_writes_normalized_node_artifacts(tmp_path, monkeypatch):
    ...
    rd = Path(res.run_dir)
    for rel in [
        "investigation.raw.txt",
        "investigation_normalized.json",
        "investigation.json",
        "plan.raw.txt",
        "plan_normalized.json",
        "plan.json",
        "selection_normalized.json",
    ]:
        assert (rd / rel).exists()
    graph = json.loads((rd / "orchestration_graph.json").read_text())
    by_id = {n["id"]: n for n in graph["nodes"]}
    assert (
        by_id["investigate"]["artifacts"]["normalized"]
        == "investigation_normalized.json"
    )
