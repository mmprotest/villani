...
def ctx(patch="p.diff", changed=None, reasons=None):
    return HumanApprovalPromptContext(
        ...
        patch_path=patch,
        changed_files=["f.txt"] if changed is None else changed,
        cost_so_far=0.1,
...
def test_noninteractive_provider_never_overrides():
    ...
    assert (
        r.decision == "skipped"
        and not r.prompted
        and r.skipped_reason == "non_interactive"
        and not r.valid_override
    )
...
def test_test_provider_accept_without_patch_not_valid():
    assert (
        not TestHumanApprovalProvider("accept")
        .request_approval(ctx(patch=None))
        .valid_override
    )
...
def test_acceptance_valid_human_override_requires_changed_files():
    a = {
        ...
        "patch_path": "p.diff",
        "changed_files": [],
        "human_approval": {"decision": "accept", "valid_override": True},
        "review": {
            "passed": False,
            "decision": "uncertain",
