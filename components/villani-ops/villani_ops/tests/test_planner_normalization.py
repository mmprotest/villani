...
def _plan(payload, requested=3, **kw):
    norm, notes = normalize_plan_payload(
        payload, requested_candidate_attempts=requested, **kw
    )
...
def test_summary_aliases():
    cases = [
        ({"plan": ["Inspect failing tests", "Fix pricing"]}, "Inspect failing tests"),
        ({"plan": "Do the plan"}, "Do the plan"),
        ({"steps": ["a", "b"]}, "a; b"),
        ({"approach": "Use modules"}, "Use modules"),
        ({"thought": "Task spans files"}, "Task spans files"),
        ({"analysis": "Analyze bug"}, "Analyze bug"),
    ]
...
def test_strategy_aliases():
    assert (
        _plan({"summary": "x", "execution_strategy": "parallel"})[0].strategy
        == "parallel_candidates"
    )
    assert (
        _plan({"summary": "x", "execution_strategy": "single"})[0].strategy
        == "single_task"
    )
    assert (
        _plan({"summary": "x", "execution_strategy": "decompose"})[0].strategy
    ...
    plan, _ = _plan(
        {
            "subtasks": [
                "Fix pricing",
                {"title": "Fix inventory", "objective": "Rollback reservations"},
            ]
        }
    )
...
def test_resulting_state_files_drive_decomposition_and_action_shape():
    payload = {
        "thought": "The task spans checkout, pricing, inventory, orders, and receipts.",
        "command": "ls -R",
        "resulting_state": {
            "files": [
                "src/signalshop/pricing.py",
                "src/signalshop/inventory.py",
                "src/signalshop/checkout.py",
                "src/signalshop/orders.py",
            ]
        },
    }
...
class FakeClient:
    ...
    def complete_json(self, *args, **kwargs):
        return LLMCallResult(
            parsed_json=self.payload, raw_text=self.raw, backend_name="b", model="m"
        )


def test_normalized_planner_does_not_set_fallback_and_writes_artifact(tmp_path):
    planner = Planner(FakeClient({"plan": ["a", "b"], "confidence": 0.8}))
    plan, _ = planner.plan(
        ...
        mode="performance",
        backend_name="b",
        backend=Backend(name="b", provider="local", model="m", base_url="http://x"),
        run_dir=tmp_path,
    )
    ...
    assert (
        json.loads((tmp_path / "plan_normalized.json").read_text())[
            "planner_normalized"
        ]
        is True
    )
...
def test_strict_and_fallback_write_plan_normalized(tmp_path):
    ...
    Planner(FakeClient(strict)).plan(
        task=Task(repo_path=str(tmp_path), objective="x"),
        classification={},
        investigation={},
        repo_summary=None,
    ...
    bad = tmp_path / "bad"
    bad.mkdir()
    plan, _ = Planner(FakeClient({"confidence": 0.5})).plan(
        task=Task(repo_path=str(tmp_path), objective="x"),
        classification={},
        investigation={},
        repo_summary=None,
    ...
    assert (
        json.loads((bad / "plan_normalized.json").read_text())["planner_fallback_used"]
        is True
    )


def test_progress_output_includes_normalized(capsys):
    ConsoleProgressReporter().node_completed(
        SimpleNamespace(kind="plan"),
        {
...
def test_report_plan_metadata_and_no_bare_fallback_used(tmp_path):
    dec = Decision(
        ...
        plan={
            ...
            "confidence": 0.6,
            "planner_normalized": True,
            "planner_normalization_notes": ["Mapped plan"],
            "planner_fallback_used": False,
        },
    )
    report = write_performance_report(
        tmp_path,
        Task(repo_path=str(tmp_path), objective="fix"),
        None,
        [],
        None,
        dec,
...
def test_unusable_payload_requires_fallback_signal():
    norm, _ = normalize_plan_payload(
        {"confidence": 0.2}, requested_candidate_attempts=3
    )
    try:
...
def test_regression_action_shaped_payload_requests_decompose_node(
    ...
    repo.mkdir()
    subprocess.run(
        ["git", "init"], cwd=repo, check=True, capture_output=True, timeout=10
    )
    subprocess.run(
        ["git", "config", "user.email", "a@b.c"], cwd=repo, check=True, timeout=10
    )
    subprocess.run(
        ["git", "config", "user.name", "A"], cwd=repo, check=True, timeout=10
    )
    ...
    monkeypatch.setattr(
        ...
        lambda self, task, backends, out_path=None, backend_override=None, **kw: (
            task.classification
            or __import__(
                "villani_ops.core.task", fromlist=["TaskClassification"]
            ).TaskClassification(difficulty="medium", category="bugfix", risk="medium"),
            LLMCallResult(parsed_json={}, raw_text="{}", backend_name="b", model="m"),
        ),
    )
    inv_cls = __import__(
        "villani_ops.performance.models", fromlist=["InvestigationResult"]
    ).InvestigationResult
    monkeypatch.setattr(
        "villani_ops.performance.investigator.Investigator.investigate",
        lambda self, task, cls, backend_name, backend, run_dir, **kw: (
            inv_cls(summary="look"),
            LLMCallResult(parsed_json={}, raw_text="{}", backend_name="b", model="m"),
        ),
    )
    monkeypatch.setattr(
        "villani_ops.orchestration.planner.Planner.decompose",
        lambda self, **kw: (
            DecompositionResult(
                should_use_decomposition=True, reason="decomposed", confidence=0.8
            ),
            LLMCallResult(parsed_json={}, raw_text="{}", backend_name="b", model="m"),
        ),
    )
    monkeypatch.setattr(
        "villani_ops.review.reviewer.LLMReviewer.review",
        lambda self, *a, **kw: (
            __import__(
                "villani_ops.review.reviewer", fromlist=["ReviewResult"]
            ).ReviewResult(decision="fail", recommended_action="fail"),
            LLMCallResult(parsed_json={}, raw_text="{}", backend_name="b", model="m"),
        ),
    )
    runner = SimpleNamespace(
        name="villani-code",
        run_task=lambda **kw: RunnerResult(
            exit_code=0, stdout="", stderr="", duration_ms=1
        ),
    )
    engine = OrchestrationEngine(
        backends={"b": backend},
        execution_policy=policy_for_mode("performance"),
    ...
    res = engine.run(
        repo=repo,
        task=Task(repo_path=str(repo), objective="fix checkout"),
        candidate_attempts=3,
        classify=True,
    )
...
def test_plan_repair_regression_checkout_context_does_not_inject_domain_decomposition():
    ...
    inv_raw = {
        "summary": "Task spans checkout flow failures.",
        "files_to_modify": classification["likely_files"],
        "implementation_steps": [
            "Fix pricing",
            "Fix inventory reservation",
            "Fix payment rollback",
            "Fix receipt rendering",
        ],
    }
    inv_norm, _ = normalize_investigation_payload(inv_raw)
    plan = PlanResult(
        summary="Fix failing checkout tests directly.",
    ...
    repaired, notes = repair_plan_against_context(
        plan,
        requested_candidate_attempts=1,
        task="Fix the failing tests",
        success_criteria="Fix the failing tests. The checkout flow must correctly price carts, reserve inventory atomically, create orders with stable IDs, release reservations on payment failure, and render deterministic receipts. Tests pass and diff is minimal.",
        classification=classification,
        investigation=inv_norm,
    )
    assert len(inv_norm["relevant_files"]) == 5 and inv_norm["confidence"] >= 0.70
...
def test_plan_repair_not_for_narrow_task():
    ...
    repaired, notes = repair_plan_against_context(
        plan,
        requested_candidate_attempts=2,
        task="fix typo",
        success_criteria="tests pass",
        classification={"likely_files": ["a.py"], "estimated_attempts_needed": 1},
        investigation={"relevant_files": ["a.py"], "confidence": 0.9},
    )
