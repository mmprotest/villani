...
def test_acceptance_requires_successful_runner_and_passing_review(tmp_path):
    ...
    base = {
        ...
        "review": {"passed": True, "decision": "pass", "recommended_action": "accept"},
    }
    ...
    bad = {
        **base,
        "review": {"passed": False, "decision": "fail", "recommended_action": "fail"},
    }
    ...
    human = {
        **bad,
        ...
        "patch_path": str(patch),
        "changed_files": ["hello.txt"],
        "human_approval": {"decision": "accept"},
    }
        ...
        **human,
        "human_approval": {
            ...
            "shown_evidence": {
                ...
                "acceptance_blockers": ["review decision is uncertain"],
            },
        },
    }
...
class FakeClient:
    ...
    def complete_json(self, *args, **kwargs):
        return LLMCallResult(
            raw_text="{}",
            parsed_json={"profile": "cheap", "attempts": self.attempts},
            input_tokens=1,
            output_tokens=1,
            estimated_cost=0,
            backend_name="policy",
...
def test_policy_normalizes_cheap_and_filters_disabled_non_coding():
    backs = {
        "cheap": _backend("cheap", cap=10, in_cost=0, out_cost=0),
        "exp": _backend("exp", cap=90, in_cost=10, out_cost=10),
        "off": _backend("off", enabled=False),
        "review": _backend("review", roles=("policy",), cap=99),
    }
    cls = TaskClassification(difficulty="easy", category="bugfix", risk="low")
    strat, _ = PolicyEngine(
        FakeClient(
            [
                {"backend": "exp", "max_attempts": 1},
                {"backend": "off", "max_attempts": 1},
                {"backend": "review", "max_attempts": 1},
            ]
        )
    ).generate(cls, backs, "cheap")
...
def test_quality_hard_starts_highest_capability():
    backs = {
        "cheap": _backend("cheap", cap=10, in_cost=0, out_cost=0),
        "exp": _backend("exp", cap=90, in_cost=10, out_cost=10),
        "policy": _backend("policy", roles=("policy",), cap=1),
    }
    cls = TaskClassification(difficulty="hard", category="bugfix", risk="high")
    strat, _ = PolicyEngine(
        FakeClient([{"backend": "cheap", "max_attempts": 1}])
    ).generate(cls, backs, "quality")
