...
def _validate(raw, profile="balanced"):
    return ExecutionStrategy.model_validate(
        normalize_execution_strategy_payload(raw, profile)
    )
            ...
            "execution_phases": [
                {
                    ...
                    "assigned_backend": "qwen9b",
                    "max_attempts": 1,
                    "instructions": "Use the cheaper backend first.",
                },
                {
                    "phase": "escalation",
                    "assigned_backend": "qwen35b",
                    "max_attempts": 1,
                    "instructions": "Escalate only if needed.",
                },
            ],
...
def test_execution_phases_preferred_over_backend_sequence():
    strategy = _validate(
        {
            "backend_sequence": ["qwen35b", "qwen9b"],
            "execution_phases": [
                {"assigned_backend": "qwen9b", "max_attempts": 1},
                {"assigned_backend": "qwen35b", "max_attempts": 1},
            ],
        }
    )
    assert [a.backend for a in strategy.attempts] == ["qwen9b", "qwen35b"]
...
def test_cli_profile_wins():
    strategy = _validate(
        {"profile": "quality", "backend_sequence": ["qwen35b"]}, "balanced"
    )
    assert strategy.profile == "balanced"
...
def test_invalid_planning_fields_trigger_fallback_artifact(tmp_path):
    ...
    strategy, _ = PolicyEngine(
        _Client(
            {"execution_phases": [{"phase": "initial_attempt", "assigned_backend": ""}]}
        )
    ).generate(
        TaskClassification(difficulty="medium", risk="medium"),
        backs,
        "balanced",
        tmp_path / "execution_strategy.json",
    )
    artifact = json.loads(
        (tmp_path / "controller_calls" / "policy_error.json").read_text()
    )
    assert artifact["fallback_used"] is True
    assert (
        artifact["fallback_reason"]
        if "fallback_reason" in artifact
        else artifact["validation_error"]
    )
...
def test_fallback_artifact_accurate_on_validation_failure(tmp_path):
    ...
    PolicyEngine(_Client({"attempts": "not-a-list"})).generate(
        TaskClassification(), backs, "balanced", tmp_path / "execution_strategy.json"
    )
    artifact = json.loads(
        (tmp_path / "controller_calls" / "policy_error.json").read_text()
    )
...
def test_real_shape_does_not_use_fallback(tmp_path):
    ...
    backs = {"qwen9b": _backend("qwen9b", 20), "qwen35b": _backend("qwen35b", 90)}
    strategy, _ = PolicyEngine(_Client(payload)).generate(
        TaskClassification(difficulty="medium", risk="medium"),
        backs,
        "balanced",
        tmp_path / "execution_strategy.json",
    ...
    assert [a.backend for a in strategy.attempts] == ["qwen9b", "qwen35b"]
    assert not any(
        "deterministic fallback" in w or "Policy engine produced no valid attempts" in w
