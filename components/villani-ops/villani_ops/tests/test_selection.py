    ...
    s = select_winner(
        [
            ...
            },
            {
                ...
                "result": 1,
                "recommendedAction": "inspect_manually",
                "successEvidence": ["runtime validation passed"],
            },
        ],
        seed=1,
    )
...
def test_accept_tie_is_seed_deterministic():
    ...
    assert (
        select_winner(cs, 99).winnerCandidateId
        == select_winner(cs, 99).winnerCandidateId
    )
...
def test_comparison_packet_includes_and_truncates(tmp_path):
    ...
    c = SimpleNamespace(
        ...
        verifier_result={
            ...
            "recommendedAction": "accept",
            "reason": "summary",
            "criticalRequirement": "edge case",
            "directEvidenceForCriticalRequirement": "targeted test",
            "criticalRequirementCovered": True,
            "criticalRequirementEvidenceRefs": ["ev-0001"],
            "criticalRequirementCoverageProven": True,
            "successEvidence": ["B" * 1000],
        },
    )
...
def test_comparison_packet_includes_critical_requirement_fields():
    packet = build_llm_comparison_packet(
        [
            {
                "candidateId": "c1",
                "result": 1,
                "recommendedAction": "inspect_manually",
                "criticalRequirement": "rollback",
                "directEvidenceForCriticalRequirement": "rollback test passed",
                "criticalRequirementCovered": True,
            }
        ]
    )
...
def test_no_candidate_with_coverage_proven_falls_back_without_fabricating():
    ...
    assert all(
        not row["criticalRequirementCoverageProven"] for row in s.candidateQuality
    )
        ...
        [
            {
                ...
                "criticalRequirementCovered": True,
                "criticalRequirementCoverageProven": False,
                "criticalRequirementEvidenceRefs": ["ev-1"],
                "criticalRequirementEvidenceMatch": {
                    "ev-1": {"matchesCriticalRequirement": False}
                },
            },
            {
                "candidateId": "same",
                ...
                "criticalRequirementCovered": True,
                "criticalRequirementCoverageProven": True,
                "criticalRequirementEvidenceRefs": ["ev-2"],
                "criticalRequirementEvidenceMatch": {
                    "ev-2": {"matchesCriticalRequirement": True}
                },
            },
        ],
        seed=1,
...
def test_llm_comparison_packet_includes_evidence_match():
    ...
    assert (
        packet[0]["verifier"]["criticalRequirementEvidenceMatch"]["ev-1"][
...
def test_evidence_extraction_reads_debug_commands_jsonl(tmp_path):
    ...
    (d / "session_meta.json").write_text(
        json.dumps({"tool_input": {"command": "echo meta"}})
    )
    rows = [
    ...
    c = SimpleNamespace(
        candidate_id="candidate-001",
        debug_dir=d,
        changed_files=["src/app"],
        verifier_result={"result": 1, "toolsUsed": ["not-real-tool"]},
    )
...
def test_structured_record_metadata_can_mark_validation(tmp_path):
    ...
    row = build_candidate_evidence_matrix(
        [
            SimpleNamespace(
                candidate_id="c",
                debug_dir=debug,
                changed_files=[],
                verifier_result={"result": 1},
            )
        ]
    )[0]
...
def test_repo_vs_candidate_classification_uses_changed_file_overlap():
    ...
    assert (
        _classify_test_source("run-tests checks/existing_cleanup_case", changed_files)
        == "repo"
    )
    assert (
        _classify_test_source("run-tests checks/generated_cleanup_case", changed_files)
        == "candidate"
    )
...
def test_unknown_when_validation_source_is_ambiguous():
    from villani_ops.orchestrator.selection import (
        _classify_test_source,
        _command_looks_like_validation,
    )
...
def test_report_contains_specific_winner_and_loser_evidence(tmp_path):
    ...
    matrix = [
        {
            ...
            "commands_run": ["run-tests checks/cleanup_case"],
            "tests_run": [
                {
                    "command": "run-tests checks/cleanup_case",
                    "passed": True,
                    "source": "repo",
                }
            ],
            "files_changed": [],
            "direct_behavioral_evidence": [
                {
                    "requirement": "cleanup",
                    "evidence": "interruption shutdown closes resources",
                    "strength": "strong",
                }
            ],
        ...
        },
        {
            ...
            "missing_requirement_flags": ["no explicit cleanup/cancellation evidence"],
            "risk_flags": ["resource leak risk"],
            "evidence_score": {
                "direct_behavioral": 0,
                "repo_tests": 0,
                "candidate_tests": 0,
    ...
    text = write_selection_report(
        tmp_path / "selection_report.md", matrix, "winner"
    ).read_text()
...
def test_lexicographic_tie_break_replaces_ord_sum():
    ...
    candidates = [
        {
            "candidateId": "candidate-ba",
            "result": 1,
            "successEvidence": ["runtime validation passed"],
        },
        {
            "candidateId": "candidate-ab",
            "result": 1,
            "successEvidence": ["runtime validation passed"],
        },
    ]
...
def test_evidence_ranking_prefers_repo_test_over_candidate_microtest(tmp_path):
    ...
    da.mkdir()
    db.mkdir()
    (da / "commands.jsonl").write_text(
        json.dumps({"command": "validate --case /tmp/generated_case", "exitCode": 0})
        + "\n"
    )
    (db / "commands.jsonl").write_text(
        json.dumps({"command": "run-tests checks/cleanup_case", "exitCode": 0}) + "\n"
    )
    a = SimpleNamespace(
        candidate_id="A",
        debug_dir=da,
        changed_files=["checks/generated_case"],
        verifier_result={
            "result": 1,
            "confidence": 0.99,
            "successEvidence": ["implemented code added"],
        },
    )
    b = SimpleNamespace(
        candidate_id="B",
        ...
        verifier_result={
            "result": 1,
            "confidence": 0.1,
            "successEvidence": ["runtime validation observed cleanup behavior"],
        },
    )
...
def test_evidence_matrix_records_gaps_when_no_artifact_commands():
    ...
    row = build_candidate_evidence_matrix(
        [{"candidateId": "c", "result": 1, "successEvidence": ["implemented code"]}]
    )[0]
