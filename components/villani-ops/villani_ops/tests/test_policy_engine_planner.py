...
def narrow_cls(category="bug_fix", difficulty="medium", risk="medium", confidence=0.9):
    return TaskClassification(
        difficulty=difficulty,
        risk=risk,
        category=category,
        confidence=confidence,
        relevant_file_paths=["tests/test_foo.py", "src/pkg/foo.py"],
        task_shape_signals={
            "relevant_file_count": 2,
            "explicit_tests_mentioned": True,
            "failing_tests_mentioned": True,
...
def test_max_attempts_is_separate_from_policy():
    strat = plan_execution_strategy(
        {"small": b("small", 35, 0.1), "large": b("large", 70, 2)},
        TaskClassification(difficulty="medium", risk="low", category="bug_fix"),
        "cheap",
        max_attempts=3,
    )
...
def test_cheap_starts_with_cheapest_viable_backend():
    backs = {
        "small": b("small", 20, 0.01),
        "medium": b("medium", 45, 0.1),
        "large": b("large", 80, 1),
    }
    strat = plan_execution_strategy(
        backs,
        TaskClassification(difficulty="easy", risk="low"),
        "cheap",
        max_attempts=2,
    )
...
def test_cheap_skips_hopeless_backend():
    backs = {"tiny": b("tiny", 1, 0.001), "qwen35b": b("qwen35b", 51, 0.1)}
    cls = TaskClassification(
        difficulty="hard",
        risk="high",
        confidence=0.2,
        task_shape_signals={
            "relevant_file_count": 9,
...
def test_cheap_retry_threshold_allows_reasonable_retry():
    ...
    strat = plan_execution_strategy(
        {"qwen9b": b("qwen9b", 24, 0.01), "qwen35b": b("qwen35b", 51, 0.1)},
        cls,
        "cheap",
        max_attempts=3,
    )
...
def test_balanced_starts_cheap_but_escalates_sooner():
    backs = {"qwen9b": b("qwen9b", 24, 0.01), "qwen35b": b("qwen35b", 51, 0.1)}
...
def test_quality_uses_strongest_backend_repeatedly():
    backs = {"qwen9b": b("qwen9b", 24, 0.01), "qwen35b": b("qwen35b", 51, 0.1)}
    strat = plan_execution_strategy(
        backs, TaskClassification(), "quality", max_attempts=3
    )
...
def test_required_capability_estimate():
    assert (
        estimate_required_capability(
            TaskClassification(difficulty="easy", risk="low", category="bug_fix")
        )
        == 15
    )
    assert (
        estimate_required_capability(
            TaskClassification(difficulty="medium", risk="low", category="feature")
        )
        == 30
    )
    assert (
        estimate_required_capability(
            TaskClassification(difficulty="medium", risk="medium", category="test_fix")
        )
        == 33
    )
    assert (
        estimate_required_capability(
            TaskClassification(difficulty="hard", risk="high", category="security")
        )
        == 63
    )
...
def test_p_solve_is_not_cliffy_at_minus_21():
    ...
    assert estimate_backend_solve_probability(b("gap", 24), cls, 45) >= 0.15
...
def test_task_shape_bonus_is_category_agnostic():
    ...
    assert estimate_backend_solve_probability(
        b("qwen9b", 24), a, estimate_required_capability(a)
    ) == estimate_backend_solve_probability(
        b("qwen9b", 24), c, estimate_required_capability(c)
    )


def test_policy_plan_includes_estimates():
    strat = plan_execution_strategy(
        {"small": b("small", 35, 0.01)},
        TaskClassification(),
        "balanced",
        max_attempts=1,
    )
...
def test_no_enabled_coding_backend():
    with pytest.raises(ValueError, match="No enabled coding backends available"):
        plan_execution_strategy(
            {"review": b("review", 90, 1, roles=("review",))},
            TaskClassification(),
            "balanced",
        )
...
def test_cli_accepts_max_attempts_with_mocked_run(monkeypatch, tmp_path):
    class D:
        ...
        final_state = "failed"
        final_action = "fail"
        classification = {"difficulty": "medium", "category": "bug_fix", "risk": "low"}
        execution_strategy = {"max_attempts": 4, "attempts": [{}, {}]}
        attempts_used = 0
        retries_used = 0
        escalations_used = 0
        human_reviews_requested = 0
        human_reviews_skipped = 0
    ...
    monkeypatch.setattr(
        "villani_ops.cost_policy.executor.CostPolicyVillaniOps.run", fake_run
    )
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
