from villani_ops.orchestration.progress import (
...
class N:
    def __init__(self, kind, assigned_backend="code", assigned_model="m"):
        self.kind = kind
        self.assigned_backend = assigned_backend
...
def test_default_progress_prints_candidate_review_selector_final(capsys, tmp_path):
    ...
    r.candidate_started("attempt_001", 1, 3, "qwen35b")
    r.candidate_completed(
        "attempt_001",
        1,
        3,
        {"exit_code": 0, "changed_files": ["a.py"], "patch_path": str(patch)},
    )
    r.review_completed(
        "attempt_001",
        1,
    ...
    r.selector_completed(
        SelectionResult(decision="select", selected_attempt_id="attempt_002"),
        ["Normalized selected_candidate_id to selected_attempt_id=attempt_002"],
    )
...
def test_quiet_progress_suppresses_updates(capsys):
    ...
    r.start_run(
        run_dir="x", mode="performance", runner="villani-code", candidate_attempts=1
    )
    r.final_decision(False, reason="No candidate passed acceptance gates.")
    assert capsys.readouterr().out == ""


def test_verbose_progress_prints_backend(capsys):
    r = ConsoleProgressReporter(verbose=True)
    r.node_started(N("classify"))
...
def test_fallback_line_printed(capsys):
    r = ConsoleProgressReporter()
    r.fallback_used("invalid selected attempt", "attempt_001")
...
def test_patch_progress_checks_non_empty_content(capsys, tmp_path):
    ...
    r.candidate_completed(
        ...
        1,
        1,
        {"exit_code": 0, "changed_files": ["a.py"], "patch_path": str(p)},
    )
    p.write_text("   \n")
    r.candidate_completed(
        "attempt_001",
    ...
    r.candidate_completed(
        "attempt_001", 1, 1, {"exit_code": 0, "changed_files": ["a.py"]}
    )
    out = capsys.readouterr().out
...
def test_normalization_and_fallback_progress(capsys):
    ...
    r.selector_completed(
        SelectionResult(
            ...
            selector_reason_synthesized=True,
        ),
        [],
    )
    r.node_completed(
...
def _ev(type, payload=None):
    return OpsEvent(
        event_id="e", run_id="r", timestamp="now", type=type, payload=payload or {}
    )
...
def test_agentic_progress_prints_key_events(capsys):
    ...
    for ev in [
        ...
        _ev(
            ...
            {"attempt_id": "pricing_attempt_001", "subtask_id": "pricing"},
        ),
        _ev(
    ...
    assert (
        "[agentic] Subtask pricing attempt pricing_attempt_001 completed: exit_code=0"
        in out
    )
...
def test_agentic_progress_quiet_suppresses(capsys):
    AgenticProgressReporter(enabled=False).on_event(
        _ev("run_started", {"run_dir": "runs/r"})
    )
    assert capsys.readouterr().out == ""
    ...
    r.on_event(
        _ev(
            "candidate_attempt_started",
            {
                ...
                "backend_name": "qwen35b",
                "artifact_paths": {"stdout": "✅.log"},
            },
        )
    )
    out = capsys.readouterr().out
