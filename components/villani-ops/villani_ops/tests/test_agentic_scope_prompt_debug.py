...
from villani_ops.agentic.tools import (
    build_subtask_runner_prompt,
    import_villani_code_debug_evidence,
    assess_scope_compliance,
...
def test_subtask_prompt_sections_and_scope_rules():
    st = SubtaskState(
        ...
        success_criteria="Parser test passes",
        relevant_files=["src/parser.py"],
    )
    prompt = build_subtask_runner_prompt(
        parent_task="parent",
        parent_success_criteria="all parent criteria",
        subtask=st,
        allowed_files=st.relevant_files,
        forbidden_files=[".villani", ".villani_code"],
        validation_commands=["python -m pytest tests/test_parser.py -v"],
        dependency_context=None,
        merge_contract=None,
    )
    assert "ONE Villani Ops subtask, not the whole parent task" in prompt
    assert (
        "Parent success criteria are provided only so you understand the larger system"
        in prompt
    )
...
def test_import_villani_code_debug_evidence_filters_validation_commands(tmp_path):
    ...
    adir.mkdir()
    (adir / "commands.jsonl").write_text(
        json.dumps({"command": "ls", "exit_code": 0})
        + "\n"
        + json.dumps(
            {
    ...
    a = CandidateAttemptState(
        attempt_id="a", status="completed", scope="candidate", artifacts_dir=str(adir)
    )
...
def test_scope_assessment_subtask_extra_files_require_exception():
    blocked = assess_scope_compliance(
        scope="subtask",
    ...
    internal = assess_scope_compliance(
        scope="candidate",
        changed_files=[".villani/log"],
        allowed_files=[],
        scope_exception_text=None,
        subtask=None,
    )
...
def test_imported_failed_validation_blocks_candidate_acceptance_and_review_payload(
    tmp_path,
):
    ...
    state = OpsRunState(
        ...
        execution_path="parallel_candidates",
    )
