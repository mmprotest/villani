...
def test_extract_usage_variants_and_missing():
    ...
    assert extract_usage_tokens({"usage": {"input_tokens": 2, "output_tokens": 4}}) == {
        "input_tokens": 2,
        "output_tokens": 4,
        "total_tokens": 6,
    }
    assert extract_usage_tokens({"usage": {"input_tokens": 7, "output_tokens": 8}}) == {
        "input_tokens": 7,
        "output_tokens": 8,
        "total_tokens": 15,
    }
...
def test_usage_recorder_writes_artifacts_thread_safe(tmp_path):
    ...
    rec.write_artifacts()
    assert (
        len((tmp_path / "usage.jsonl").read_text(encoding="utf-8").splitlines()) == 20
    )
...
def test_runner_missing_usage_records_unavailable():
    r = usage_record_from_runner(
        run_id="r",
        phase="candidate_attempt",
        role="coding",
        backend=backend(),
        result=SimpleNamespace(
            input_tokens=0, output_tokens=0, token_accounting_status="missing"
        ),
