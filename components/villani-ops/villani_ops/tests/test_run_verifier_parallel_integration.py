...
from villani_ops.orchestrator.verifier_parallel import VerifierParallelOrchestrator
from villani_ops.materialize import (
    discover_latest_verifier_parallel_state,
    wrapper_exit_code,
)
...
def _ok_run(orchestration_dir="o"):
    return {
        ...
        "status": "completed",
        "winnerCandidateId": "candidate-001",
        "winnerResult": 1,
        "selectionPath": "s",
        "integrationPath": "i",
...
def test_existing_orchestrator_values_still_validate(monkeypatch, tmp_path):
    ...
    class FakeDecision:
        accepted = True
        mode = "performance"
        performance_backend_name = None
        performance_backend_model = None
        decomposition_executed = False
        candidate_attempts_requested = 1
        candidate_attempts_completed = 1
        winning_attempt_id = "attempt_001"
        reason = "ok"
    ...
    for orch in ["adaptive", "agentic", "graph"]:
        res = runner.invoke(
            app,
            ["run", "--repo", str(tmp_path), "--task", "x", "--orchestrator", orch],
            catch_exceptions=False,
        )
...
def test_unknown_orchestrator_lists_verifier_parallel(tmp_path):
    res = runner.invoke(
        app, ["run", "--repo", str(tmp_path), "--task", "x", "--orchestrator", "nope"]
    )
...
def test_verifier_parallel_dispatch_skips_legacy_paths(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(VerifierParallelOrchestrator, "run", fake_run)
    monkeypatch.setattr(
        "villani_ops.agentic.OpsRunner",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("agentic touched")),
    )
    import villani_ops.cli.main as main
    ...
    monkeypatch.setattr(
        main,
        "VillaniOps",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("graph touched")),
    )
    res = runner.invoke(
        app,
        [
...
def test_task_file_backend_and_verifier_backend_mapping(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda self: captured.append(self.config) or _ok_run(),
    )
    res = runner.invoke(
        app,
        [
...
def test_materialization_discovers_verifier_parallel_artifacts(tmp_path):
    od = tmp_path / "ws" / "orchestrations" / "orch-1"
    od.mkdir(parents=True)
    (od / "selection.json").write_text(
        json.dumps({"winnerCandidateId": "candidate-001"})
    )
    (od / "integration.json").write_text(
        json.dumps({"status": "integrated", "winnerCandidateId": "candidate-001"})
    )
    (od / "orchestration.json").write_text(
        json.dumps({"status": "completed", "winnerCandidateId": "candidate-001"})
    )
...
def test_wrapper_exit_code_does_not_swallow_startup_failure_without_acceptance(
    tmp_path,
):
...
def test_wrapper_exit_code_allows_failed_process_only_after_accepted_materialization(
    tmp_path,
):
    od = tmp_path / "ws" / "orchestrations" / "orch-1"
    od.mkdir(parents=True)
