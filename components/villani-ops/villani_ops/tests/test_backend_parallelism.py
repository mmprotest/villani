...
from villani_ops.orchestration.engine import OrchestrationEngine, _now
...
def test_backend_yaml_writes_max_parallel(tmp_path):
    ...
    s.save_backends(
        {"b": Backend(name="b", provider="openai", model="m", max_parallel=2)}
    )
...
def test_limiter_caps_per_backend_and_releases_after_failure():
    limiter = BackendConcurrencyLimiter(
        {"a": Backend(name="a", provider="openai", model="m", max_parallel=2)}
    )
    active = 0
    observed = 0
    lock = threading.Lock()
...
def test_parallel_subtask_code_nodes_observe_limit_distinct_worktrees_and_artifact(
    tmp_path,
):
    backends = {"b": Backend(name="b", provider="openai", model="m", max_parallel=2)}
    engine = _ParallelProbeEngine(
        backends=backends,
    ...
    nodes = [
        OrchestrationNode(id=f"subtask_{sid}_code", kind="code", objective=sid)
    ...
    context = SimpleNamespace(
        ...
        task_context=SimpleNamespace(
            classification=None, investigation=None, plan=None, decomposition=None
        ),
        parallel_execution={
            "enabled": True,
            "backend_limits": {"b": 2},
...
def test_parallel_normal_candidate_code_nodes_observe_backend_limit_and_artifact(
    tmp_path,
):
    backends = {"b": Backend(name="b", provider="openai", model="m", max_parallel=2)}
    engine = _CandidateParallelProbeEngine(
        backends=backends,
        execution_policy=_Policy(),
        runner_adapter=_Runner(),
        workspace=tmp_path,
    ...
    nodes = [
        OrchestrationNode(id=f"code_attempt_{i:03d}", kind="code", objective=str(i))
        for i in range(1, 4)
    ]
    graph = OrchestrationGraph(run_id="r", nodes=nodes)
    context = SimpleNamespace(
    ...
    assert context.parallel_execution["started_attempts"] == [
        "code_attempt_001",
        "code_attempt_002",
        "code_attempt_003",
    ]
    assert sorted(context.parallel_execution["completed_attempts"]) == [
        "code_attempt_001",
        "code_attempt_002",
        "code_attempt_003",
    ]
...
class _E2EProbeEngine(OrchestrationEngine):
    ...
    def _event(self, node, action):
        with self.lock:
            self.events.append((node.id if node else None, action, time.time()))
    ...
    def _execute_classify_node(self, node, context):
        ...
        return self._finish(
            context,
            node,
            NodeExecutionResult(node_id=node.id, status="succeeded", data={}),
            {},
        )
    ...
    def _execute_plan_node(self, node, context):
        ...
        context.plan = p
        context.task_context.plan = p.model_dump(mode="json")
        return self._finish(
            context,
            node,
            NodeExecutionResult(
                node_id=node.id, status="succeeded", data=context.task_context.plan
            ),
    ...
    def _execute_decompose_node(self, node, context):
        ...
        return self._finish(
            ...
            node,
            NodeExecutionResult(
                node_id=node.id, status="skipped", data={"intentional_skip": True}
            ),
    ...
    def _execute_code_node(self, node, context):
        ...
        time.sleep(
            {
                "code_attempt_001": 0.01,
                "code_attempt_002": 0.01,
                "code_attempt_003": 0.06,
            }.get(node.id, 0.03)
        )
        aid = node.id.replace("code_", "")
        adir = context.run_dir / "attempts" / aid
        adir.mkdir(parents=True, exist_ok=True)
        wt = context.run_dir / "worktrees" / aid
        wt.mkdir(parents=True, exist_ok=True)
        patch = adir / "patch.diff"
        status = self.outcomes.get(aid, "accept")
        if status != "code_fail":
        ...
        meta = {
            ...
            "patch_path": str(patch) if patch.exists() else None,
            "changed_files": ["f"] if patch.exists() else [],
            "input_tokens": 10,
            "output_tokens": 5,
            "token_accounting_status": "reported",
        }
        if status == "code_fail":
            meta.update(
                {
        ...
        node.result = meta
        self._add_usage(context, "coding", 10, 5, 0.25)
        with self.lock:
            self.active -= 1
        ...
        return self._finish(
            ...
            NodeExecutionResult(
                node_id=node.id,
                status="failed" if status == "code_fail" else "succeeded",
                data=meta,
                error="code failed" if status == "code_fail" else None,
            ),
            meta,
        )
    ...
    def _execute_review_node(self, node, context):
        ...
        time.sleep(
            {"attempt_001": 0.01, "attempt_002": 0.01, "attempt_003": 0.08}.get(
                aid, 0.01
            )
        )
        ...
        eligible = (
            self.outcomes.get(aid, "accept") == "accept"
            and meta.get("exit_code") == 0
            and meta.get("patch_path")
        )
        meta.update(
            {
                "review": {
                    "decision": "pass" if eligible else "fail",
                    "recommended_action": "accept" if eligible else "fail",
                    "summary": "r",
                    "score": 0.9 if eligible else 0.1,
                },
                "acceptance_eligible": bool(eligible),
                "acceptance_blockers": [] if eligible else ["rejected"],
                "candidate_summary": {
                    "attempt_id": aid,
                    "acceptance_eligible": bool(eligible),
                },
        ...
        self._add_usage(context, "review", 1, 2, 0.05)
        ...
        res = self._finish(
            context,
            node,
            NodeExecutionResult(node_id=node.id, status="succeeded", data=meta),
            meta,
        )
        self._event(node, "end")
        return res
    ...
    def _execute_select_node(self, node, context):
        ...
        eligible = [a for a in context.attempts if a.get("acceptance_eligible")]
        sel = eligible[-1]["attempt_id"] if eligible else None
        context.selection = SelectionResult(
            decision="select" if sel else "reject_all",
            selected_attempt_id=sel,
            summary="s",
            confidence=1.0,
        )
        context.winner = eligible[-1] if eligible else None
        return self._finish(
            context,
            node,
            NodeExecutionResult(
                node_id=node.id,
                status="succeeded",
                data=context.selection.model_dump(mode="json"),
            ),
    ...
    def _execute_verify_node(self, node, context):
        ...
        return self._finish(
            context,
            node,
            NodeExecutionResult(node_id=node.id, status="succeeded", data=data),
            data,
        )
...
def _run_e2e(tmp_path, attempts, max_parallel, outcomes=None):
    ...
    res = e.run(
        repo=repo,
        task=Task(repo_path=str(repo), objective="x"),
...
def git_repo_simple(path):
    ...
    path.mkdir()
    subprocess.run(
        ["git", "init"], cwd=path, check=True, capture_output=True, timeout=10
    )
    subprocess.run(
        ["git", "config", "user.email", "a@b.c"], cwd=path, check=True, timeout=10
    )
    subprocess.run(
        ["git", "config", "user.name", "A"], cwd=path, check=True, timeout=10
    )
    (path / "f").write_text("x\n")
...
def test_e2e_parallel_candidate_scheduler_max_two(tmp_path):
    ...
    assert e.max_seen == 2
    assert {
        n for n, a, t in e.events if n and n.startswith("code_attempt") and a == "start"
    } == {"code_attempt_001", "code_attempt_002", "code_attempt_003"}
    assert len({a["worktree_path"] for a in res.attempts}) == 3
    assert len({a["patch_path"] for a in res.attempts}) == 3
    for i in range(1, 4):
        assert _event_time(e, f"review_attempt_{i:03d}", "start") > _event_time(
            e, f"code_attempt_{i:03d}", "end"
        )
    ...
    assert (
        pe["enabled"] is True
        and pe["candidate_attempts"] == 3
        and pe["max_observed_parallelism"] == 2
    )
...
def test_e2e_parallel_candidate_scheduler_max_one(tmp_path):
    ...
    assert (
        json.loads((rd / "candidates" / "parallel_execution.json").read_text())[
...
def test_e2e_mixed_candidate_outcomes_selects_accepted_candidate_after_all_reviews(
    tmp_path,
):
    e, res, rd = _run_e2e(
        tmp_path,
        3,
        2,
        {"attempt_001": "code_fail", "attempt_002": "reject", "attempt_003": "accept"},
    )
    ...
    assert all(
        statuses[f"review_attempt_{i:03d}"] in {"succeeded", "failed", "skipped"}
        for i in range(1, 4)
    )
...
def test_parallel_candidate_shared_state_totals_and_json_artifacts(tmp_path):
    ...
    graph = json.loads((rd / "orchestration_graph.json").read_text())
    assert all(
        n["status"] in {"succeeded", "failed", "skipped"}
        for n in graph["nodes"]
        if n["id"].startswith("code_attempt_")
    )
    pe = json.loads((rd / "candidates" / "parallel_execution.json").read_text())
    assert (
        len(pe["started_attempts"])
        == len(pe["completed_attempts"])
        == len(pe["results"])
        == 3
    )
