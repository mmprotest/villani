...
def test_backend_save_load(tmp_path):
    s = FileStorage(tmp_path / ".villani-ops")
    s.init_workspace()
    b = Backend(
        name="local",
        provider="local",
    ...
    s.save_backends({"local": b})
    assert s.load_backends()["local"].model == "m"


def test_policy_load_save(tmp_path):
    p = Policy(name="p", attempts=[AttemptPlan(backend="b")])
    path = tmp_path / "p.yaml"
    p.save(path)
    assert Policy.load(path).attempts[0].backend == "b"


def test_init_creates_workspace(tmp_path):
    s = FileStorage(tmp_path / ".villani-ops")
    s.init_workspace()
    assert (s.workspace / "config.yaml").exists() and (s.workspace / "runs").exists()
...
def test_copy_isolation_does_not_mutate_source(tmp_path):
    ...
    dst = CopyIsolation().create(src, tmp_path / "run" / "repo")
...
def test_diff_capture_detects_modified_file(tmp_path):
    ...
    out = capture_diff(a, b, tmp_path / "d.patch")
    assert "-one" in out.read_text() and "+two" in out.read_text()
...
def test_selector_score_and_tie_break():
    a1 = Attempt(
        ...
        runner_name="shell",
        estimated_cost=1,
        validation=ValidationResult(passed=True, score=0.8, summary="", validator="t"),
    )
    a2 = Attempt(
        attempt_id="a2",
        run_id="r",
        ...
        runner_name="shell",
        estimated_cost=2,
        validation=ValidationResult(passed=True, score=0.9, summary="", validator="t"),
    )
    assert (
        select_attempt("r", [a1, a2], Policy(name="p").selection).winning_attempt_id
        == "a2"
    )
    a2.validation.score = 0.8
    assert (
        select_attempt("r", [a1, a2], Policy(name="p").selection).winning_attempt_id
        == "a1"
    )
...
def test_cli_init_backend_runner(tmp_path, monkeypatch):
    ...
    res = runner.invoke(app, ["backend", "list"])
    assert res.exit_code == 0 and "local" in res.output
    assert (
        runner.invoke(
            app, ["runner", "set", "shell", "--command", "python x.py"]
        ).exit_code
        == 0
    )
    res = runner.invoke(app, ["runner", "list"])
    assert res.exit_code == 0 and "python x.py" in res.output
