...
def summary(**kw):
    data = {
        "run_id": "20260622T191913_880358Z",
        "status": "completed",
        "started_at": "2026-06-22T19:19:13Z",
        "duration_ms": 103098,
        "turn_count": 4,
        "total_tool_calls": 3,
        "tool_calls_by_name": {"Ls": 1, "Write": 1, "Read": 1},
        "total_file_reads": 1,
        "total_file_writes": 1,
        "model_requests": 4,
        "model_failures": 0,
...
def test_parser_reads_summary_and_verifies_model_responses(tmp_path):
    ...
    write_jsonl(
        tmp_path / "model_responses.jsonl",
        [
            {
                "usage": {
                    "prompt_tokens": i,
    ...
    write_jsonl(
        ...
        [
            {"tool_name": "Ls"},
            {"tool_name": "Write", "tool_category": "file_mutation"},
            {"tool_name": "Read", "normalized_args_summary": {"file_path": "a.py"}},
        ],
    )
...
def test_parser_detects_mismatch(tmp_path):
    (tmp_path / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=10, tokens_output=20))
    )
    write_jsonl(
        tmp_path / "model_responses.jsonl",
...
def test_ls_is_not_substantive_file_read(tmp_path):
    ...
    write_jsonl(
        tmp_path / "tool_calls.jsonl",
        [
            {"tool_name": "Ls", "started_at": "2026-06-22T19:19:14Z"},
            {"tool_name": "Write", "started_at": "2026-06-22T19:19:15Z"},
            {
                "tool_name": "Read",
                "started_at": "2026-06-22T19:19:16Z",
                "normalized_args_summary": {"file_path": "x"},
            },
        ],
    )
    t = parse_villani_code_debug_artifact(tmp_path)
...
def test_malformed_jsonl_does_not_crash(tmp_path):
    (tmp_path / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=1, tokens_output=2))
    )
    write_jsonl(
        tmp_path / "model_responses.jsonl",
        [
...
def test_parser_sorts_mixed_timezone_and_missing_timestamps_safely(tmp_path):
    (tmp_path / "final_summary.json").write_text(
        json.dumps(summary(total_tool_calls=5, total_file_reads=1, total_file_writes=1))
    )
    write_jsonl(
        tmp_path / "model_responses.jsonl",
        [
    ...
    write_jsonl(
        ...
        [
            ...
            {
                "tool_name": "Read",
                "started_at": "2026-06-22T19:19:13.100000+00:00",
                "normalized_args_summary": {"file_path": "x.py"},
            },
            {
                "tool_name": "Write",
                "tool_category": "file_mutation",
                "started_at": "2026-06-22T19:19:14.100000",
            },
            {
                "tool_name": "Bash",
...
def test_parser_treats_malformed_timestamps_like_missing_timestamps(tmp_path):
    (tmp_path / "final_summary.json").write_text(
        json.dumps(summary(total_tool_calls=3, total_file_reads=1, total_file_writes=1))
    )
    write_jsonl(
        tmp_path / "model_responses.jsonl",
        [
    ...
    assert t.first_substantive_file_read_tool_index == 3
    assert any(
        "Malformed tool call timestamp" in w for w in t.token_accounting_warnings
    )
...
def test_parser_resolves_nested_trace_dir(tmp_path):
    ...
    (trace / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=7, tokens_output=3))
    )
    write_jsonl(
        trace / "model_responses.jsonl",
        [{"usage": {"prompt_tokens": 7, "completion_tokens": 3, "total_tokens": 10}}],
    )
    write_jsonl(
        trace / "tool_calls.jsonl",
        [{"tool_name": "Read", "normalized_args_summary": {"file_path": "x.py"}}],
    )
...
def test_parser_chooses_newest_nested_trace_dir(tmp_path):
    ...
    new.mkdir()
    (old / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=1, tokens_output=1))
    )
    (new / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=9, tokens_output=2))
    )
    import os

    os.utime(old / "final_summary.json", (1, 1))
    os.utime(new / "final_summary.json", (2, 2))
    write_jsonl(
        new / "model_responses.jsonl",
...
def test_parser_supports_response_wrapped_usage(tmp_path):
    (tmp_path / "final_summary.json").write_text(
        json.dumps(summary(tokens_input=10, tokens_output=5))
    )
    write_jsonl(
        tmp_path / "model_responses.jsonl",
        [
...
def test_parser_supports_compatible_token_keys(tmp_path):
    ...
    write_jsonl(
        ...
        [
            ...
            {
                "payload": {
                    "usage": {"tokens_input": 6, "tokens_output": 3, "tokens_total": 9}
                }
            },
        ],
    )
