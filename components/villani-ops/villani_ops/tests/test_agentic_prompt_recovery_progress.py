...
def test_recovery_prompt_names_likely_select_path_tool(tmp_path):
    ...
    assert (
        'Call ops_select_execution_path with path="decomposed_subtasks"'
        in rr.message["content"]
    )
...
def test_progress_reports_deterministic_recovery_action():
    ...
    reporter.on_event(
        OpsEvent(
            ...
            type="recovery_deterministic_action_executed",
            payload={
                "tool_name": "ops_select_execution_path",
                "tool_input": {"path": "decomposed_subtasks"},
