...
from villani_ops.agentic.tools import (
    build_adaptive_subtask_runner_prompt,
...
class FailReviewer:
    def review(self, *, state, attempt, scope):
        ...
        return {
            ...
            "score": 0.2,
            "summary": "discount order is wrong",
            "evidence": ["coupon after tax"],
            "issues": ["ordering"],
            "blockers": ["discount_order_wrong"],
        }
...
def make_state(tmp_path):
    ...
    run.mkdir()
    (repo / "tests").mkdir()
    (repo / "tests" / "test_pricing.py").write_text("def test_x():\n    assert False\n")
    return OpsRunState(
        run_id="r",
        run_dir=str(run),
        repo_path=str(repo),
        task="fix parser and pricing",
        success_criteria="all behaviours pass",
        mode="performance",
        runner="villani-code",
        ...
        investigation={
            ...
            "validation_plan": {
                "commands": [
                    {"cmd": 'python -c "import sys; sys.exit(0)"', "purpose": "global"}
                ]
            },
        },
        plan={"summary": "decompose", "strategy": "decompose_then_execute"},
        decomposition={"merge_strategy": "merge minimal patches"},
        decomposition_validated=True,
        decomposition_accepted=True,
        execution_path="decomposed_subtasks",
        subtasks=[
            SubtaskState(
                subtask_id="pricing",
                title="pricing",
                objective="fix pricing only",
                relevant_files=["src/pricing.py"],
            )
        ],
    )
...
def make_ctx(tmp_path, runner=None, reviewer=None):
    ...
    return OpsToolContext(
        ...
        transcript=[],
        runner_adapter=runner or SubtaskRunner(),
        reviewer=reviewer or FailReviewer(),
        backend=b,
        backend_name="b1",
        coding_backend=b,
...
def test_ops_run_next_subtask_attempt_runs_one_and_observes(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        {"subtask_id": "pricing", "reason": "first"},
        "x",
        c,
    )
...
def test_subtask_retry_prompt_includes_first_failure_and_do_not_repeat(tmp_path):
    ...
    prompt = build_adaptive_subtask_runner_prompt(
        ...
        reason="retry",
        repair=True,
        base_attempt_id="pricing_attempt_001",
    )
...
def test_subtask_observation_refresh_idempotent_and_distinct_attempts_count(tmp_path):
    ...
    h_observe_completed_attempt(
        s, OpsObserveCompletedAttemptInput(attempt_id=aid, reason="refresh"), c
    )
    assert len(s.attempt_observations) == 1
    assert s.backend_assessments["b1"]["attempts"] == 1
    execute_tool_with_policy(
        s,
        "ops_run_next_subtask_attempt",
        {"subtask_id": "pricing", "reason": "second"},
        "y",
        c,
    )
...
def test_deadlock_fallback_runs_adaptive_candidate_with_learnings(tmp_path):
    ...
    s.attempt_observations = [
        AttemptObservation(
            attempt_id="pricing_attempt_001",
            scope="subtask",
            subtask_id="pricing",
            backend_name="b1",
    ...
    s.decomposed_execution_blockers = ["decomposition_deadlocked"]
    ...
    st.attempts = [
        CandidateAttemptState(
            attempt_id=f"pricing_attempt_{i:03d}",
            status="failed",
            scope="subtask",
            subtask_id="pricing",
        )
    ...
    res = execute_tool_with_policy(
        s, "ops_start_candidate_fallback", {"reason": "deadlock"}, "fb", c
    )
    assert not res.is_error, res.content
    assert len(s.candidates) == 0
    res = execute_tool_with_policy(
        s, "ops_run_next_fallback_candidate_attempt", {"reason": "deadlock"}, "fb2", c
    )
    assert not res.is_error, res.content
...
def test_fallback_retry_prompt_includes_previous_failure_feedback(tmp_path):
    ...
    execute_tool_with_policy(
        s, "ops_run_next_fallback_candidate_attempt", {"reason": "first"}, "fb1", c
    )
    assert len(s.attempt_observations) == 1
    execute_tool_with_policy(
        s,
        "ops_run_next_fallback_candidate_attempt",
        {"reason": "retry", "base_attempt_id": "candidate_001", "repair": True},
        "fb2",
        c,
    )
    ...
    def run_task(
        ...
        *,
        ...
        target = Path(repo_path) / (
            "src/parser.py" if sid == "parser" else "src/checkout.py"
        )
        target.parent.mkdir(parents=True, exist_ok=True)
        prior = (
            (Path(repo_path) / "src/parser.py").read_text()
            if (Path(repo_path) / "src/parser.py").exists()
            else ""
        )
...
def make_two_subtask_state(tmp_path):
    ...
    run.mkdir()
    (repo / "src").mkdir()
    (repo / "src" / "parser.py").write_text("# base parser\n")
    (repo / "src" / "checkout.py").write_text("# base checkout\n")
    return OpsRunState(
        run_id="r",
        run_dir=str(run),
        repo_path=str(repo),
        task="fix parser then checkout",
        success_criteria="all behaviours pass",
        mode="performance",
        runner="villani-code",
        candidate_attempts=1,
        investigation={"summary": "multi"},
        plan={"summary": "decompose", "strategy": "decompose_then_execute"},
        decomposition={"merge_strategy": "merge minimal patches"},
        decomposition_validated=True,
        decomposition_accepted=True,
...
def test_accepted_subtask_patch_applies_to_rolling_integration_and_next_base(tmp_path):
    s = make_two_subtask_state(tmp_path)
    runner = MultiFileRunner()
    c = make_ctx(tmp_path, runner, AcceptReviewer())
    res = execute_tool_with_policy(
        s,
        "ops_run_next_subtask_attempt",
        {"subtask_id": "parser", "reason": "first"},
        "p",
        c,
    )
    assert not res.is_error, res.content
    ...
    assert (
        "# parser accepted"
        in (Path(s.decomposition_integration_worktree) / "src/parser.py").read_text()
    )
    res = execute_tool_with_policy(
        s,
        "ops_run_next_subtask_attempt",
        {"subtask_id": "checkout", "reason": "downstream"},
        "c",
        c,
    )
    assert not res.is_error, res.content
    assert (
        "# parser accepted"
        in (runner.calls[-1]["repo_path"] / "src/parser.py").read_text()
    )
    assert (
        "This subtask is running on top of previously accepted subtask patches."
        in runner.calls[-1]["task"]
    )
...
def test_accepted_patch_application_is_not_duplicated(tmp_path):
    ...
    row = _apply_accepted_patch_to_integration(
        s, s.subtasks[0], s.subtasks[0].attempts[0], c
    )
    assert row == first
    assert (
        Path(s.decomposition_integration_worktree) / "src/parser.py"
    ).read_text().count("# parser accepted") == 1
...
def test_fallback_prompt_is_budgeted(tmp_path):
    ...
    s.decomposed_execution_status = "blocked"
    s.decomposed_execution_blockers = ["x" * 5000]
    s.attempt_observations = [
        AttemptObservation(
            attempt_id=f"a{i}",
            scope="subtask",
