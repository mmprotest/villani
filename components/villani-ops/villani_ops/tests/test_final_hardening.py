...
def _valid_human(**updates):
    h = {
        "requested": True,
        "request_reasons": ["reviewer_recommended_ask_human"],
        "prompted": True,
        "skipped_reason": None,
        "decision": "accept",
        "valid_override": True,
        "shown_evidence": {
            ...
            "reviewer_summary": "bad but approved",
            "acceptance_blockers": ["runner exit code is 1"],
        },
    }
    h.update(updates)
    return h
...
def test_human_override_strict_acceptance_matrix():
    ...
    for decision in ["reject", "retry", "escalate", "fail", "skipped"]:
        ok, blockers = human_override_blockers(
            _attempt(_valid_human(decision=decision))
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda h: {k: v for k, v in h.items() if k != "valid_override"},
            "valid_override",
        ),
...
class MalformedAcceptProvider:
    ...
    def request_approval(self, context):
        h = _valid_human()
        h.update(self.approval)
        return HumanApprovalResult.model_validate(h)


def test_malformed_human_approval_does_not_override_and_report_has_blockers(
    ...
    ops, _ = make_ops(
        ...
        provider=MalformedAcceptProvider({"valid_override": False}),
        exit_code=1,
    )
    res = ops.run(
        repo,
        task=__import__("villani_ops.core.task", fromlist=["Task"]).Task(
            repo_path=str(repo), objective="edit"
        ),
    ...
    step = json.loads(
        (
            Path(res.run_dir) / "attempts" / "attempt_001" / "controller_decision.json"
        ).read_text()
    )
...
def test_valid_human_override_accepts_nonzero_and_uncertain(tmp_path, monkeypatch):
    ...
    ops, _ = make_ops(
        ...
        provider=TestHumanApprovalProvider("accept"),
        exit_code=1,
    )
    res = ops.run(
        repo,
...
def test_retry_and_escalation_attempt_ids_are_new_on_next_attempt(
    tmp_path, monkeypatch
):
    repo = tmp_path / "repo"
    git_repo(repo)
    strat = ExecutionStrategy(
        profile="balanced",
        attempts=[
            StrategyAttempt(backend="code", max_attempts=2),
            StrategyAttempt(backend="strong"),
        ],
    )
    reviews = [
        ReviewResult(decision="fail", recommended_action="retry_same_backend"),
        ReviewResult(decision="fail", recommended_action="escalate"),
        ReviewResult(passed=True, decision="pass", recommended_action="accept"),
    ]
    ...
    steps = [
        json.loads(x)
        for x in (Path(res.run_dir) / "controller_steps.jsonl").read_text().splitlines()
    ]
    pairs = [
        (s["attempt_id"], s["state_before"], s["action"], s["state_after"])
        for s in steps
    ]
    ...
    assert [a["attempt_id"] for a in res.attempts] == [
        "attempt_001",
        "attempt_002",
        "attempt_003",
    ]
...
def _setup_compare(tmp_path, monkeypatch, reviews):
        ...
        assert (
            runner.invoke(
                ...
                [
                    ...
                    str(cost),
                    "--roles",
                    "coding,classification,policy,review",
                    "--workspace",
                    str(ws),
                ],
            ).exit_code
    ...
    def classify(self, task, backends, out_path=None):
        ...
        return TaskClassification(
            difficulty=cats[idx][0], category=cats[idx][1], risk="low"
        ), LLMCallResult(
            parsed_json={},
            raw_text="{}",
            backend_name="cheap",
    ...
    monkeypatch.setattr(
        "villani_ops.classification.classifier.TaskClassifier.classify", classify
    )

    def generate(self, cls, backends, profile, out_path=None):
        return ExecutionStrategy(
            profile=profile,
            strategy_summary=f"{profile} strategy",
            attempts=[StrategyAttempt(backend=profile, max_attempts=1)],
        ), LLMCallResult(
            parsed_json={},
            raw_text="{}",
    ...
    monkeypatch.setattr(
        "villani_ops.policy_engine.engine.PolicyEngine.generate", generate
    )
    q = list(reviews)

    def review(
        self, task, classification, coding_backend, attempt, backends, out_path=None
    ):
        r = (
            q.pop(0)
            if q
            else ReviewResult(
                passed=True, decision="pass", recommended_action="accept", score=0.8
            )
        )
        if out_path:
            Path(out_path).write_text(r.model_dump_json())
        return r, LLMCallResult(
            parsed_json={},
            raw_text="{}",
            backend_name=coding_backend.name,
            model=coding_backend.model,
            input_tokens=6,
            output_tokens=3,
            estimated_cost=0.03,
        )
    ...
    tasks.write_text(
        "\n".join(
            [
                json.dumps(
                    {"id": "t1", "objective": "edit 1", "success_criteria": "ok"}
                ),
                json.dumps(
                    {"id": "t2", "objective": "edit 2", "success_criteria": "ok"}
                ),
                json.dumps(
                    {"id": "t3", "objective": "edit 3", "success_criteria": "ok"}
                ),
            ]
        )
        + "\n"
...
def test_compare_basic_repeat_resume_breakdowns_and_safety(tmp_path, monkeypatch):
    reviews = [
        ReviewResult(
            passed=True, decision="pass", recommended_action="accept", score=0.9
        )
    ] * 12
    repo, ws, tasks, count = _setup_compare(tmp_path, monkeypatch, reviews)
    before = (repo / "hello.txt").read_text()
    ...
    assert (
        len(data) == 12
        and {r["trial_index"] for r in data} == {0, 1}
        and {r["task_id"] for r in data} == {"t1", "t2"}
    )
    required = {
        "comparison_id",
        "policy",
    ...
    assert (
        ...
        and all(Path(r["report_path"]).exists() for r in data)
    )
    text = out.read_text()
    assert (
        "## Thesis Signal" in text
        and "Failure reason breakdown" in text
        and "Category/difficulty breakdown" in text
        and "significance" not in text.lower()
    )
    assert (repo / "hello.txt").read_text() == before and subprocess.run(
        ["git", "status", "--porcelain"],
        cwd=repo,
        text=True,
        capture_output=True,
    ...
    assert (
        res2.exit_code == 0
        and len(json.loads(out.with_suffix(".json").read_text())) == 12
        and int(count.read_text()) == calls
    )
...
def test_compare_zero_accepts_and_failure_breakdown(tmp_path, monkeypatch):
    repo, ws, tasks, count = _setup_compare(
        tmp_path,
        monkeypatch,
        [ReviewResult(decision="fail", recommended_action="fail", score=0.1)] * 6,
    )
    out = tmp_path / "comparison.md"
    res = runner.invoke(
        app,
    ...
    assert (
        "N/A" in text
        and "Failure reason breakdown" in text
        and "Attempt is not acceptable" in text
    )


def test_pr_prepare_success_push_and_gh_failures_branch_dirty_patch(
    tmp_path, monkeypatch
):
    ...
    monkeypatch.setattr(
        "shutil.which", lambda n: None if n == "gh" else "/usr/bin/" + n
    )
    res = runner.invoke(
        app,
        [
    ...
    init_git(tmp_path / "repo2")
    rd2 = make_run(ws, tmp_path / "repo2", good)
    (tmp_path / "repo2" / "hello.txt").write_text("dirty\n")
    assert (
        runner.invoke(
            app,
            [
    ...
    art = json.loads((rd3 / "pr.json").read_text())
    assert (
        res.exit_code == 0
        and art["url"] == "https://example.test/pr/1"
        and "--title T --body B" in calls.read_text()
    )

    repo4 = tmp_path / "repo4"
    init_git(repo4)
    rd4 = make_run(ws, repo4, good)
    calls.unlink(missing_ok=True)
    monkeypatch.setattr("shutil.which", lambda n: str(gh) if n == "gh" else None)
    res = runner.invoke(
        app, ["pr", "r1", "--title", "T", "--body", "B", "--workspace", str(ws)]
    )
    art = json.loads((rd4 / "pr.json").read_text())
    assert (
        res.exit_code != 0
        and art["commit_sha"]
        and art["manual_commands"]
        and art["recovery_instructions"]
        and not calls.exists()
    )

    repo5 = tmp_path / "repo5"
    init_git(repo5)
    ...
    assert (
        ...
        in json.loads((rd5 / "pr.json").read_text())["stderr"]
    )
