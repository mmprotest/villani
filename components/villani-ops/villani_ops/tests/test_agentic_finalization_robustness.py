...
def _eligible_state(tmp_path):
    ...
    s = OpsRunState(
        ...
        plan={"strategy": "parallel_candidates"},
    )
    s.candidates.append(
...
def test_repeated_select_and_finalize_are_idempotent(tmp_path):
    s = _eligible_state(tmp_path)
    c = ctx(tmp_path)
    select = execute_tool_with_policy(
        s,
        "ops_select_winner",
        {
            "decision": "select",
    ...
    again = execute_tool_with_policy(
        s,
        "ops_finalize_run",
        {
            "decision": "accepted",
...
def test_recorder_suppresses_duplicate_terminal_events(tmp_path):
    ...
    r.record(
        "selection_completed",
        payload={"decision": "select", "selected_attempt_id": "a"},
    )
    r.record(
        "selection_completed",
        payload={"decision": "select", "selected_attempt_id": "a"},
    ...
    r.record(
        "run_finalized", payload={"decision": "accepted", "selected_attempt_id": "a"}
    )
    r.record(
        "run_finalized", payload={"decision": "accepted", "selected_attempt_id": "a"}
    )
...
def test_durable_write_preserves_prior_target_after_replace_failure(
