...
def _eligible_attempt(tmp_path, s, aid="candidate_001"):
    wt = tmp_path / aid / "worktree"
    wt.mkdir(parents=True)
    patch = tmp_path / aid / "diff.patch"
    ...
    s.investigation = {"summary": "s", "confidence": 1}
...
def test_candidate_validation_defaults_to_worktree_and_attaches(tmp_path):
    s = state(tmp_path)
    c = ctx(tmp_path)
    a, wt = _eligible_attempt(tmp_path, s)
    res = execute_tool_with_policy(
        ...
        "ops_run_validation",
        {
            ...
            "commands": [
                {
                    "cmd": "python -c \"import pathlib; pathlib.Path('marker.txt').write_text('ok')\""
                }
            ],
        },
        "v",
...
def test_candidate_validation_rejects_cwd_escape_and_embedded_cd(tmp_path):
    s = state(tmp_path)
    c = ctx(tmp_path)
    a, _ = _eligible_attempt(tmp_path, s)
    out = execute_tool_with_policy(
        s,
        "ops_run_validation",
        {
            "target": "candidate",
            "target_id": a.attempt_id,
            "commands": [{"cmd": 'python -c "print(1)"', "cwd": str(tmp_path)}],
        },
        "v",
        c,
    )
    ...
    out = execute_tool_with_policy(
        ...
        "ops_run_validation",
        {
            "target": "candidate",
            "target_id": a.attempt_id,
            "commands": [{"cmd": 'cd / && python -c "print(1)"'}],
        },
        "v2",
        c,
    )
...
def test_repo_validation_uses_repo_path_not_candidate_evidence(tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        "ops_run_validation",
        {
            ...
            "commands": [
                {
                    "cmd": "python -c \"import pathlib; pathlib.Path('repo_marker.txt').write_text('ok')\""
                }
            ],
        },
        "v",
...
def test_recovery_recommends_selection_then_finalization(tmp_path):
    ...
    a.validation = {
        ...
        "status": "passed",
        "commands": [
            {"cmd": 'python -c "print(1)"', "passed": True, "status": "passed"}
        ],
    }
...
def test_candidate_ids_sequential_with_parallel_batches(tmp_path):
    ...
    execute_tool_with_policy(
        s, "ops_submit_investigation", {"summary": "s", "confidence": 1}, "i", c
    )
    execute_tool_with_policy(
        s,
        "ops_submit_plan",
    ...
    execute_tool_with_policy(
        ...
        {"path": "parallel_candidates", "reason": "r"},
        "x",
        c,
    )
    res = execute_tool_with_policy(
        s, "ops_launch_candidates", {"attempts": 3, "reason": "r"}, "l", c
    )
    assert not res.is_error
    assert [a.attempt_id for a in s.candidates] == [
        "candidate_001",
        "candidate_002",
        "candidate_003",
    ]
...
def test_windows_mode_rejects_unix_head(monkeypatch, tmp_path):
    ...
    res = execute_tool_with_policy(
        ...
        "ops_run_validation",
        {
            "target": "candidate",
            "target_id": a.attempt_id,
            "commands": [{"cmd": "python -m pytest 2>&1 | head -300"}],
        },
        "v",
        c,
    )
