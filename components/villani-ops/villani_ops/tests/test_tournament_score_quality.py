...
def test_tournament_models_normalize_scores_on_construction():
    ...
    cmp = PairwiseCandidateComparison(
        candidate_a="a", candidate_b="b", winner="tie", confidence=7, rationale="r"
    )
...
def test_unproven_critical_behaviour_caps_review_scores():
    ...
    data["evidence_gaps"] = [
        "critical required behaviour is unproven and not guaranteed"
    ]
...
def test_ranking_treats_8_5_and_0_85_equivalently_after_normalization(tmp_path):
    ...
    s.candidate_risk_reviews = {
        "candidate_001": _review("candidate_001", 8.5, 3.5),
        "candidate_002": _review("candidate_002", 0.85, 0.35),
    }
    ...
    assert (
        rank.ranked_candidates[0].correctness_score
        == rank.ranked_candidates[1].correctness_score
        == 0.85
    )
    assert (
        rank.ranked_candidates[0].hidden_test_risk_score
        == rank.ranked_candidates[1].hidden_test_risk_score
        == 0.35
    )
...
def test_candidate_review_prompt_contains_adversarial_quality_guidance(tmp_path):
    ...
    assert (
        "Do not reward a candidate merely because it contains words or structures"
        in prompt
    )
...
def test_pairwise_prompt_is_compact_and_includes_signature_not_full_state(tmp_path):
    ...
    sig = CandidateImplementationSignature(
        candidate_id="a",
        changed_files=["x"],
