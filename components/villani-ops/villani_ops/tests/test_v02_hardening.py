...
def make_run(ws: Path, repo: Path, patch_text: str, accepted=True):
    ...
    (rd / "task.json").write_text(json.dumps({"repo_path": str(repo)}))
    patch = rd / "diff.patch"
...
def test_backend_direct_key_stored_masked_and_conflict_fails(tmp_path, monkeypatch):
    ...
    res = runner.invoke(
        ...
        [
            ...
            "dummy",
            "--roles",
            "coding,classification,review,policy",
        ],
    )
    assert res.exit_code == 0
    raw = (tmp_path / ".villani-ops" / "backends.yaml").read_text()
    assert "api_key: dummy" in raw
    assert "dummy" not in runner.invoke(app, ["backend", "list"]).output
    assert (
        "***REDACTED***" in runner.invoke(app, ["backend", "show", "local-qwen"]).output
    )
    bad = runner.invoke(
        app,
        [
...
class FakeClient:
    ...
    def complete_json(self, *a, **k):
        return LLMCallResult(
            parsed_json={"profile": "balanced", "attempts": self.attempts},
            raw_text="{}",
            backend_name="p",
            model="m",
        )
...
def test_policy_guardrails_balanced_and_invented_repaired():
    backs = {
        "cheap": _b("cheap", cap=10, cost=0),
        "strong": _b("strong", cap=90, cost=5),
        "policy": _b("policy", roles=("policy",)),
    }
    cls = TaskClassification(difficulty="easy", category="bugfix", risk="low")
    strat, _ = PolicyEngine(
        FakeClient([{"backend": "invented"}, {"backend": "strong"}])
    ).generate(cls, backs, "balanced")
...
def test_apply_guards_check_and_success_and_commit(tmp_path, monkeypatch):
    ...
    art = json.loads((rd / "apply.json").read_text())
    assert art["exit_code"] == 0 and art["commit_sha"]
...
def test_branch_existing_refuses_and_force_branch(tmp_path, monkeypatch):
    ...
    assert (
        runner.invoke(
            app, ["branch", "r1", "--name", "feature", "--workspace", str(ws)]
        ).exit_code
        != 0
    )
    res = runner.invoke(
        app,
        ["branch", "r1", "--name", "feature", "--force-branch", "--workspace", str(ws)],
    )
    assert (
        res.exit_code == 0
        and subprocess.run(
...
def test_pr_missing_gh_saves_manual_artifact(tmp_path, monkeypatch):
    ...
    art = json.loads((rd / "pr.json").read_text())
    assert art["gh_available"] is False and art["manual_commands"]
