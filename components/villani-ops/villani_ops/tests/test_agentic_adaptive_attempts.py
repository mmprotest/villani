...
def make_state(tmp_path):
    ...
    return OpsRunState(
        ...
        plan={"summary": "single", "strategy": "single_task"},
        execution_path="single_task",
    )
    ...
    def run_task(
        ...
        *,
        ...
        return SimpleNamespace(
            ...
            model_failures=0,
            total_tool_calls=3,
            tool_calls_by_name={"read": 1, "write": 1, "cmd": 1},
            total_file_reads=1,
            total_file_writes=1,
            commands_executed=1,
            commands_failed=1,
            first_substantive_file_read_tool_index=1,
            first_substantive_file_read_seconds=0.1,
            first_file_mutation_tool_index=2,
            first_file_mutation_seconds=0.2,
            first_command_tool_index=3,
            first_command_seconds=0.3,
            token_accounting_status="available",
            token_accounting_warnings=[],
            telemetry={"x": 1},
            debug_artifact_dir=str(artifacts_dir),
            resolved_trace_dir=str(artifacts_dir),
            input_tokens=10,
            output_tokens=5,
            total_tokens=15,
            total_cost=0.01,
...
class FailReviewer:
    ...
    def review(self, *, state, attempt, scope):
        return {
            ...
            "score": 0.2,
            "summary": "Patch handles receipt rendering but not rollback path.",
            "evidence": ["rollback still broken"],
            "issues": ["missing rollback"],
            "blockers": ["rollback_not_fixed"],
        }
...
def make_ctx(tmp_path, runner=None):
    ...
    return OpsToolContext(
        ...
        transcript=[],
        runner_adapter=runner or TelemetryRunner(),
        reviewer=FailReviewer(),
        backend=b1,
        backend_name="b1",
        coding_backend=b1,
        coding_backend_name="b1",
...
def test_next_candidate_attempt_runs_one_and_observes_with_telemetry(tmp_path):
    ...
    c = make_ctx(tmp_path, runner)
    res = execute_tool_with_policy(
        s, "ops_run_next_candidate_attempt", {"reason": "first try"}, "a1", c
    )
...
def test_second_attempt_prompt_contains_curated_learning_and_do_not_repeat(tmp_path):
    ...
    res = execute_tool_with_policy(
        s, "ops_run_next_candidate_attempt", {"reason": "focused retry"}, "a2", c
    )
    assert not res.is_error, res.content
...
def test_backend_name_selects_actual_backend_and_unknown_errors(tmp_path):
    ...
    bad = execute_tool_with_policy(
        ...
        {"backend_name": "missing", "reason": "try missing"},
        "bad",
        c,
    )
    assert bad.is_error
...
def test_next_candidate_attempt_rejects_after_budget_exhausted(tmp_path):
    ...
    runner = TelemetryRunner()
    c = make_ctx(tmp_path, runner)
    first = execute_tool_with_policy(
        s, "ops_run_next_candidate_attempt", {"reason": "only try"}, "a1", c
    )
    assert not first.is_error, first.content
    assert "ops_run_next_candidate_attempt" not in s.allowed_next_actions()
    second = execute_tool_with_policy(
        s, "ops_run_next_candidate_attempt", {"reason": "too many"}, "a2", c
    )
    assert second.is_error
    assert "budget exhausted" in second.content


def test_observation_refreshes_after_later_validation_and_review_without_duplicates(
    tmp_path,
):
    ...
    s.candidates[0].review = None
    s.candidates[0].review_status = "not_run"
    s.candidates[0].review_retry_count = 0
    h_observe_completed_attempt(
        s,
        OpsObserveCompletedAttemptInput(attempt_id=aid, reason="clear review snapshot"),
        c,
    )
    original = s.attempt_observations[0]
    assert len(s.attempt_observations) == 1
    h_observe_completed_attempt(
        s, OpsObserveCompletedAttemptInput(attempt_id=aid, reason="again"), c
    )
    assert len(s.attempt_observations) == 1
    ...
    h_validation(
        ...
        OpsRunValidationInput(
            ...
            commands=[
                ValidationCommand(
                    cmd='python -c "import sys; sys.exit(0)"', purpose="later pass"
                )
            ],
        ),
        c,
...
def _manual_attempt(aid="candidate_001", backend="b1"):
    ...
    Path("/tmp/diff.patch").write_text(
        "diff --git a/app.py b/app.py\n--- a/app.py\n+++ b/app.py\n@@ -1 +1 @@\n-old\n+new\n"
    )
    wt = Path("/tmp") / f"wt_{aid}"
    ...
    return CandidateAttemptState(
        ...
        patch_path="/tmp/diff.patch",
        worktree_path=str(wt),
        runner_telemetry={"total_file_reads": 1, "total_file_writes": 1},
...
def test_backend_and_runner_assessments_are_idempotent_on_observation_refresh(tmp_path):
    ...
    s.candidates = [a]
    h_observe_completed_attempt(
        s, OpsObserveCompletedAttemptInput(attempt_id=a.attempt_id, reason="first"), c
    )
    assert s.backend_assessments["b1"]["attempts"] == 1
    assert s.runner_assessment["attempts"] == 1
    h_observe_completed_attempt(
        s, OpsObserveCompletedAttemptInput(attempt_id=a.attempt_id, reason="refresh"), c
    )
...
def test_validation_and_review_refresh_update_counts_without_incrementing_attempts(
    tmp_path,
):
    from villani_ops.agentic.tools import (
        h_observe_completed_attempt,
    ...
    h_validation(
        ...
        OpsRunValidationInput(
            ...
            commands=[
                ValidationCommand(
                    cmd='python -c "import sys; sys.exit(0)"', purpose="pass"
                )
            ],
        ),
        c,
    ...
    c.reviewer = type(
        ...
        (),
        {
            "review": lambda self, *, state, attempt, scope: {
                "decision": "pass",
    ...
    h_review_attempt(
        s, OpsReviewAttemptInput(attempt_id=a.attempt_id, scope="candidate"), c
    )
    assert s.backend_assessments["b1"]["attempts"] == 1
    assert s.runner_assessment["attempts"] == 1
...
def test_accepted_refresh_and_multiple_backends_count_unique_attempts(tmp_path):
    ...
    s = make_state(tmp_path)
    c = make_ctx(tmp_path)
    a1 = _manual_attempt("candidate_001", "b1")
    a1.validation = {"passed": True, "status": "passed", "commands": []}
    a1.validation_status = "passed"
    a1.validation_results = [a1.validation]
    a1.review = {"decision": "pass", "recommended_action": "accept", "blockers": []}
    a1.review_status = "passed"
    a1.acceptance_eligible = True
    ...
    for a in s.candidates:
        h_observe_completed_attempt(
            s,
            OpsObserveCompletedAttemptInput(attempt_id=a.attempt_id, reason="observe"),
            c,
        )
    h_observe_completed_attempt(
        s,
        OpsObserveCompletedAttemptInput(
            attempt_id=a1.attempt_id, reason="accepted refresh"
        ),
        c,
    )
