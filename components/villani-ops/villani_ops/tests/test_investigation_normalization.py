...
def test_investigation_aliases_and_confidence_preserve_extras():
    raw = {
        ...
        "implementation_steps": ["fix pricing"],
        "test_validation": "pytest",
        "identified_bugs": [
            {"file": "e.py", "issue": "bad tax", "fix": "apply coupon first"}
        ],
    }
    ...
    assert (
        "fix pricing" in inv.implementation_plan
        and "apply coupon first" in inv.implementation_plan
    )
...
def test_useful_raw_signals_raise_confidence_without_summary():
    norm, _ = normalize_investigation_payload(
        {"files_to_modify": "a.py", "implementation_steps": "fix it"}
    )
    inv = InvestigationResult.model_validate(norm)
    assert inv.confidence >= 0.65
...
def test_malformed_summary_with_useful_files_synthesizes_safe_summary():
    norm, _ = normalize_investigation_payload(
        {"summary": {"bad": "shape"}, "files_to_modify": ["a.py"]}
    )
    assert (
        norm["summary"]
