...
def test_strategy_name_alias_validates():
    payload = normalize_execution_strategy_payload(
        {"strategy_name": "balanced", "planned_attempts": [_attempt()]}, "balanced"
    )
...
def test_cli_profile_wins_over_model_profile():
    payload = normalize_execution_strategy_payload(
        {"profile": "quality", "planned_attempts": [_attempt()]}, "balanced"
    )
    strategy = ExecutionStrategy.model_validate(payload)
    assert strategy.profile == "balanced"
...
def test_deterministic_fallback_on_validation_failure(tmp_path):
    ...
    strategy, call = PolicyEngine(_InvalidClient()).generate(
...
def test_policy_error_artifact_written(tmp_path):
    ...
    PolicyEngine(_InvalidClient()).generate(
        TaskClassification(), backs, "balanced", tmp_path / "execution_strategy.json"
    )
