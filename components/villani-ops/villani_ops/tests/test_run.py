...
def setup_workspace(tmp_path):
    ...
    assert (
        runner.invoke(
            app, ["policy", "create-default", "--name", "p"], catch_exceptions=False
        ).exit_code
        == 0
    )
...
def test_run_unconfigured_runner_fails_honestly(tmp_path, monkeypatch):
    ...
    assert (
        "Shell runner command is not configured" in (run_dir / "report.md").read_text()
    )
...
def test_run_configured_shell_edits_file_valid(tmp_path, monkeypatch):
    ...
    assert (
        runner.invoke(
            app,
            ["runner", "set", "shell", "--command", command],
            catch_exceptions=False,
        ).exit_code
        == 0
    )
    ...
    run_dir = next((tmp_path / ".villani-ops" / "runs").iterdir())
    attempt = run_dir / "attempts" / "attempt_001"
    assert (attempt / "diff.patch").exists() and "+changed" in (
        attempt / "diff.patch"
    ).read_text()
    assert (
        (attempt / "attempt.json").exists()
        and (attempt / "validation.json").exists()
        and (run_dir / "decision.json").exists()
        and (run_dir / "report.md").exists()
    )
...
def test_cli_run_emits_progress_markers(tmp_path, monkeypatch):
    ...
    class FakeOps:
        ...
        def run(self, **kwargs):
            ...
            return type(
                "R", (), {"decision": FakeDecision(), "report_path": "/tmp/report.md"}
            )()

    monkeypatch.setattr(main, "CostPolicyVillaniOps", FakeOps)
    res = runner.invoke(
        app,
        ["cost-run", "--repo", str(repo), "--task", "edit", "--policy", "balanced"],
        catch_exceptions=False,
    )
    assert res.exit_code == 0
    for text in [
        "Starting Villani Ops run",
        "Classifying task",
        "Generating policy",
        "Running attempt_001",
        "Reviewing attempt_001",
        "Finalizing decision",
        "Report:",
    ]:
