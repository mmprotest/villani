...
def setup_plan(s, c):
    execute_tool_with_policy(
        s, "ops_submit_investigation", {"summary": "i", "confidence": 1}, "i", c
    )
    execute_tool_with_policy(
        s,
        "ops_submit_plan",
...
def decomp(s, c, items=2):
    ...
    return execute_tool_with_policy(
        s,
...
def test_semantic_decomposition_acceptance_computed(tmp_path):
    ...
    res = execute_tool_with_policy(
        s, "ops_validate_decomposition", {"semantic": True}, "v", c
    )
...
def test_no_advisory_only_rejected_falls_back_explicitly(tmp_path):
    ...
    c = ctx(tmp_path)
    setup_plan(s, c)
    s.decomposition = {"should_use_decomposition": False}
    s.subtasks = []
    execute_tool_with_policy(s, "ops_validate_decomposition", {}, "v", c)
    res = execute_tool_with_policy(
        s,
        "ops_select_execution_path",
        {"path": "parallel_candidates", "reason": "rejected"},
        "x",
        c,
    )
    assert (
        not res.is_error
        and s.decomposition_fallback_used
        and not s.decomposition_executed
    )
...
def test_accepted_decomposition_runs_one_subtask_and_blocks_bulk_launchers(tmp_path):
    ...
    assert not execute_tool_with_policy(
        s,
        "ops_select_execution_path",
        {"path": "decomposed_subtasks", "reason": "ok"},
        "x",
        c,
    ).is_error
    assert execute_tool_with_policy(
        s, "ops_launch_candidates", {"attempts": 3, "reason": "bad"}, "lc", c
    ).is_error
    ...
    bulk = execute_tool_with_policy(
        s,
        "ops_launch_subtasks",
        {"subtask_ids": ["s0", "s1"], "attempts_per_subtask": 3, "reason": "go"},
        "ls",
        c,
    )
    assert bulk.is_error and "legacy/internal compatibility tool" in str(bulk.content)
    res = execute_tool_with_policy(
        s, "ops_run_next_subtask_attempt", {"reason": "go"}, "one", c
    )
...
def test_rejected_decomposition_falls_back_then_launches_candidates(tmp_path):
    ...
    assert not execute_tool_with_policy(
        s,
        "ops_select_execution_path",
        {"path": "parallel_candidates", "reason": "fallback"},
        "p",
        c,
    ).is_error
    assert not execute_tool_with_policy(
        s, "ops_launch_candidates", {"attempts": 3, "reason": "go"}, "l", c
    ).is_error
...
def test_recovery_recommends_select_path_after_accepted_decomposition(tmp_path):
    ...
    s.plan = {"summary": "p", "should_decompose": True}
    ...
    s.subtasks = [
        SubtaskState(subtask_id="s0", title="s0", objective="o"),
        SubtaskState(subtask_id="s1", title="s1", objective="o"),
    ]
...
def test_recovery_recommends_ready_subtask_launch_after_path_selected(tmp_path):
    ...
    s.subtasks = [
        SubtaskState(subtask_id="s0", title="s0", objective="o"),
        SubtaskState(subtask_id="s1", title="s1", objective="o", dependencies=["s0"]),
    ]
    rec = recommend_next_agentic_action(s)
