...
def test_collect_relevant_file_snippets_includes_test_and_source(tmp_path):
    ...
    snippets = collect_relevant_file_snippets(
        repo, "Fix failing tests in tests/test_foo.py", tree
    )
...
def test_collect_relevant_file_snippets_skips_generated_cache(tmp_path):
    ...
    snippets = collect_relevant_file_snippets(
        repo, "Fix failing tests for foo.pyc", tree
    )
    assert snippets == []
...
def test_classifier_context_includes_relevant_file_snippets(tmp_path):
    ...
    cls, _ = TaskClassifier(Client()).classify(
        Task(repo_path=str(repo), objective="Fix failing tests in tests/test_foo.py"),
        {"cls": backend},
    )
