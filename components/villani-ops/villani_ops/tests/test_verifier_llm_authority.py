...
def _verdict(result, confidence=0.8, action=None, reason="raw LLM judgement"):
    return {
        "result": result,
...
def test_raw_llm_success_remains_final_success_with_candidate_active_failures():
    out = calibrate(
        _det(active=[{"id": "ev-001", "kind": "command", "text": "candidate failure"}]),
        _verdict(1),
    )
...
def test_raw_llm_failure_remains_final_failure_with_candidate_validation_signals():
    out = calibrate(
        _det(
            validations=[
                {"id": "ev-002", "validationStrength": "strong", "text": "PASS"}
            ]
        ),
        _verdict(0),
    )
...
def test_audit_adjudication_not_in_result_path_even_with_cfg():
    out = calibrate(
        _det(active=[{"text": "candidate failure"}]), _verdict(1), cfg={"unused": True}
    )
    assert out["result"] == 1
...
def test_raw_llm_failure_not_flipped_by_generic_pass_output():
    det = _det(
        validations=[
            {"text": "generic PASS/self-validation", "validationStrength": "weak"}
        ]
    )
...
def test_raw_llm_failure_not_flipped_by_setup_or_install_evidence():
    det = _det(
        validations=[
            {"text": "setup/install succeeded", "validationStrength": "medium"}
        ]
    )
...
def test_raw_llm_success_not_flipped_by_missing_deterministic_deliverable_label():
    ...
    out = calibrate(
        det, _verdict(1, reason="tool evidence showed required file content")
    )
    assert out["result"] == 1
    assert any(
        "deliverable evidence labels are missing" in f.lower() for f in out["riskFlags"]
    )
...
def test_calibration_trace_records_non_mutating_policy(tmp_path):
    ...
    out = calibrate(
        _det(active=[{"text": "candidate failure"}]), _verdict(1), trace=Trace()
    )
...
def test_candidate_evidence_packet_uses_neutral_candidate_fields(tmp_path):
    ...
    (d / "commands.jsonl").write_text(
        json.dumps({"command": "echo PASS", "exit_code": 0, "stdout": "PASS"}) + "\n"
    )
    ...
    assert set(pkt["candidateEvidence"]) >= {
        "candidateFailures",
...
def test_critical_coverage_accept_remains_accept():
    v = _verdict(1, confidence=0.82, action="accept")
    v.update(
        {
            "criticalRequirement": "abnormal path works",
            "directEvidenceForCriticalRequirement": "targeted test exercised abnormal path",
            "criticalRequirementCovered": True,
            "criticalRequirementEvidenceRefs": ["ev-pass"],
            "criticalRequirementEvidenceMatch": {
                "ev-pass": {
                    "matchesCriticalRequirement": True,
                    "requirementCondition": "abnormal path works",
                    "evidenceCondition": "abnormal path works",
                    "whySameCondition": "targeted test exercised abnormal path",
                    "limitations": [],
                }
            },
        }
    ...
    out = calibrate(
        _det(validations=[{"id": "ev-pass", "validationStrength": "strong"}]), v
    )
    assert out["result"] == 1
...
def test_critical_coverage_false_downgrades_accept_and_caps_confidence():
    v = _verdict(1, confidence=0.88, action="accept")
    v.update(
        {
            "criticalRequirement": "cleanup on cancellation",
            "directEvidenceForCriticalRequirement": "normal path test only",
...
def test_failure_result_not_changed_by_critical_coverage_gate():
    v = _verdict(0, confidence=0.77, action="reject")
...
def test_critical_coverage_warning_preserves_existing_warnings():
    v = _verdict(1, confidence=0.8, action="accept")
...
def _covered_verdict(refs=None, confidence=0.82, action="accept"):
    ...
    v.update(
        {
            ...
            "criticalRequirementEvidenceMatch": {
                r: {
                    "matchesCriticalRequirement": True,
                    "requirementCondition": "critical behavior",
                    "evidenceCondition": "critical behavior",
                    "whySameCondition": "same condition",
                    "limitations": [],
                }
                for r in (refs or [])
            },
...
def test_critical_coverage_source_inspection_ref_downgrades_accept():
    det = _det(
        validations=[
            {"id": "src", "kind": "source_inspection", "validationStrength": "strong"}
        ]
    )
...
def test_critical_coverage_import_and_file_existence_refs_downgrade_unless_artifact_only():
    det = _det(
        validations=[
            {"id": "imp", "kind": "import_check", "validationStrength": "strong"}
        ],
    ...
    artifact_det = _det(
        deliverables=[{"id": "artifact", "kind": "file_existence_check"}]
    )
...
def test_critical_coverage_normal_path_for_abnormal_requirement_downgrades_accept():
    det = _det(
        validations=[
            {"id": "normal", "validationStrength": "strong", "normalPathOnly": True}
        ]
    )
...
def test_critical_coverage_behavioral_runtime_evidence_accepts():
    det = _det(
        validations=[
            {"id": "runtime", "kind": "runtime_trace", "validationStrength": "strong"}
        ]
    )
...
def test_proven_coverage_not_downgraded_by_stale_or_diagnostic_disagreement():
    ...
    det = _det(
        active=[{"id": "old", "stale": True, "text": "earlier failure"}],
        validations=[
            {
                "id": "ev-pass",
    ...
    det["evidenceRegistry"] = {
        ...
        "old": {"id": "old", "stale": True},
    }
    out = calibrate(det, v)
    assert out["recommendedAction"] == "accept"
    det = _det(
        active=[{"id": "diag", "kind": "diagnostic", "diagnosticOnly": True}],
        validations=[
            {
                "id": "ev-pass",
                "validationStrength": "strong",
    ...
    det["evidenceRegistry"] = {
        ...
        "diag": {"id": "diag", "kind": "diagnostic", "diagnosticOnly": True},
    }
    out = calibrate(det, v)
    assert out["recommendedAction"] == "accept"
...
def test_proven_coverage_downgraded_by_final_decisive_failure():
    ...
    det = _det(
        active=[
            {"id": "fail", "validationStrength": "failure", "finalStateDecisive": True}
        ],
        validations=[
            {
                "id": "ev-pass",
    ...
    det["evidenceRegistry"] = {
        ...
        "fail": {
            "id": "fail",
            "validationStrength": "failure",
            "finalStateDecisive": True,
        },
    }
    out = calibrate(det, v)
    assert out["recommendedAction"] == "inspect_manually"
