...
def backends():
    return {
        "small": Backend(
            ...
            model="s",
            capability_score=1,
            roles=["coding", "review", "selection", "classification", "investigation"],
        ),
        "large": Backend(
            name="large",
            provider="openai-compatible",
...
def test_prior_escalation_uses_explicit_flags_only():
    ...
    assert not prior_forces_escalation(
        [
            NodeResult(
                status="succeeded",
                data={
                    "note": "this unrelated text says blocker but is not a blocker flag"
                },
            )
        ]
    )
...
def test_policy_escalation_scoped_prior_results():
    ...
    ctx = TaskContext(objective="x", confidence=0.95)
    assert (
        CheapExecutionPolicy()
        .select_backend(
            node=node,
            backends=backends(),
            task_context=ctx,
            prior_results=[NodeResult(has_review_blocker=True)],
        )
        .backend_name
        == "large"
    )
    verify = OrchestrationNode(
        id="verify",
        kind="verify",
        objective="verify",
        difficulty="easy",
    ...
    assert (
        BalancedExecutionPolicy()
        .select_backend(
            node=verify,
            backends=backends(),
            task_context=ctx,
            prior_results=[NodeResult(has_acceptance_blocker=True)],
        )
        .backend_name
        == "large"
    )
    safe = OrchestrationNode(
        id="investigate",
        kind="investigate",
    ...
    assert (
        CheapExecutionPolicy()
        .select_backend(
            node=safe, backends=backends(), task_context=ctx, prior_results=[]
        )
        .backend_name
        == "small"
    )
...
def test_cli_non_performance_backend_output(monkeypatch, tmp_path):
    class Result:
        def __init__(self, mode):
            ...
            self.decision = Decision(
                ...
                candidate_attempts_completed=0,
                orchestration_graph_path="orchestration_graph.json",
                node_backend_assignments={"investigate": "small"},
            )

    def fake_run(self, **kwargs):
        return Result(kwargs["mode"])
    ...
    def perf_run(self, **kwargs):
        ...
        r.decision.performance_backend_name = "large"
        r.decision.performance_backend_model = "l"
        return r
...
def test_report_selection_uses_evidence_and_missing_fallback(tmp_path):
    sel = SelectionResult(
        ...
        reasons=["better test evidence"],
        rejected_attempts=["attempt_001"],
        confidence=0.91,
        selector_backend="large",
    )
    decision = Decision(
        run_id="r", accepted=True, winning_attempt_id="attempt_002", plan={}
    )
    cands = [
        {
            ...
            "review_score": 0.1,
            "review_issues": ["missing test evidence"],
            "acceptance_eligible": False,
            "acceptance_blockers": ["patch is missing"],
        },
        {
            ...
            "review_recommended_action": "accept",
            "review_score": 0.92,
            "review_summary": "tests pass",
            "review_issues": [],
            "review_evidence": ["pytest passed"],
    ...
    missing = write_performance_report(
        tmp_path / "m",
        Task(repo_path=str(tmp_path), objective="fix"),
        None,
        [],
        SelectionResult(decision="reject_all", reasons=["none eligible"]),
        Decision(run_id="r2"),
        1.0,
    )
    assert (
        "Detailed winner evidence was not available in artifacts" in missing.read_text()
    )


def test_controller_steps_populated_and_performance_avoids_estimate_cost(
    ...
    class NoEstimateBackend(Backend):
        def estimate_cost(self, *a, **k):
            raise AssertionError("estimate_cost called")
    ...
    ops.storage.save_backends(
        {
            "code": NoEstimateBackend(
                name="code",
                provider="openai-compatible",
                base_url="http://x/v1",
                model="m",
    ...
    res = ops.run(
        repo,
        Task(repo_path=str(repo), objective="edit"),
    ...
    assert (
        steps
        and decision["controller_steps"]
        and decision["controller_steps_path"] == "controller_steps.jsonl"
    )
    actions = [json.loads(s)["action"] for s in steps]
    assert (
        "backend_assigned" in actions
        and "node_started" in actions
        and any(a in actions for a in ["node_succeeded", "node_failed", "node_skipped"])
    )
    ...
    for n in json.loads((rd / "orchestration_graph.json").read_text())["nodes"]:
        if n["status"] in {"succeeded", "failed", "skipped"}:
            ...
            assert (
                (nd / "node.json").exists()
                and (nd / "input.json").exists()
                and (nd / "output.json").exists()
            )
...
def test_report_component_specific_fallback_labels(tmp_path):
    ...
    inv = InvestigationResult(
        ...
        investigation_fallback_reason="bad inv",
    )
    sel = SelectionResult(
        decision="reject_all",
        summary="none",
    ...
    dec = Decision(
        ...
        plan={
            ...
            "planner_fallback_reason": "bad plan",
        },
        failure_reason="none",
    )
    report = write_performance_report(
        tmp_path, Task(repo_path=str(tmp_path), objective="fix"), inv, [], sel, dec, 1.0
    ).read_text()
