    ...
    def run_task(
        ...
        *,
        ...
        return SimpleNamespace(
            stdout="changed agentic_result.txt",
            stderr="",
            exit_code=0,
            telemetry_path=None,
        )
...
def test_prose_only_model_response_triggers_recovery_and_no_progress_fails(tmp_path):
    r = OpsRunner(
        client=FakeClient([[{"type": "text", "text": "I will plan"}], []]),
        max_recovery_attempts=1,
    ).run(req(tmp_path))
    assert r.state.status == "failed"
    assert (
        "agentic_orchestrator_no_progress"
        in json.loads((Path(r.run_dir) / "state.json").read_text())["final_decision"][
            "summary"
        ]
    )
...
class FakeSubtaskRunner:
    ...
    def run_task(
        ...
        *,
        ...
        return SimpleNamespace(
            stdout="changed subtask file", stderr="", exit_code=0, telemetry_path=None
        )
...
def test_decomposed_smoke_with_explicit_nonproduction_fakes(tmp_path):
    blocks = [
        ...
        tc(
            "ops_submit_plan",
            {
                ...
                "decomposition_reason": "independent files",
                "candidate_attempts": 1,
        ...
        tc(
            ...
            {"decomposition_id": "current", "semantic": False},
        ),
        tc(
            "ops_select_execution_path",
            {"path": "decomposed_subtasks", "reason": "validated"},
        ),
        ...
        tc(
            "ops_review_attempt",
            {"attempt_id": "integration_001", "scope": "integration"},
        ),
        tc(
            "ops_run_validation",
            {"target": "integration", "commands": [{"cmd": 'python -c "print(1)"'}]},
        ),
        tc(
            "ops_select_winner",
            {
                "selected_attempt_id": "integration_001",
                "decision": "select",
                "summary": "integrated",
                "confidence": 1.0,
            },
    ...
    repo = tmp_path / "repo"
    repo.mkdir()
    r = OpsRunner(client=FakeClient(blocks)).run(
        req(
            tmp_path,
            candidate_attempts=1,
            repo_path=str(repo),
            runner_adapter=FakeSubtaskRunner(),
        )
    )
    assert r.state.status == "completed"
...
def test_no_tool_call_after_accepted_decomposition_selects_path_deterministically(
    tmp_path,
):
    repo = tmp_path / "repo"
    repo.mkdir()
    r = OpsRunner(
        client=FakeClient(decomposed_prefix_blocks() + [[]]),
        max_turns=5,
    ...
    assert (r.state.final_decision or {}).get("blockers") != [
        "agentic_orchestrator_no_progress"
    ]
    events = (Path(r.run_dir) / "runtime_events.jsonl").read_text()
...
def test_no_tool_call_after_decomposed_path_launches_ready_subtasks(tmp_path):
    ...
    r = OpsRunner(client=FakeClient(blocks), max_turns=6, max_recovery_attempts=0).run(
        req(
            tmp_path,
            candidate_attempts=1,
            repo_path=str(repo),
    ...
    assert [s.subtask_id for s in r.state.subtasks if s.attempts] in (
        ["s0"],
        ["s0", "s1"],
    )
...
def test_blank_response_after_valid_adaptive_plan_launches_tournament_not_no_progress(
    tmp_path,
):
    blocks = [
        tc("ops_submit_investigation", {"summary": "s", "confidence": 1.0}),
    ...
    r = OpsRunner(client=FakeClient(blocks), max_recovery_attempts=1, max_turns=4).run(
        req(tmp_path, orchestrator="adaptive", candidate_attempts=3)
    )
