    ...
    patch_backend_post(
        ...
        lambda *a, **k: type(
            ...
            (),
            {
                ...
                "json": lambda s: _tc("verifier_final_verdict", _verdict()),
            },
        )(),
    )
...
def test_native_read_tool_then_final_records_outer_and_inner(monkeypatch, tmp_path):
    ...
    run = load_debug_run(FIX)
    det = deterministic_result(run, mode="llm_tool_loop")
    trace = VerifierTraceWriter(tmp_path / "ws", FIX, tmp_path / "trace", True, "full")
    trace.start({"x": 1})
    calls = []
            ...
            if len(calls) == 1:
                return _tc(
                    "verifier_read_tool",
                    {
                        ...
                        "args": {"query": "PASS", "limit": 2},
                        "reason": "Need exact validation output.",
                    },
                )
            ...
            return _tc(
                ...
                _verdict(
                    toolsUsed=[
                        {
                            "tool": "search_commands",
                            "reason": "Inspected validation output.",
                        }
                    ]
                ),
            )
    ...
    rows = [
        json.loads(x)
        for x in (tmp_path / "trace" / "tool_calls.jsonl").read_text().splitlines()
    ]
    assert res["llmProtocol"] == "native_tool_calls"
    assert (
        rows[0]["llmTool"] == "verifier_read_tool"
        and rows[0]["verifierTool"] == "search_commands"
    )
    assert (
        json.loads((tmp_path / "trace" / "verification_result.json").read_text())[
            "llmProtocol"
        ]
        == "native_tool_calls"
    )
...
def test_unknown_native_tool_no_hallucinated_verdict(monkeypatch, tmp_path):
    ...
    patch_backend_post(
        ...
        lambda *a, **k: type(
            "R",
            (),
            {"raise_for_status": lambda s: None, "json": lambda s: _tc("bad_tool", {})},
        )(),
    )
    ...
    patch_backend_post(
        ...
        lambda *a, **k: type(
            ...
            (),
            {
                "raise_for_status": lambda s: None,
                "json": lambda s: _tc(
                    "verifier_final_verdict", _verdict(result=1, verdict="failure")
                ),
            },
        )(),
    )
...
def test_reasoning_content_traced_and_retried(monkeypatch, tmp_path):
    ...
    def post(*a, **k):
        ...
        data = (
            {
                "choices": [
                    {"message": {"content": "", "reasoning_content": "hidden final"}}
                ]
            }
            if n["i"] == 1
            else _tc("verifier_final_verdict", _verdict())
        )
        return type(
...
def test_backend_rejects_tools_falls_back_to_legacy(monkeypatch, tmp_path):
    ...
    def post(*a, **k):
        n["i"] += 1
        if "tools" in k["json"]:
            raise httpx.HTTPStatusError("tools rejected", request=None, response=None)
        return type(
            "R",
            (),
            {
...
def test_legacy_prose_plus_json_extracted(monkeypatch, tmp_path):
    ...
    patch_backend_post(
        ...
        lambda *a, **k: type(
            ...
            {
                "raise_for_status": lambda s: None,
                "json": lambda s: {"choices": [{"message": {"content": text}}]},
            },
        )(),
    )
    res = llm_result(run, det, workspace=str(tmp_path))
