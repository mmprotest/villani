...
def _state(tmp_path):
    ...
    return OpsRunState(
        ...
        plan={"strategy": "decompose_then_execute"},
        decomposition={"merge_strategy": "ordered"},
...
def _ctx(state):
    return OpsToolContext(
        ...
        recorder=OpsEventRecorder(Path(state.run_dir), "r"),
        transcript=[],
        production=False,
...
def _attempt(tmp_path, state, scope="subtask"):
    ...
    a = CandidateAttemptState(
        attempt_id="part_attempt_001",
        status="completed",
        scope=scope,
        subtask_id=("part" if scope == "subtask" else None),
        changed_files=["app.txt"],
        patch_path=str(patch),
        worktree_path=str(wt),
        review={"decision": "pass", "recommended_action": "accept", "blockers": []},
...
def test_supporting_and_diagnostic_failures_do_not_block_focused_passing_subtask_acceptance(
    tmp_path,
):
    s = _state(tmp_path)
    a = _attempt(tmp_path, s)
...
def test_acceptance_blocking_failure_blocks_candidate_and_observation(tmp_path):
    s = _state(tmp_path)
    a = _attempt(tmp_path, s, scope="candidate")
    c = _ctx(s)
    h_validation(
        s,
        OpsRunValidationInput(
...
def test_subtask_review_payload_exposes_scoped_decision_not_global_blocker(tmp_path):
    ...
    assert any(
        "Diagnostic and exploratory failures are evidence" in x
        for x in payload["subtask_review_criteria"]
    )
    ...
    res = h_validation(
        ...
        OpsRunValidationInput(
            ...
            commands=[
                ValidationCommand(
                    cmd='python -c "import sys; sys.exit(1)"',
                    source="investigation_discovered",
                    scope="subtask",
                    subtask_id="part",
                    purpose="discovered but not selected",
                ),
            ],
        ),
        c,
...
def test_subtask_name_similarity_does_not_promote_to_blocking(tmp_path):
    ...
    res = h_validation(
        ...
        OpsRunValidationInput(
            ...
            commands=[
                ValidationCommand(
                    cmd='python -c "import sys; sys.exit(1)" # part parser test',
                    source="investigation_discovered",
                    scope="subtask",
                    subtask_id="part",
                ),
...
def test_recovery_commits_review_accepted_focused_passing_subtask_instead_of_retry(
    tmp_path,
):
...
def test_runner_trace_failures_are_diagnostic_and_do_not_poison_attempt_validation(
    tmp_path,
):
    ...
    a = _attempt(tmp_path, s)
    c = _ctx(s)
    trace_dir = Path(a.artifacts_dir or tmp_path / "artifacts")
    trace_dir.mkdir(exist_ok=True)
    a.artifacts_dir = str(trace_dir)
    (trace_dir / "transcript.json").write_text(
        '{"commands":[{"cmd":"python -m pytest","passed":false,"status":"failed"}]}'
    )
