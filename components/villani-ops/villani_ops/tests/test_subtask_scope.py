...
from villani_ops.orchestration.engine import (
    _subtask_prompt,
    _subtask_review_prompt,
    OrchestrationEngine,
)
...
def test_subtask_prompt_prevents_full_task_solving_and_lists_scope():
    decomp = SimpleNamespace(
        subtasks=[
            SimpleNamespace(
                id="fix_pricing_logic", title="Pricing", objective="Fix pricing"
            ),
            SimpleNamespace(
                id="fix_inventory_atomicity",
                title="Inventory",
                objective="Fix inventory",
            ),
            SimpleNamespace(
                id="fix_checkout_payment_failure",
                title="Checkout",
    ...
    st = {
        ...
        "title": "Pricing",
        "objective": "Fix pricing",
        "relevant_files": ["src/signalshop/pricing.py"],
    }
    prompt = _subtask_prompt(
        Task(repo_path=".", objective="Fix shop", success_criteria="all pass"),
        decomp,
        st,
    )
    ...
    assert (
        "- fix_inventory_atomicity" in prompt
        and "- fix_checkout_payment_failure" in prompt
    )
...
def test_subtask_review_prompt_judges_current_subtask_only():
    st = {
        "id": "fix_receipt_formatting",
        "objective": "Fix receipt formatting",
        "relevant_files": ["src/signalshop/receipt.py"],
    }
    prompt = _subtask_review_prompt(
        Task(repo_path=".", objective="Fix pricing inventory checkout receipt"),
        st,
        [st, {"id": "fix_inventory_atomicity"}],
    )
...
def test_scope_analysis_records_overreach_overlap_and_skips_high_risk():
    ...
    subtasks = [
        {"id": "fix_pricing_logic", "relevant_files": ["src/signalshop/pricing.py"]},
        {
            "id": "fix_checkout_payment_failure",
            "relevant_files": ["src/signalshop/checkout.py"],
        },
    ]
    accepted = [
        {
            "subtask_id": "fix_pricing_logic",
            "changed_files": [
                "src/signalshop/pricing.py",
                "src/signalshop/checkout.py",
            ],
            "review": {"scope_ok": True, "integration_risk": "low"},
        },
        {
            "subtask_id": "fix_checkout_payment_failure",
            "changed_files": ["src/signalshop/checkout.py"],
            "review": {"scope_ok": True, "integration_risk": "low"},
        },
    ]
    ...
    assert scope["overlapping_files"]["src/signalshop/checkout.py"] == [
        "fix_pricing_logic",
        "fix_checkout_payment_failure",
    ]


def test_clean_scoped_patch_is_integrated_by_scope_analysis():
    engine = OrchestrationEngine.__new__(OrchestrationEngine)
    scope = engine._analyze_subtask_scope(
        [
            {
                "subtask_id": "fix_pricing_logic",
                "changed_files": ["src/signalshop/pricing.py"],
                "review": {"scope_ok": True, "integration_risk": "low"},
            }
        ],
        [{"id": "fix_pricing_logic", "relevant_files": ["src/signalshop/pricing.py"]}],
