...
def test_successful_write_read_and_final_summary_changed_files_create_evidence(
    tmp_path,
):
    d = _debug(
        tmp_path,
    ...
    assert any(
        e["kind"] == "file_write" and e["path"] == "meeting_scheduled.ics"
        for e in cats["fileMutation"]
    )
                            ...
                            "content": json.dumps(
                                {
                                    ...
                                    "reason": "Write created the required output file",
                                    "criticalRequirement": "required output file",
                                    "directEvidenceForCriticalRequirement": "Write created required output file",
...
def test_recovered_refused_tool_calls_do_not_remain_active_after_validation(tmp_path):
    d = _debug(
        tmp_path,
        "Push main and dev branches and verify https://localhost:8443/index.html",
        tools=[
            {
                "tool_call_id": "bad",
                "tool_name": "Bash",
        ...
        commands=[
            {
                "command": "git clone git@localhost:/git/project /tmp/final-test",
                "exit_code": 0,
                "stdout": "clone ok",
            },
            {"command": "git push origin main", "exit_code": 0, "stdout": "push ok"},
            {
                ...
                "exit_code": 0,
                "stdout": "PASS: Main branch serves correct content",
            },
        ],
    )
    pkt = build_packet(load_debug_run(d))
...
def test_inline_heredoc_local_function_is_weak_and_calibration_keeps_failure(tmp_path):
    d = _debug(
        tmp_path,
        "Implement largest_eigenvalue in /app/eigen.py",
        commands=[
            {
    ...
    v = {
        ...
        "reason": "Validation only tested an inline implementation, not /app/eigen.py",
        "riskFlags": [],
    }
    out = calibrate(det, v)
...
def test_read_debug_file_filename_alias_and_safety(tmp_path):
    ...
    assert (
        tools.read_debug_file(filename="tool_calls.jsonl")["path"] == "tool_calls.jsonl"
    )
...
def test_adjudication_changes_reason_not_stale():
    det = {
        "evidenceByCategory": {
            "activeFailures": [{"text": "material post validation failure"}],
            "recoveredFailures": [],
            "deliverableEvidence": [],
