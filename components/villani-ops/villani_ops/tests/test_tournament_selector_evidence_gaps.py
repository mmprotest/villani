...
def _review(cid="candidate_001", **kw):
    ...
    data = dict(
        ...
        evidence_used=kw.pop("evidence_used", []),
        evidence_gaps=kw.pop("evidence_gaps", []),
        strengths=kw.pop("strengths", []),
        risks=kw.pop("risks", []),
        likely_hidden_failures=kw.pop("likely_hidden_failures", []),
        edge_cases_considered=[],
        edge_cases_missed=kw.pop("edge_cases_missed", []),
        debug_artifact_findings=[],
        command_findings=[],
        patch_findings=[],
        minimality_score=0.8,
        correctness_score=kw.pop("correctness_score", 0.95),
        hidden_test_risk_score=kw.pop("hidden_test_risk_score", 0.1),
        review_quality=kw.pop("review_quality", "model_full"),
        recommendation=kw.pop("recommendation", "strong_accept"),
        rationale=kw.pop("rationale", "ok"),
    )
    data.update(kw)
...
def test_detects_generic_critical_evidence_gap_phrases():
    ...
    phrases = [
        "no test exercises critical path",
        "not verified cleanup",
        "no evidence rollback happens",
        "cannot determine recovery behaviour",
...
def test_detector_requires_unverified_and_critical_and_honors_authoritative_validation():
    ...
    assert not detect_critical_evidence_gaps(
        _review(
            evidence_gaps=["not verified cleanup"],
            strengths=["authoritative validation passed"],
        )
    )
...
def test_review_penalty_caps_scores_and_downgrades():
    ...
    rv = apply_review_risk_penalties(
        _review(
            evidence_gaps=["no evidence rollback happens"],
            correctness_score=9.5,
            hidden_test_risk_score=1,
            confidence=9,
...
def test_pairwise_draft_conversion_and_fallback_gap_caps():
    ...
    from villani_ops.agentic.tools import (
        _pairwise_from_draft,
        _compare_pair,
        apply_review_risk_penalties,
    )

    draft = PairwiseComparisonDraft(
        candidate_a="a",
        candidate_b="b",
        material_differences=["hash differs"],
        winner="candidate_a",
        confidence=8,
        rationale="r",
        extra="ignored",
    ...
    a = apply_review_risk_penalties(
        _review(
            "a",
            evidence_gaps=["not verified cleanup"],
            correctness_score=0.95,
            hidden_test_risk_score=0.1,
        )
    )
    b = _review("b", correctness_score=0.7, hidden_test_risk_score=0.4)
...
def test_ranking_downgrades_fallback_selected_with_critical_gap(tmp_path):
    ...
    s.candidate_risk_reviews = {
        "a": apply_review_risk_penalties(
            _review("a", evidence_gaps=["no test exercises error path"])
        ),
        "b": _review("b", correctness_score=0.5, hidden_test_risk_score=0.5),
    }
    ...
    assert rank.selection_confidence <= 0.45
