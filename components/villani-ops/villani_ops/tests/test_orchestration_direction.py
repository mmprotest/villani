...
def test_cli_modes_and_legacy_rejections(tmp_path):
    ...
    assert (
        runner.invoke(
            app, ["run", "--repo", str(tmp_path), "--task", "x", "--mode", "bad"]
        ).exit_code
        != 0
    )
    assert (
        ...
        in runner.invoke(
            app, ["run", "--repo", str(tmp_path), "--task", "x", "--policy", "cheap"]
        ).output
    )
    assert (
        "Backend assignment is controlled by the execution policy"
        in runner.invoke(
            app, ["run", "--repo", str(tmp_path), "--task", "x", "--backend", "x"]
        ).output
    )
    assert (
        "Human approval is not supported in the primary orchestration path"
        in runner.invoke(
            app, ["run", "--repo", str(tmp_path), "--task", "x", "--human-approval"]
        ).output
    )
    assert (
        "Unknown option: --bogus"
        in runner.invoke(
            app, ["run", "--repo", str(tmp_path), "--task", "x", "--bogus"]
        ).output
    )
...
def test_policies_route_from_real_signals():
    ...
    easy = TaskContext(
        objective="x",
        classification={"difficulty": "easy", "risk": "low", "confidence": 0.95},
        investigation={"confidence": 0.95},
        plan={"expected_difficulty": "easy", "confidence": 0.95},
    )
    hard = TaskContext(
        objective="x",
        classification={"difficulty": "hard", "risk": "high", "confidence": 0.4},
        plan={"expected_difficulty": "hard", "confidence": 0.4},
    )
    n = OrchestrationNode(id="code_attempt_001", kind="code", objective="code")
    assert (
        policy_for_mode("performance")
        .select_backend(node=n, backends=b, task_context=easy)
        .backend_name
        == "b-mid"
    )
    assert (
        policy_for_mode("cheap")
        .select_backend(node=n, backends=b, task_context=easy)
        .backend_name
        == "a-low"
    )
    assert (
        policy_for_mode("cheap")
        .select_backend(
            node=OrchestrationNode(id="c", kind="code", objective=""),
            backends=b,
            task_context=hard,
        )
        .backend_name
    ...
    assert (
        policy_for_mode("quality")
        .select_backend(
            node=OrchestrationNode(id="r", kind="review", objective=""),
            backends=b,
            task_context=easy,
        )
        .backend_name
    ...
    assert (
        policy_for_mode("cheap")
        .select_backend(
            node=OrchestrationNode(id="p", kind="plan", objective=""),
            backends=b,
            task_context=easy,
            prior_results=[NodeResult(status="failed")],
        )
        .backend_name
        == "c-high"
    )
    b["c-high"].enabled = False
    assert (
        policy_for_mode("performance")
        .select_backend(node=n, backends=b, task_context=easy)
        .backend_name
    ...
    b["a-low"].enabled = False
    b["b-mid"].enabled = False
    with pytest.raises(ValueError, match="No enabled backends"):
        policy_for_mode("performance").select_backend(
            node=n, backends=b, task_context=easy
        )
...
def test_graph_scheduler_and_shape(tmp_path):
    ...
    assert set(g.get("select").dependencies) == {
        "review_attempt_001",
        "review_attempt_002",
    }
    s = GraphScheduler()
    ready = s.next_ready_nodes(g)
    assert [n.id for n in ready] == ["classify"]
    g.mark_running("classify")
    g.mark_succeeded("classify", summary="ok")
    assert [n.id for n in s.next_ready_nodes(g)] == ["investigate"]
    g.mark_failed("investigate", "boom")
    s.next_ready_nodes(g)
    assert g.get("plan").status == "skipped"
    out = tmp_path / "orchestration_graph.json"
    g.write(out)
    assert out.exists() and "investigate" in out.read_text()
