...
class VerifierParallelRunState:
    ...
    def winner_candidate_id(self) -> str | None:
        return self.selection.get("winnerCandidateId") or self.orchestration.get(
            "winnerCandidateId"
        )
...
def _selection_sort_key(selection: MaterializableSelection) -> datetime:
    return (
        selection.completed_at
        or selection.created_at
        or _mtime_dt(selection.workspace_path / "materialization.json")
        or _mtime_dt(selection.workspace_path)
        or datetime.min.replace(tzinfo=timezone.utc)
    )


def discover_latest_verifier_parallel_state(
    workspace: str | Path,
) -> VerifierParallelRunState | None:
    ...
    for odir in dirs:
        if (odir / "selection.json").exists() and (odir / "integration.json").exists():
            return VerifierParallelRunState(
                odir,
                _read_json(odir / "selection.json"),
                _read_json(odir / "integration.json"),
                _read_json(odir / "orchestration.json"),
            )
    return None
...
def selection_from_orchestration_dir(odir: Path) -> MaterializableSelection | None:
    ...
    if mat_path.exists() and mat.get("status") not in MATERIALIZABLE_STATUSES:
        return None
    candidate_id = (
        mat.get("winnerCandidateId")
        or sel.get("winnerCandidateId")
        or integ.get("winnerCandidateId")
        or orch.get("winnerCandidateId")
    )
    if not candidate_id:
        return None
    result = (
        mat.get("winnerResult") if "winnerResult" in mat else sel.get("winnerResult")
    )
    fallback = bool(
        mat.get("fallbackWinner") or sel.get("fallback") or sel.get("fallbackWinner")
    )
    if result not in (1, "1", None) and not fallback:
        return None
    patch = (
        _resolve_path(mat.get("patchPath"), odir)
        or _resolve_path(sel.get("winnerPatchPath"), odir)
        or _resolve_path(integ.get("patchPath"), odir)
        or _candidate_patch_from_records(odir, candidate_id)
    )
    if not patch:
        return None
    source = mat.get("source") or orch.get("mode") or "verifier-orchestration"
    if source not in {"verifier-parallel", "verifier-sequential"} and not str(
        source
    ).startswith("verifier"):
        source = "verifier-orchestration"
    target = _resolve_path(
        mat.get("targetRepo") or integ.get("targetRepo") or orch.get("repo"), odir
    )
    meta = {
        "materialization": mat,
        "selection": sel,
        ...
        "materializationPath": str(mat_path) if mat_path.exists() else None,
    }
    created = _parse_dt(mat.get("createdAt") or orch.get("createdAt")) or _mtime_dt(
        mat_path if mat_path.exists() else odir
    )
    completed = _parse_dt(
        mat.get("completedAt") or orch.get("completedAt") or integ.get("completedAt")
    )
    return MaterializableSelection(
        str(source),
        created,
        completed,
        odir,
        ...
        patch,
        candidate_id,
        int(result) if str(result).isdigit() else result,
        fallback,
        meta,
    )


def discover_verifier_orchestration_materializations(
    workspace: str | Path,
) -> list[MaterializableSelection]:
    root = Path(workspace) / "orchestrations"
    if not root.exists():
        return []
    return [
        ...
        if odir.is_dir()
        for s in [selection_from_orchestration_dir(odir)]
        if s
    ]


def discover_legacy_materializations(
    workspace: str | Path,
) -> list[MaterializableSelection]:
    ...
    for rdir in root.iterdir():
        if (
            not rdir.is_dir()
            or not (rdir / "decision.json").exists()
            or not (rdir / "task.json").exists()
        ):
            continue
        ...
        if patch:
            out.append(
                MaterializableSelection(
                    "legacy-run",
                    _mtime_dt(rdir / "decision.json"),
                    None,
                    rdir,
                    repo,
                    patch,
                    dec.get("winning_attempt_id"),
                    1,
                    False,
                    {"decision": dec, "task": task},
                )
            )
    return out


def discover_materializable_selections(
    workspace: str | Path,
) -> list[MaterializableSelection]:
    return discover_legacy_materializations(
        workspace
    ) + discover_verifier_orchestration_materializations(workspace)


def discover_materializable_selection(
    workspace: Path, *, policy: str = "accepted"
) -> MaterializableSelection | None:
    sels = discover_materializable_selections(workspace)
    return max(sels, key=_selection_sort_key) if sels else None


def newest_materializable_selection(
    workspace: str | Path,
) -> MaterializableSelection | None:
...
def apply_patch_safely(repo: Path, patch: Path) -> dict[str, Any]:
    ...
    try:
        ...
        apply_args = ["apply", str(patch)]
        used_line_ending_fallback = False
        if chk.returncode != 0:
            status = _git(repo, ["status", "--porcelain", "--untracked-files=all"])
            line_ending_check = _git(
                repo,
                ["apply", "--ignore-space-change", "--check", str(patch)],
            )
            if (
                status.returncode != 0
                or status.stdout
                or line_ending_check.returncode != 0
            ):
                raise RuntimeError(
                    "git apply --check failed: "
                    + (chk.stderr.strip() or chk.stdout.strip())
                )
            apply_args = ["apply", "--ignore-space-change", str(patch)]
            used_line_ending_fallback = True
        ap = _git(repo, apply_args)
        if ap.returncode != 0:
            raise RuntimeError(
                "git apply failed: " + (ap.stderr.strip() or ap.stdout.strip())
            )
        return {
            "attempted": True,
            "exit_code": 0,
            ...
            "line_ending_fallback": used_line_ending_fallback,
        }
    finally:
        if temporary_git:
            git_dir = repo / ".git"
...
def materialize_latest(
    workspace: str | Path, repo: str | Path | None = None, *, policy: str = "accepted"
    ...
    logs.append(
        f"[materialize] newest run path: {selection.workspace_path if selection else None}"
    )
    if not selection:
    ...
    if selection.source.startswith("verifier"):
        logs += [
            f"[materialize] found verifier orchestration: {selection.workspace_path.name}",
            f"[materialize] source: {selection.source}",
            f"[materialize] winner candidate: {selection.winner_candidate_id}",
            f"[materialize] patch: {selection.patch_path}",
        ]
    try:
        artifact = apply_patch_safely(Path(target_repo), selection.patch_path)
        changed = artifact.get("changed_files") or []
        msg = (
            "empty selected patch; materialized no-op"
            if artifact.get("empty_patch")
            else "applied selected patch"
        )
        if selection.source.startswith("verifier"):
            logs.append(
                "[materialize] applied verifier orchestration patch"
                if not artifact.get("empty_patch")
                else "[materialize] verifier orchestration patch was empty; no changes applied"
            )
        for line in logs:
            print(line)
        return MaterializationResult(
