...
def safe_join_under(base: Path, requested: str) -> Path:
    ...
    target = (base / requested.lstrip("/")).resolve()
...
class ViewerServer:
    ...
    def start(self, try_ports=8):
        ...
        for p in range(self.port, self.port + try_ports):
            try:
                self.port = p
                self.httpd = ThreadingHTTPServer((self.host, p), self._handler())
                break
            except OSError as e:
                last = e
        if not self.httpd:
            raise last or OSError("viewer server failed")
    ...
    def _run_dir(self, run_id):
        if "/" in run_id or "\\" in run_id or run_id in {"", "..", "."}:
            return None
        try:
            p = safe_join_under(self.runs_dir, run_id)
        except ValueError:
            return None
            ...
            def do_GET(self):
                ...
                if path in {"/", ""}:
                    return self.send_text(
                        "<h1>Villani Ops Viewer</h1><p>Open /runs/&lt;run_id&gt;</p>"
                    )
                parts = [p for p in path.split("/") if p]
                if len(parts) == 2 and parts[0] == "runs":
                    rd = outer._run_dir(parts[1])
                    return (
                        self.send_text(render_viewer_html(None))
                        if rd
                        else self.notfound()
                    )
                if len(parts) >= 4 and parts[0] == "api" and parts[1] == "runs":
                    ...
                    name = parts[3]
                    if name == "snapshot":
                        return self.send_json(build_viewer_snapshot(rd))
                    files = {
                        "state": "state.json",
                    ...
                    if name in files:
                        fp = rd / files[name]
                        if not fp.exists():
                            return self.send_json({} if name != "events" else [])
                        if name == "events":
                            rows = []
