...
def static_index_html() -> str:
    return (files("villani_ops.viewer.static") / "index.html").read_text(
        encoding="utf-8"
    )
...
def render_viewer_html(snapshot: dict | None = None) -> str:
    ...
    if snapshot is None:
        return base
    blob = html.escape(
        json.dumps(snapshot, ensure_ascii=False, default=str), quote=False
    )
    return base.replace(
        "<script>\nconst embedded=",
        f'<script id="villani-run-snapshot" type="application/json">{blob}</script><script>\nconst embedded=',
        1,
    )
