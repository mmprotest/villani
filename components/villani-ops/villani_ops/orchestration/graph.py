...
class OrchestrationGraph(BaseModel):
    ...
    def dependency_satisfied(
        self, dep_id: str, node: OrchestrationNode | None = None
    ) -> bool:
        dep = self.get(dep_id)
        if dep.status == "succeeded":
            if (
                node is not None
                and node.kind == "code"
                and node.id.startswith("subtask_")
                and dep.id.startswith("subtask_")
                and dep.id.endswith("_acceptance_summary")
            ):
                return (
                    bool((dep.result or {}).get("accepted_attempt_id"))
                    or (dep.result or {}).get("status") == "accepted"
                )
            return True
        ...
        if (
            ...
            and dep.kind == "review"
            and dep.status in {"succeeded", "failed", "skipped"}
        ):
            return True
        return False
    ...
    def has_failed_required_dependency(self, node: OrchestrationNode) -> bool:
        return any(
            not self.dependency_satisfied(d, node)
            and self.get(d).status in {"failed", "skipped"}
    ...
    def ready_nodes(self) -> list[OrchestrationNode]:
        return [
            ...
            and not self.has_failed_required_dependency(n)
            and all(self.dependency_satisfied(d, n) for d in n.dependencies)
        ]
    ...
    def mark_succeeded(
        ...
        *,
        ...
        n.status = "succeeded"
        n.completed_at = _now()
        n.result_summary = summary or n.result_summary
        n.error = None
        if artifacts:
    ...
    def mark_failed(
        self, node_id: str, error: str, *, summary: str | None = None
    ) -> None:
        n = self.get(node_id)
        n.status = "failed"
        n.completed_at = _now()
        n.error = error
        n.result_summary = summary or error
