...
class GraphScheduler:
    def dependency_statuses(
        self, node: OrchestrationNode, graph: OrchestrationGraph
    ) -> dict[str, str]:
    ...
    def can_run(self, node: OrchestrationNode, graph: OrchestrationGraph) -> bool:
        return (
            node.status in {"pending", "ready"}
            and not graph.has_failed_required_dependency(node)
            and all(graph.dependency_satisfied(d, node) for d in node.dependencies)
        )
    ...
    def next_ready_nodes(self, graph: OrchestrationGraph) -> list[OrchestrationNode]:
        for node in graph.nodes:
            if node.status == "pending" and graph.has_failed_required_dependency(node):
                graph.mark_skipped(
                    node.id, "Required dependency failed or was unsafe to skip."
                )
