...
class LLMClient:
    def complete_json(
        ...
        if backend.provider not in {"openai-compatible", "local", "openai"}:
            raise ValueError(
                f"Backend provider '{backend.provider}' cannot be used for JSON LLM calls"
            )
        ...
        try:
            ...
            base = dict(
                ...
                output_tokens=out,
                estimated_cost=(
                    backend.estimate_cost(inp, out) if estimate_cost else 0.0
                ),
            ...
            try:
                ...
                raise LLMCallError(
                    f"LLM JSON parse failed for schema {schema_name} on backend {backend.name}: {pe}",
                    result=result,
                    parse_error=str(pe),
                ) from pe
            ...
            raise RuntimeError(
                f"LLM JSON call failed for schema {schema_name} on backend {backend.name}: {e}"
            ) from e
