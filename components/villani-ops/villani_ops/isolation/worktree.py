...
class GitWorktreeIsolation:
    def create(
        ...
        wt = Path(workspace).expanduser().resolve() / "worktrees" / run_id / attempt_id
...
def _changed_names(repo: Path, log: list) -> list[str]:
    names = subprocess.run(
        ["git", "diff", "--name-only", "HEAD"], cwd=repo, text=True, capture_output=True
    )
...
def capture_worktree(repo_path: str | Path, out_dir: str | Path) -> dict:
    ...
    diff = subprocess.run(
        ["git", "diff", "--binary", "HEAD"], cwd=repo, text=True, capture_output=True
    )
    names = subprocess.run(
        ["git", "diff", "--name-only", "HEAD"], cwd=repo, text=True, capture_output=True
    )
    ...
    changed = out / "changed_files.json"
    git_status = out / "git_status.txt"
    cmds = out / "git_commands.json"
    patch.write_text(diff.stdout)
    files = [x for x in names.stdout.splitlines() if x]
    changed.write_text(json.dumps(files, indent=2))
    git_status.write_text(status)
    cmds.write_text(json.dumps(log, indent=2))
    return {
        "patch_path": str(patch),
        "changed_files": files,
        "changed_files_path": str(changed),
        "git_status": status,
        "git_status_path": str(git_status),
        "git_commands_path": str(cmds),
        "filtered_generated_files": filtered_generated,
    }
