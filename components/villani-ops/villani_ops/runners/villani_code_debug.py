...
def _token_counts_from_usage(
    ...
    input_tokens, has_input = _first_token_value(
        usage, ("prompt_tokens", "input_tokens", "tokens_input")
    )
    output_tokens, has_output = _first_token_value(
        usage, ("completion_tokens", "output_tokens", "tokens_output")
    )
    total_tokens, has_total = _first_token_value(
...
def _is_command(t: dict[str, Any]) -> bool:
    ...
    args = _args(t)
    return (
        bool(args.get("command"))
        or t.get("tool_category") == "command"
        or name in {"bash", "shell", "exec", "runcommand", "terminal", "exec_command"}
        or "shell" in name
    )
...
def resolve_villani_code_trace_dir(debug_dir: Path) -> Path:
    ...
    if (debug_dir / "final_summary.json").exists() or (
    ...
    if debug_dir.exists():
        for child in debug_dir.iterdir():
            if child.is_dir() and (
                (child / "final_summary.json").exists()
                or (child / "summary.json").exists()
            ):
...
def parse_villani_code_debug_artifact(debug_dir: Path) -> VillaniCodeTelemetry:
    warnings: list[str] = []
    debug_dir = Path(debug_dir)
    trace_dir = resolve_villani_code_trace_dir(debug_dir)
    final = trace_dir / "final_summary.json"
    summ = trace_dir / "summary.json"
    ...
    tel = VillaniCodeTelemetry(
        debug_dir=str(debug_dir),
        resolved_trace_dir=str(trace_dir),
        token_accounting_warnings=warnings,
    )
    if final.exists():
        tel.final_summary_path = str(final)
    if summ.exists():
        tel.summary_path = str(summ)
    if not summary:
        tel.token_accounting_status = "missing"
        warnings.append("No final_summary.json or summary.json found.")
    ...
    for k in [
        ...
        "total_file_reads",
        "total_file_writes",
        "total_file_patches_applied",
        "total_file_patch_failures",
        "commands_executed",
        "commands_failed",
        "files_touched",
        "unique_files_read",
    ...
    tel.output_tokens = int(
        summary.get("tokens_output") or summary.get("output_tokens") or 0
    )
    ...
    tel.changed_files_from_debug = list(
        summary.get("changed_files_from_debug") or summary.get("changed_files") or []
    )
        ...
        for r in _jsonl(responses_path, warnings):
            ...
            if u:
                ...
                if has_total and not has_input:
                    warnings.append(
                        "Model response usage has total tokens but no input token key."
                    )
                if has_total and not has_output:
                    warnings.append(
                        "Model response usage has total tokens but no output token key."
                    )
        if count and (rin, rout, rtot) == (
            tel.input_tokens,
            tel.output_tokens,
            tel.total_tokens,
        ):
            tel.token_accounting_status = "verified"
        elif count:
            tel.token_accounting_status = "mismatch"
            warnings.append(
                f"Token totals mismatch: summary input/output/total={tel.input_tokens}/{tel.output_tokens}/{tel.total_tokens}; model_responses input/output/total={rin}/{rout}/{rtot}."
            )
        else:
            tel.token_accounting_status = "summary_only"
            warnings.append("No usable usage entries found in model_responses.jsonl")
    else:
        tel.token_accounting_status = "summary_only"
        warnings.append("Missing optional artifact: model_responses.jsonl")

    tools = (
        _jsonl(trace_dir / "tool_calls.jsonl", warnings)
        if (trace_dir / "tool_calls.jsonl").exists()
        else []
    )
    if tools:
        sorted_tools = sorted(
            enumerate(tools),
            key=lambda it: (
                _sort_timestamp(it[1].get("started_at"), warnings),
                it[1].get("turn_index")
                if it[1].get("turn_index") is not None
                else 10**9,
                it[0],
            ),
        )
        tel.total_tool_calls = len(sorted_tools)
        tel.tool_calls_by_name = (
            dict(Counter(_tool_name(t) for _, t in sorted_tools if _tool_name(t)))
            or tel.tool_calls_by_name
        )
        ...
        for idx, (_, t) in enumerate(sorted_tools, 1):
            ...
            name = _tool_name(t)
            cat = t.get("tool_category")
            args = _args(t)
            if tel.first_tool_call_index is None:
                tel.first_tool_call_index = idx
                tel.first_tool_call_seconds = sec
            if tel.first_substantive_file_read_tool_index is None and (
                name == "Read" or (cat == "file_read" and args.get("file_path"))
            ):
                tel.first_substantive_file_read_tool_index = idx
            ...
            if tel.first_command_tool_index is None and _is_command(t):
                tel.first_command_tool_index = idx
                tel.first_command_seconds = sec
    else:
...
def write_runner_telemetry(
    debug_dir: Path, out_path: Path, backend: Backend | None = None
) -> VillaniCodeTelemetry:
