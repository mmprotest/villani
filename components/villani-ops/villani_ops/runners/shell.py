...
class ShellRunner:
    ...
    def run(self, context: RunnerContext) -> RunnerResult:
        if not context.command:
            return RunnerResult(
                exit_code=2, stderr="Shell runner command is not configured."
            )
        ...
        vals = {
            ...
            "backend_model": context.backend.model,
            "backend_base_url": context.backend.base_url or "",
        }
        cmd = context.command.format(
            **{k: shlex.quote(str(v)) for k, v in vals.items()}
        )
        env = os.environ.copy()
        env.update(context.backend.env)
