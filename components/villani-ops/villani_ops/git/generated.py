...
def is_generated_or_cache_path(path: str) -> bool:
    ...
    if p.name in _GENERATED_NAMES or p.suffix in _GENERATED_SUFFIXES:
        return True
    if p.name.endswith(".egg-info") or any(
        part.endswith(".egg-info") for part in p.parts
    ):
        return True
...
def split_generated_paths(paths: list[str]) -> tuple[list[str], list[str]]:
    return (
        [p for p in paths if not is_generated_or_cache_path(p)],
        [p for p in paths if is_generated_or_cache_path(p)],
    )
