...
_CLEANUP_REQUIREMENT_RE = re.compile(
    r"\b(interrupt|interruption|cancel|cancellation|cleanup|clean up|shutdown|shut down|resource|rollback)\b",
    re.IGNORECASE,
)
_ABNORMAL_REQUIREMENT_RE = re.compile(
    r"\b(cleanup|clean up|cancellation|cancel|interrupt|interruption|rollback|shutdown|shut down|failure handling|timeout|persistence|recovery|edge cases?|resource management|resource)\b",
    re.IGNORECASE,
)
_SEVERE_RISK_RE = re.compile(
    r"\b(no command log|no repo test|only candidate|pass/fail unknown|critical requirement|no direct evidence|missing cleanup|missing cancellation|broad exception|swallow|fake test|hardcod|ignored failure|unrelated|resource leak)\b",
    re.IGNORECASE,
)
_VALIDATION_TERM_RE = re.compile(
    r"\b(tests?|specs?|checks?|verify|verification|validate|validation|assert|assertion)\b",
    re.IGNORECASE,
)
_BEHAVIOR_RE = re.compile(
    r"\b(runtime|validation|validated|execution|executed|observed|output|cleanup|cancellation|rollback|persistence|end-to-end|e2e|passed|ran|verified)\b",
    re.IGNORECASE,
)
_SOURCE_RE = re.compile(
    r"\b(source|inspection|appears|implemented|code added|function modified|import changed|diff|patch|changed file|updated)\b",
    re.IGNORECASE,
)
...
def _extract_command_records_from_debug_dir(debug_dir) -> list[dict]:
    ...
    for name in ("final_summary.json", "summary.json", "session_meta.json"):
        ...
        if data is not None:
            records.extend(_walk_command_records(data))
    return records
...
def _extract_command_records_from_text(text) -> list[dict]:
    ...
    for line in str(text or "").splitlines():
        ...
        if m:
            records.append({"_command": m.group(1).strip()})
    return records
...
def _passed_from_record(record) -> bool | None:
    ...
    for k in ("status", "result", "success"):
        if k in record:
            ...
            if sv in {"success", "passed", "pass", "ok", "completed"}:
                return True
            if sv in {"failure", "failed", "fail", "error"}:
                return False
    return None
...
def _extract_candidate_commands(candidate, verifier_result) -> list[str]:
    ...
    for path in (_candidate_stdout_path(candidate), _candidate_stderr_path(candidate)):
        if path and path.is_file():
            try:
                records.extend(
                    _extract_command_records_from_text(
...
_TEMP_INLINE_RE = re.compile(
    r"(<<|/tmp/|\b(?:temp|tmp|scratch|generated|ad-hoc|adhoc|one-off|inline)\b)",
    re.IGNORECASE,
)
_INLINE_ARG_RE = re.compile(
    r"(^|\s)-[A-Za-z]*[ce](\s|=).*(\bassert\b|[;{}()])", re.IGNORECASE | re.DOTALL
)
...
def _record_declares_test(record) -> bool:
    ...
    for key, value in record.items():
        if str(key) in _METADATA_VALIDATION_FIELDS and _VALIDATION_TERM_RE.search(
            str(value or "")
        ):
            return True
    return False
...
def _path_overlaps_changed_files(path_token, changed_files) -> bool:
    ...
    if not token:
        return False
    changed = [
        _normalize_path_for_overlap(str(x))
        for x in (changed_files or [])
        if str(x).strip()
    ]
    if not changed:
        return False
    direct = [
        c
        for c in changed
        if token == c or token.endswith("/" + c) or c.endswith("/" + token)
    ]
    if direct:
        return True
    ...
    return (
        ...
        and sum(1 for c in changed if c.split("/")[-1] == basename) == 1
    )
...
def _has_direct_abnormal_path_evidence(row) -> bool:
    if _has_repo_abnormal_path_test_evidence(row):
        return True
    return any(
        _mentions_abnormal_path(text) for text in _row_direct_evidence_texts(row)
    )


def _extract_candidate_tests(
    candidate, verifier_result, commands, changed_files
) -> list[dict]:
    tests = []
    seen = set()
    records = _extract_command_records_from_debug_dir(_candidate_debug_dir(candidate))
    ...
    for cmd in commands:
        ...
        if _record_declares_test(rec) or _command_looks_like_validation(cmd):
            item = {
                "command": cmd,
                "passed": _passed_from_record(rec),
                "source": _classify_test_source(cmd, changed_files, rec),
            }
        ...
        for field, passed in (("successEvidence", True), ("failureEvidence", False)):
            for item in _list_field(verifier_result, field):
                ...
                if _command_looks_like_validation(text) and text not in seen:
                    tests.append(
                        {"command": text[:300], "passed": passed, "source": "unknown"}
                    )
                    seen.add(text)
    return tests
...
def _evidence_item(requirement: str, evidence: Any) -> dict[str, str]:
    return {
        "requirement": str(requirement or "unspecified requirement"),
...
def build_candidate_evidence_matrix(
    candidates: list[Any], selected_candidate_id: str | None = None
) -> list[dict[str, Any]]:
    rows = []
    for c in candidates:
        v = _vr(c)
        ...
        missing_flags = missing + [
            str(r.get("requirement") or r.get("id") or "unsatisfied requirement")
            for r in reqs
        ...
        if v.get("criticalRequirementCovered") is False:
            ...
            v.get("criticalRequirementCovered") is True
            and v.get("criticalRequirementCoverageProven") is not True
        ):
            missing_flags.append(
                "critical requirement coverage was not proven by same-condition evidence"
            )
        direct = []
        source = []
        for t in tests:
            if t.get("passed") is True:
                direct.append(
                    _evidence_item(
                        "repo test" if t.get("source") == "repo" else "candidate test",
                        t["command"],
                    )
                )
        for x in success:
            success_text = str(x)
            target = source
            if _BEHAVIOR_RE.search(success_text) and (
                not _SOURCE_RE.search(success_text)
                or _mentions_abnormal_path(success_text)
            ):
                target = direct
            target.append(
                _evidence_item(
                    (
                        reqs[0].get("requirement")
                        if reqs
                        else v.get("criticalRequirement")
                    )
                    or "task behavior",
                    x,
                )
        ...
        if v.get("directEvidenceForCriticalRequirement"):
            direct.append(
                _evidence_item(
                    v.get("criticalRequirement") or "critical requirement",
                    v.get("directEvidenceForCriticalRequirement"),
                )
            )
        for r in reqs:
            ...
            if r.get("status") == "satisfied" and ev and _BEHAVIOR_RE.search(str(ev)):
                direct.append(
                    _evidence_item(
                        r.get("requirement") or r.get("id") or "requirement", ev
                    )
                )
        evidence_probe = {
            "tests_run": tests,
        ...
        for r in reqs:
            ...
            if r.get("status") == "satisfied" and _ABNORMAL_REQUIREMENT_RE.search(req):
                ...
                if _has_candidate_abnormal_path_test_evidence(evidence_probe):
                    ...
                    continue
                missing_flags.append(
                    "no direct evidence for abnormal-path requirement: " + req
                )
        if changed:
            source.append(_evidence_item("changed files", ", ".join(changed)))
        if _candidate_patch_path(c):
            source.append(_evidence_item("patch", str(_candidate_patch_path(c))))
        if not commands:
            risks.append("no command log evidence")
        if commands and not tests:
            risks.append("no repo test evidence found")
        if tests and all(t.get("passed") is None for t in tests):
            risks.append("test pass/fail unknown for all tests")
        if (
            tests
            and any(t.get("source") == "candidate" for t in tests)
            and not any(t.get("source") == "repo" for t in tests)
        ):
            risks.append("only candidate-authored tests found")
        ...
        repo_tests = sum(
            1 for t in tests if t["source"] == "repo" and t["passed"] is True
        )
        candidate_tests = sum(
            1 for t in tests if t["source"] == "candidate" and t["passed"] is True
        )
        ...
        score = {
            "direct_behavioral": float(
                sum(
                    {"strong": 4, "medium": 3, "weak": 1}[e["strength"]] for e in direct
                )
            ),
            "repo_tests": float(repo_tests * 8),
            "candidate_tests": float(candidate_tests * 1.5),
            "source_inference": float(min(len(source), 3) * 0.75),
            "requirement_coverage": float(12 * satisfied / total),
            "risk_penalty": float(risk_penalty),
            "final": 0.0,
        }
        score["final"] = (
            score["direct_behavioral"]
            + score["repo_tests"]
            + score["candidate_tests"]
            + score["source_inference"]
        ...
        rows.append(
            {
                "candidate_id": cid,
                "verifier_result": _result_label(v),
                "verifier_confidence": v.get("confidence"),
                "commands_run": commands,
                "tests_run": tests,
                "files_changed": changed,
                "debug_dir": str(_candidate_debug_dir(c))
                if _candidate_debug_dir(c)
                else None,
                "stdout_path": str(_candidate_stdout_path(c))
                if _candidate_stdout_path(c)
                else None,
                "stderr_path": str(_candidate_stderr_path(c))
                if _candidate_stderr_path(c)
                else None,
                "patch_path": str(_candidate_patch_path(c))
                if _candidate_patch_path(c)
                else None,
                "direct_behavioral_evidence": direct,
                "source_level_inference_evidence": source,
                "missing_requirement_flags": missing_flags,
                "risk_flags": risks,
...
def _evidence_rank_components(row: dict[str, Any]) -> tuple[Any, ...]:
    ...
    severe = sum(
        1
        for r in row["risk_flags"] + row["missing_requirement_flags"]
        if _SEVERE_RISK_RE.search(str(r))
    )
    return (
        -severe,
        -len(row["missing_requirement_flags"]),
        -len(row["risk_flags"]),
        s["repo_tests"],
        s["direct_behavioral"],
        s["candidate_tests"],
        s["requirement_coverage"],
        s["source_inference"],
        -s["risk_penalty"],
        s["final"],
        float(row["verifier_confidence"] or 0),
    )
...
def _make_selection_reason(row, winner, all_rows) -> str:
    repo = next(
        (
            t["command"]
            for t in row.get("tests_run", [])
            if t.get("source") == "repo" and t.get("passed") is True
        ),
        None,
    )
    ...
    parts = [
        f"Selected because {row['candidate_id']} had the strongest evidence-ranked selection record"
    ]
    details = []
    if direct:
        details.append(f"had direct behavioral evidence: {direct}")
    if repo:
        details.append(f'passed repo test evidence from "{repo}"')
    details.append(covered)
    if not row.get("missing_requirement_flags"):
        details.append("had no missing requirement flags")
    if loser:
        gap = (
            _snippet(loser.get("missing_requirement_flags"))
            or _snippet(loser.get("risk_flags"))
            or _snippet(loser.get("source_level_inference_evidence"))
            or "weaker evidence"
        )
        details.append(
            f"while closest losing candidate {loser['candidate_id']} had {gap}"
        )
...
def _make_rejection_reason(row, winner) -> str:
    gaps = []
    if row.get("missing_requirement_flags"):
        gaps.append("lacked " + str(row["missing_requirement_flags"][0]))
    if row.get("risk_flags"):
        gaps.append("had " + str(row["risk_flags"][0]))
    if not any(
        t.get("source") == "repo" and t.get("passed") is True
        for t in row.get("tests_run", [])
    ):
        gaps.append("had no passed repo test evidence")
    if not row.get("direct_behavioral_evidence") and row.get(
        "source_level_inference_evidence"
    ):
        gaps.append("only provided source-level inference")
    win_direct = _snippet((winner or {}).get("direct_behavioral_evidence"))
    if winner and win_direct:
        gaps.append(
            f"while {winner['candidate_id']} had direct behavioral validation: {win_direct}"
        )
    return (
        "Rejected because "
        + "; ".join(
            gaps or ["its concrete evidence was weaker than the selected candidate"]
        )
        + "."
    )


def finalize_evidence_reasons(
    rows: list[dict[str, Any]], winner_id: str | None
) -> list[dict[str, Any]]:
    rows = sorted(rows, key=lambda r: r["candidate_id"])
    rows.sort(key=_evidence_rank_components, reverse=True)
    ...
    for r in rows:
        if r["candidate_id"] == winner_id:
            ...
            r["final_selection_reason"] = _make_selection_reason(r, winner, rows)
        else:
            r["selection_status"] = "rejected"
            r["final_selection_reason"] = _make_rejection_reason(r, winner)
    return rows
...
def write_candidate_evidence_matrix(
    path: str | Path, matrix: list[dict[str, Any]]
) -> Path:
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(json.dumps(matrix, indent=2, default=str), encoding="utf-8")
    return p
...
def write_selection_report(
    path: str | Path, matrix: list[dict[str, Any]], winner_id: str | None
) -> Path:
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    ...
    lines = [
        ...
        "",
        "## Winner",
        f"- Candidate: {winner_id or ''}",
        f"- Reason: {(winner or {}).get('final_selection_reason', 'No winner selected.')}",
    ]
    llm_recommended = next(
        (r for r in matrix if r.get("llm_comparison_recommended")), None
    )
    if llm_recommended:
        note = llm_recommended.get("llm_comparison_advisory_note") or (
            "LLM comparison matched the evidence-ranked winner."
            if llm_recommended["candidate_id"] == winner_id
            else f"LLM comparison recommended {llm_recommended['candidate_id']}, but evidence-ranked selector selected {winner_id} because {(winner or {}).get('final_selection_reason', 'it had stronger evidence.')}"
        )
        lines += [
            "",
            "## LLM Comparison Advisory",
            f"- Recommended candidate: {llm_recommended['candidate_id']}",
            f"- Evidence-ranked winner: {winner_id or ''}",
            "- Used for final decision: no",
            f"- Notes: {note}",
        ]
    lines += [
        "",
        "## Candidate Ranking",
        "candidate_id | verifier_result | coverage | direct_behavioral | repo_tests | source_inference | risk_flags | status | reason",
        "--- | --- | ---: | ---: | ---: | ---: | --- | --- | ---",
    ]
    ranked = sorted(matrix, key=lambda r: r["candidate_id"])
    ranked.sort(key=_evidence_rank_components, reverse=True)
    for r in ranked:
        s = r["evidence_score"]
        lines.append(
            f"{r['candidate_id']} | {r['verifier_result']} | {s['requirement_coverage']:.2f} | {s['direct_behavioral']:.2f} | {s['repo_tests']:.2f} | {s['source_inference']:.2f} | {'; '.join(r['risk_flags'])} | {r['selection_status']} | {r['final_selection_reason']}"
        )
    lines += [
        "",
        "## Why the winner won",
        (winner or {}).get("final_selection_reason", "No winner selected."),
    ]
    if winner:
        lines += _bullets(
            "Direct behavioral evidence",
            [e.get("evidence") for e in winner.get("direct_behavioral_evidence", [])],
        )
        lines += _bullets(
            "Repo test evidence",
            [
                t.get("command")
                for t in winner.get("tests_run", [])
                if t.get("source") == "repo"
            ],
        )
        cand_tests = [
            t.get("command")
            for t in winner.get("tests_run", [])
            if t.get("source") == "candidate"
        ]
        if cand_tests:
            lines += _bullets("Candidate-authored test evidence", cand_tests)
        lines += _bullets(
            "Source-level inference",
            [
                e.get("evidence")
                for e in winner.get("source_level_inference_evidence", [])
            ],
        )
        lines += [
            "- Missing requirements avoided: "
            + (
                "none recorded"
                if not winner.get("missing_requirement_flags")
                else "; ".join(winner.get("missing_requirement_flags", [])[:3])
            )
        ]
        ...
        lower = (
            "; ".join(
                (
                    losers[0].get("risk_flags")
                    or losers[0].get("missing_requirement_flags")
                    or ["weaker concrete evidence"]
                )[:3]
            )
            if losers
            else "no losing candidate"
        )
    ...
    for r in ranked:
        ...
        lines += _bullets(
            "Missing requirement flags", r.get("missing_requirement_flags")
        )
        lines += _bullets("Risk flags", r.get("risk_flags"))
        lines += [
            "- Test evidence weakness: "
            + (
                "no passed repo test evidence"
                if not any(
                    t.get("source") == "repo" and t.get("passed") is True
                    for t in r.get("tests_run", [])
                )
                else "has passed repo test evidence"
            )
        ]
        lines += [
            "- Direct behavioral evidence weakness: "
            + (
                "none recorded"
                if not r.get("direct_behavioral_evidence")
                else _snippet(r.get("direct_behavioral_evidence"))
            )
        ]
        if winner:
            lines += [
                f"- Specific comparison to winner: {winner['candidate_id']} - {winner.get('final_selection_reason', '')}"
            ]
    gaps = []
    for r in matrix:
        if not r.get("commands_run"):
            gaps.append(f"{r['candidate_id']}: no command logs found")
        if not any(t.get("source") == "repo" for t in r.get("tests_run", [])):
            gaps.append(f"{r['candidate_id']}: no repo test evidence found")
        if r.get("tests_run") and all(
            t.get("source") == "candidate" for t in r.get("tests_run", [])
        ):
            gaps.append(f"{r['candidate_id']}: only candidate-authored tests found")
        if r.get("tests_run") and all(
            t.get("passed") is None for t in r.get("tests_run", [])
        ):
            gaps.append(f"{r['candidate_id']}: pass/fail unknown")
        for g in r.get("missing_requirement_flags", []):
            if "coverage was not proven" in g:
                gaps.append(
                    f"{r['candidate_id']}: critical requirement coverage unproven"
                )
            if "abnormal-path" in g:
                gaps.append(
                    f"{r['candidate_id']}: no direct evidence for abnormal-path requirement"
                )
            gaps.append(f"{r['candidate_id']}: {g}")
    lines += [
        "",
        "## Evidence gaps",
        *(f"- {g}" for g in (gaps or ["No evidence gaps recorded."])),
    ]
    p.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return p
...
def _vr(c):
    if hasattr(c, "verifier_result"):
        return getattr(c, "verifier_result") or {}
...
def _confidence(c):
    try:
        return float(
            (_vr(c).get("confidence") if _vr(c).get("confidence") is not None else 0.0)
            or 0.0
        )
    except Exception:
        return 0.0
...
def _summary(c):
    ...
    return {
        ...
        "directEvidenceForCriticalRequirement": v.get(
        ...
        "criticalRequirementEvidenceMatch": v.get("criticalRequirementEvidenceMatch"),
...
def _strong_accept(c) -> bool:
    return (
        _recommended_action(c) == "accept"
        and _critical_covered(c)
        and _critical_proven(c)
    )
...
def verifier_quality_details(candidate: Any) -> dict[str, Any]:
    ...
    satisfied_count = sum(
        1 for r in reqs if isinstance(r, dict) and r.get("status") == "satisfied"
    )
    unsatisfied_count = sum(
        1 for r in reqs if isinstance(r, dict) and r.get("status") == "unsatisfied"
    )
    success = _list_field(v, "successEvidence")
    behavioral_count = sum(
        1 for item in success if _STRONG_SUCCESS_EVIDENCE_RE.search(str(item))
    )
...
def _pick_by_quality(
    bucket: list[Any], rng: random.Random
    ...
    best = [
        ...
        if _evidence_rank_components(r) == best_key
    ]
    return next(c for c in bucket if _cid(c) == best_id), best, len(best) > 1


def select_winner(
    candidates: list[Any], seed: int, on_all_fail: str = "fail"
) -> SelectionResult:
    if on_all_fail not in VALID_ON_ALL_FAIL:
        raise ValueError("invalid on_all_fail")
    rng = random.Random(seed)
    allc = [_summary(c) for c in candidates]
    candidate_quality = _quality_rows(candidates)
    ...
    if successes:
        ...
        win, pool, random_tie = _pick_by_quality(bucket, rng)
        quality_applied = len(bucket) > 1
        if accepted:
            reason = (
                f"Selected {_cid(win)} by verifier quality tie-break among candidates with result = 1 and recommendedAction = accept with evidence-proven critical-requirement coverage."
                if not random_tie
                else "Selected randomly among strong accept-recommended candidates tied on verifier result and verifier quality key."
            )
        else:
            reason = (
                f"Selected {_cid(win)} by verifier quality tie-break among candidates with result = 1."
                if not random_tie
                else "Selected randomly among candidates tied on verifier result and verifier quality key."
            )
    elif on_all_fail == "fail":
        return SelectionResult(
            "villani-ops-verifier-parallel-selection-v1",
            POLICY,
            seed,
            on_all_fail,
            None,
            ...
            [],
            allc,
            "No candidates had verifier result = 1; on-all-fail=fail skipped integration.",
            candidateQuality=candidate_quality,
        )
    elif on_all_fail == "random":
        bucket = zeros or errors
        pool = [_cid(c) for c in bucket]
        win = rng.choice(bucket) if bucket else None
        fallback = "all-fail-random"
        reason = "All candidates failed; selected random fallback."
    else:
        if zeros:
            maxc = max(_confidence(c) for c in zeros)
            bucket = [c for c in zeros if _confidence(c) == maxc]
            if len(bucket) > 1:
                win, pool, random_tie = _pick_by_quality(bucket, rng)
                ...
                random_tie = False
            reason = (
                "All candidates failed; selected result = 0 candidate with highest verifier confidence."
                if not random_tie
                else "All candidates failed; selected randomly among result = 0 candidates tied on verifier confidence and verifier quality key."
            )
        else:
            bucket = errors
            pool = [_cid(c) for c in bucket]
            win = rng.choice(bucket) if bucket else None
            random_tie = len(bucket) > 1
            reason = (
                "All candidates had verifier errors; selected random error fallback."
            )
        fallback = "all-fail-best-confidence"
    v = _vr(win) if win else {}
    winner_quality = (
        next((r for r in candidate_quality if r["candidateId"] == _cid(win)), None)
        if win
        else None
    )
    return SelectionResult(
...
def build_llm_comparison_packet(
    candidates: list[Any], *, diff_limit: int = 2000, evidence_limit: int = 500
) -> list[dict[str, Any]]:
    packets = []
    for c in candidates:
        v = _vr(c)
        patch_path = getattr(c, "patch_path", None) or (
            c.get("patchPath") if isinstance(c, dict) else None
        )
        diff = ""
        if patch_path:
            try:
                ...
                diff = ""
        changed = (
            getattr(c, "changed_files", None)
            or (c.get("changedFiles") if isinstance(c, dict) else None)
            or []
        )
        packets.append(
            {
                ...
                "verifier": {
                    ...
                    "criticalRequirementCovered": v.get("criticalRequirementCovered"),
                    "criticalRequirementEvidenceRefs": _truncate_list(
                        v.get("criticalRequirementEvidenceRefs"), 10, evidence_limit
                    ),
                    "criticalRequirementCoverageProven": v.get(
                        "criticalRequirementCoverageProven"
                    ),
                    "criticalRequirementEvidenceMatch": v.get(
                        "criticalRequirementEvidenceMatch"
                    )
                    if isinstance(v.get("criticalRequirementEvidenceMatch"), dict)
                    else {},
                    "warnings": _truncate_list(v.get("warnings"), 5, evidence_limit),
                    "reason": _truncate_text(
                        v.get("reason") or v.get("summary"), evidence_limit
                    ),
                    "requirementResults": _truncate_list(
                        v.get("requirementResults"), 5, evidence_limit
                    ),
                    "successEvidence": _truncate_list(
                        v.get("successEvidence"), 5, evidence_limit
                    ),
                    "failureEvidence": _truncate_list(
                        v.get("failureEvidence"), 5, evidence_limit
                    ),
                    "missingEvidence": _truncate_list(
                        v.get("missingEvidence"), 5, evidence_limit
                    ),
...
def select_success_with_llm_comparison(
    *,
    task: str,
    ...
    if not eligible or not model or not base_url:
        return None
    backend = Backend(
        name="verifier-parallel-selector",
    ...
    user = json.dumps(
        {
            ...
            "responseSchema": {
                "selectedCandidateId": "candidate id from eligible list",
                "reason": "brief reason",
            },
            "task": task,
    ...
    if selected not in eligible:
        raise ValueError(
            f"LLM comparative selector returned invalid candidate id: {selected}"
        )
    return {
