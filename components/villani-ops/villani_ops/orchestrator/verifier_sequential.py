...
class VerifierSequentialOrchestrator(VerifierParallelOrchestrator):
    ...
    def __init__(
        self,
        config: VerifierSequentialConfig,
        runner=None,
        verifier=None,
        integrator=None,
    ):
        super().__init__(
            config, runner=runner, verifier=verifier, integrator=integrator
        )
    ...
    def _skipped_record(self, cid: str, winner_id: str | None) -> dict[str, Any]:
        return {
            "candidateId": cid,
            "status": "skipped",
            "skipReason": f"Earlier candidate {winner_id} verified as correct."
            if winner_id
            else "Not attempted.",
            "result": None,
            "verdict": None,
    ...
    def _summary(
        self, cr: CandidateResult, status: str | None = None
    ) -> dict[str, Any]:
        v = cr.verifier_result or {}
        verifier_trace = wire_path(v.get("traceDir"))
        return {
            "candidateId": cr.candidate_id,
            "status": status or ("selected" if v.get("result") == 1 else "verified"),
            "result": v.get("result"),
            "verdict": v.get("verdict"),
            "confidence": v.get("confidence"),
            "candidateDebugDir": wire_path(cr.debug_dir),
    ...
    def _first_success_selection(
        ...
        if winner:
            ...
            return {
                "schemaVersion": "villani-ops-verifier-sequential-selection-v1",
                "selectionPolicy": POLICY,
                "seed": seed,
                "onAllFail": on_all_fail,
                "winnerCandidateId": winner.candidate_id,
                "winnerResult": 1,
                "stoppedEarly": bool(skipped),
                "stopReason": f"{winner.candidate_id} verifier result = 1",
                "attemptedCandidates": attempted,
                "skippedCandidates": skipped,
                "allCandidates": allc,
                "reason": "Selected first candidate with verifier result = 1.",
            }
    ...
    def run(self):
        ...
        for i in range(1, cfg.candidates + 1):
            ...
            if winner:
                skipped = [
                    f"candidate-{j:03d}" for j in range(i + 1, cfg.candidates + 1)
                ]
                for sid in skipped:
                    append_jsonl(
                        odir / "candidates.jsonl",
                        self._skipped_record(sid, winner.candidate_id),
                    )
                break
        selection = self._first_success_selection(
            candidates, skipped, cfg.seed, cfg.on_all_fail, winner
        )
        if not winner and selection.get("winnerCandidateId"):
            winner = next(
                (
                    c
                    for c in candidates
                    if c.candidate_id == selection.get("winnerCandidateId")
                ),
                None,
            )
        ...
        if isinstance(integ, dict):
            integ["schemaVersion"] = "villani-ops-verifier-sequential-integration-v1"
            write_json(odir / "integration.json", integ)
        status = (
            "completed"
            if integ.get("status") in {"integrated", "skipped"}
            and (winner or cfg.on_all_fail == "fail")
            else "failed"
        )
        if selection.get("winnerCandidateId") is None and cfg.on_all_fail == "fail":
            status = "failed"
        mat = self._materialization_record(
            odir, "verifier-sequential", selection, integ, winner
        )
        orch = {
            "schemaVersion": "villani-ops-verifier-sequential-orchestration-v1",
            "orchestrationId": oid,
            "mode": "verifier-sequential",
            "createdAt": oid[:16],
            "completedAt": now(),
            "status": status,
            "repo": str(cfg.repo),
            ...
            "winnerCandidateId": selection.get("winnerCandidateId"),
            "selectionPolicy": POLICY,
        ...
        self._transcript(odir, candidates, skipped, selection, integ)
        ...
        out = {
            ...
            "stoppedEarly": selection.get("stoppedEarly", False),
            "attemptedCandidates": len(candidates),
            "skippedCandidates": len(skipped),
            ...
            "candidates": selection.get("allCandidates", []),
        }
        if cfg.out:
