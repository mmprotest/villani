...
def normalize_attempt_payload(
    raw_attempt: dict | str, default_reason: str = ""
) -> dict:
    ...
    for key in _TIMEOUT_ALIASES:
        if key in raw_attempt:
            ...
            break
    reason = (
        _first_string(raw_attempt, _REASON_ALIASES)
        or default_reason
        or "Normalized local-model policy attempt."
    )
    return {
        "backend": backend.strip(),
        "runner": runner.strip() or "villani_code",
        "max_attempts": _clamp(max_attempts, 1, 3),
...
def _normalize_attempt_list(value: Any, default_reason: str) -> list[dict]:
    if isinstance(value, list):
        return [
            a
            for item in value
            if (a := normalize_attempt_payload(item, default_reason))
        ]
...
def _trim_attempt_budget(attempts: list[dict], requested_profile: str) -> list[dict]:
    ...
    for attempt in attempts:
        ...
        attempt["max_attempts"] = _clamp(
            _coerce_int(attempt.get("max_attempts"), 1), 1, min(3, allowed)
        )
...
def normalize_execution_strategy_payload(raw: dict, requested_profile: str) -> dict:
    ...
    if isinstance(payload.get("attempts"), list):
        attempts = _normalize_attempt_list(
            payload.get("attempts"), "Created from attempts."
        )
    elif isinstance(payload.get("planned_attempts"), list):
        attempts = _normalize_attempt_list(
            payload.get("planned_attempts"), "Created from planned_attempts."
        )
    if not attempts:
        for source in (
            "execution_phases",
            ...
            if source in payload:
                attempts = _normalize_attempt_list(
                    payload.get(source), f"Created from {source}."
                )
                if attempts:
                    break
    if not attempts:
    ...
    if any(field in (raw or {}) for field in _PLANNING_FIELDS) and not attempts:
        payload["_normalization_error"] = (
            "Policy response contained planning fields but no attempts could be normalized."
        )
    ...
    if isinstance(stop, dict):
        mode = (
            stop.get("mode")
            or ("first_accepted" if stop.get("stop_on_success", True) else None)
            or "first_accepted"
        )
...
def _write_controller_error(
    ...
    if not run_dir:
        return
    d = Path(run_dir) / "controller_calls"
    ...
    data = {
        ...
        "model": getattr(result, "model", getattr(backend, "model", None)),
        "max_tokens": getattr(
            result, "max_tokens", getattr(backend, "max_tokens", None)
        ),
        "http_status": getattr(result, "http_status", None),
        "finish_reason": getattr(result, "finish_reason", None),
        "usage": getattr(result, "usage", {}) if result else {},
        "message_content": getattr(result, "raw_text", None),
        "reasoning_content": getattr(result, "reasoning_content", None),
        "raw_response": getattr(result, "raw_response", {}) if result else {},
        "parse_error": parse_error,
        "validation_error": validation_error,
        "raw_payload": raw_payload
        if raw_payload is not None
        else (getattr(result, "parsed_json", {}) if result else {}),
        "normalized_payload": normalized_payload or {},
        "fallback_used": fallback_used,
        "fallback_reason": fallback_reason
        or (validation_error or parse_error if fallback_used else None),
        "fallback_payload": fallback_payload or {},
    }
    (d / f"{phase}_error.json").write_text(json.dumps(data, indent=2, default=str))


def build_deterministic_fallback_strategy(
    ...
    def cost(b):
        return b.estimate_cost(1000, 1000)
    ...
    if profile == "cheap":
        attempts = [
            StrategyAttempt(
                backend=cheapest.name,
                max_attempts=1,
                reason="Deterministic fallback: cheapest coding backend.",
            )
        ]
        ...
        if cheapest.name == strongest.name:
            attempts = [
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=min(2, max_total),
                    reason="Deterministic fallback: only coding backend.",
                )
            ]
        else:
            attempts = [
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=1,
                    reason="Deterministic fallback: cheapest viable backend first.",
                ),
                StrategyAttempt(
                    backend=strongest.name,
                    max_attempts=1,
                    reason="Deterministic fallback: escalate to strongest backend.",
                ),
            ]
    else:
        ordered = []
        for b in [strongest] + sorted(
            coding, key=lambda b: (-b.capability_score, cost(b), b.name)
        ):
            if b.name not in [x.name for x in ordered]:
                ordered.append(b)
        ...
        for b in ordered:
            ...
            attempts.append(
                StrategyAttempt(
                    backend=b.name,
                    max_attempts=1,
                    reason="Deterministic fallback: quality profile escalation path.",
                )
            )
            remaining -= 1
        if remaining and attempts:
            attempts[0].max_attempts += remaining
    return ExecutionStrategy(
        profile=profile,
        strategy_summary="Policy generation failed validation, so Villani Ops used deterministic fallback policy.",
        attempts=attempts,
        warnings=[
            f"Policy generation failed validation, so Villani Ops used deterministic fallback policy. {reason}"
        ],
        stop_conditions={"mode": "first_accepted"},
    )


class StrategyAttempt(BaseModel):
    backend: str
    runner: str = "villani_code"
    max_attempts: int = 1
...
class ExecutionStrategy(BaseModel):
    ...
    cost_risk_summary: str = ""
    warnings: list[str] = Field(default_factory=list)
    backend_rankings: list[dict[str, Any]] = Field(default_factory=list)
...
class PolicyEngine:
    ...
    def generate(
        ...
        max_attempts: int | None = None,
    ) -> tuple[ExecutionStrategy, LLMCallResult]:
        from villani_ops.policy_engine.planner import (
            plan_execution_strategy,
            deterministic_policy_call,
        )

        if type(self.client) is LLMClient:
            strat = plan_execution_strategy(
                backends, classification, profile, max_attempts=max_attempts
            )
            if out_path:
                Path(out_path).write_text(strat.model_dump_json(indent=2))
        ...
        policy_backend = select_backend(backends, "policy")
        coding = coding_backends(backends)
        if not coding:
            raise ValueError("No enabled coding backends configured.")
        {b.name for b in coding}
        max_total = DEFAULT_PROFILES[profile]["max_total_attempts"]
        ctx = {
            "classification": classification.model_dump(),
            "profile": profile,
            "profile_rules": DEFAULT_PROFILES[profile],
            "coding_backends": [b.redacted_dict() for b in coding],
        }
        try:
            result = self.client.complete_json(
                policy_backend,
                SYSTEM,
                USER.format(context=json.dumps(ctx, indent=2)),
                "ExecutionStrategy",
            )
        except LLMCallError as e:
            strat = build_deterministic_fallback_strategy(
                classification, backends, profile, str(e)
            )
            _write_controller_error(
                ...
                fallback_used=True,
                fallback_payload=strat.model_dump(),
            )
            if out_path:
                Path(out_path).write_text(strat.model_dump_json(indent=2))
            return strat, e.result or LLMCallResult(
                parsed_json={},
                raw_text="",
                backend_name=policy_backend.name,
                model=policy_backend.model,
            )
        ...
        if normalized.get("_normalization_error"):
            ...
            strat = build_deterministic_fallback_strategy(
                classification, backends, profile, reason
            )
            _write_controller_error(
                Path(out_path).parent if out_path else None,
                "policy",
        ...
        for a in strat.attempts:
            if a.backend not in valid_by_name:
                warnings.append(
                    f"Removed invalid/non-coding/disabled backend {a.backend!r} from LLM strategy."
                )
                continue
            if a.runner != "villani_code":
                a.runner = "villani_code"
                warnings.append("LLM strategy runner was normalized to villani_code.")
            if total >= max_total:
                warnings.append(
                    f"LLM strategy was truncated to max {max_total} total attempts for {profile}."
                )
                break
            a.max_attempts = max(1, min(a.max_attempts, max_total - total))
            total += a.max_attempts
            kept.append(a)

        def est(b):
            budget = b.max_tokens or 8000
            mult = {"easy": 0.5, "medium": 1.0, "hard": 1.5}.get(
                classification.difficulty, 1.0
            )
            return b.estimate_cost(int(budget * mult), int(budget * 0.35 * mult))

        rankings = []
        for b in coding:
            c = max(est(b), 0.000001)
            cap = max(b.capability_score, 1)
            if profile == "cheap":
                key = (c, -cap)
            ...
            rankings.append(
                (
                    key,
                    {
                        ...
                        "estimated_attempt_cost": round(c, 6),
                        "expected_cost_rank": 0,
                        "rank_reason": f"{profile} policy ranking using capability {b.capability_score} and estimated attempt cost ${c:.6f}",
                    },
                )
            )
        rankings = [r for _, r in sorted(rankings, key=lambda x: x[0])]
        for i, r in enumerate(rankings, 1):
            r["expected_cost_rank"] = i
        # deterministic profile guardrails
        cheapest = sorted(
            coding,
            key=lambda b: (
                b.estimate_cost(1000, 1000),
                b.output_cost_per_million,
                b.input_cost_per_million,
                -b.capability_score,
            ),
        ...
        highest = sorted(
            coding,
            key=lambda b: (-b.capability_score, b.output_cost_per_million, b.name),
        )[0]
        ...
        if profile == "cheap" and easy:
            kept = [
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=1,
                    reason="Normalized cheap easy/low-risk profile to cheapest backend only.",
                )
            ]
            warnings.append(
                "LLM strategy was normalized to enforce cheap easy/low-risk constraints."
            )
            ...
            kept = [
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=1,
                    reason="Normalized cheap hard/high-risk profile to start cheapest.",
                )
            ] + (
                [
                    StrategyAttempt(
                        backend=esc.backend,
                        max_attempts=1,
                        reason=esc.reason or "One allowed escalation.",
                    )
                ]
                if esc
                else []
        ...
        if profile == "cheap" and kept and kept[0].backend != cheapest.name:
            kept.insert(
                ...
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=1,
                    reason="Normalized cheap profile to start with cheapest eligible backend.",
                ),
            )
            warnings.append(
                "LLM strategy was normalized to enforce cheap policy constraints."
            )
        if profile == "balanced" and kept and easy and kept[0].backend != cheapest.name:
            kept.insert(
                ...
                StrategyAttempt(
                    backend=cheapest.name,
                    max_attempts=1,
                    reason="Normalized balanced easy/low-risk profile to start cheaper.",
                ),
            )
            warnings.append(
                "LLM strategy was normalized to enforce balanced easy start constraints."
            )
        if profile == "quality" and hard and kept and kept[0].backend != highest.name:
            kept.insert(
                0,
                StrategyAttempt(
                    backend=highest.name,
                    max_attempts=1,
                    reason="Normalized quality hard/high-risk profile to start with highest-capability backend.",
                ),
            )
            warnings.append(
                "LLM strategy was normalized to enforce quality policy constraints."
            )
        ...
        for a in kept:
            ...
            total += a.max_attempts
            final.append(a)
        strat.attempts = final
        strat.profile = profile
        strat.backend_rankings = rankings
        strat.warnings.extend(warnings)
        if original != [a.backend for a in strat.attempts] and not warnings:
            strat.warnings.append(
                "LLM strategy was normalized to enforce policy constraints."
            )
