...
def build_timeline(run):
        ...
        pending.append(
            {
                ...
                "turn_index": None,
                "timestamp": c.ts,
                "status": str(c.exitCode) if c.exitCode is not None else None,
                "text": txt,
                "raw": c,
                "ts": _parse_ts(c.ts),
                "seq": len(pending),
        ...
        pending.append(
            {
                ...
                "timestamp": None,
                "status": str(p.ok),
                "text": f"patch[{p.index}] {p.filePath} ok={p.ok}",
                "raw": p,
                "ts": None,
                "seq": len(pending),
            }
    ...
    def base_key(e):
        ...
        if e["kind"] == "tool_call":
            return (
                1,
                e["turn_index"] if e["turn_index"] is not None else 10**9,
                e["tool_call_index"] or 0,
                e["seq"],
            )
        return (
            2,
            e["command_index"] if e["command_index"] is not None else e["seq"],
            e["seq"],
        )
