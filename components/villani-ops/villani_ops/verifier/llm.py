...
LLM_TOOLS = [
    {
        "type": "function",
        "function": {
            "name": "verifier_read_tool",
            "description": "Request a read-only verifier tool to inspect debug artifacts, commands, tool calls, transcripts, diffs, or repo files.",
            "parameters": {
                "type": "object",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Return the final binary verifier judgement.",
            "parameters": {
                "type": "object",
                "additionalProperties": False,
                "required": [
                ...
                "properties": {
                    ...
                    "reason": {"type": "string"},
                    "criticalRequirement": {"type": "string"},
                    "directEvidenceForCriticalRequirement": {"type": "string"},
                    "criticalRequirementCovered": {"type": "boolean"},
                    "criticalRequirementEvidenceRefs": {
                        "type": "array",
                        "items": {"type": "string"},
                    },
                    "criticalRequirementEvidenceMatch": {
                        ...
                        "additionalProperties": {
                            ...
                            "properties": {
                                "matchesCriticalRequirement": {"type": "boolean"},
                                "requirementCondition": {"type": "string"},
                                "evidenceCondition": {"type": "string"},
                                "whySameCondition": {"type": "string"},
                                "limitations": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                },
...
def _compact_value(value, max_text_chars):
    if isinstance(value, dict):
        return {k: _compact_value(v, max_text_chars) for k, v in value.items()}
    if isinstance(value, list):
        return [_compact_value(v, max_text_chars) for v in value]
    return _truncate_text(value, max_text_chars)


def _cap_list(items, first=0, last=0):
    items = list(items or [])
    if len(items) <= first + last or (first == 0 and len(items) <= last):
        return items
...
def compact_verifier_packet(packet: dict, max_text_chars: int = 900) -> dict:
    ...
    def strong(xs):
        return [
            x
            for x in xs or []
            if isinstance(x, dict) and x.get("validationStrength") == "strong"
        ]

    final = ev.get("finalEndToEndValidation") or []
    ev["finalEndToEndValidation"] = _cap_list(
        _dedupe_evidence(strong(final)[:5] + final[-8:]), last=13
    )
    for k, n in [
        ("testValidation", 8),
        ("serviceValidation", 8),
        ...
        ("fileMutation", 10),
        ("repoMutation", 10),
        ("deliverableEvidence", 10),
        ("constraintEvidence", 10),
    ]:
        if k in ev:
            ev[k] = _cap_list(_dedupe_evidence(ev.get(k)), last=n)
    if "recoveredFailures" in ev:
        ev["recoveredFailures"] = _cap_list(
            _dedupe_evidence(ev.get("recoveredFailures")), last=5
        )
    if "missingEvidence" in ev:
        ev["missingEvidence"] = _cap_list(
            _dedupe_evidence(ev.get("missingEvidence")), first=20, last=0
        )
    pkt["evidence"] = ev
    ...
    for k, vals in ce.items():
        ...
        for v in vals or []:
            if (
                isinstance(v, dict)
                and json.dumps(v, sort_keys=True, default=str) in flat_seen
            ):
                continue
            uniq.append(v)
    ...
    if isinstance(reg, dict):
        pkt["evidenceRegistry"] = {
            k: _compact_value(v, max_text_chars) for k, v in reg.items()
        }
    return pkt
...
def normalize_read_tool_call(obj, allowed_tools=TOOLS):
    if not isinstance(obj, dict):
        return None, "tool call is not an object"
    out = dict(obj)
    args = out.get("args")
    ...
    if (
        ...
        and isinstance(out["args"].get("tool"), str)
    ):
        tool = out["args"].get("tool")
        args = {k: v for k, v in out["args"].items() if k != "tool"}
    if not isinstance(tool, str) or not tool.strip():
        return None, "tool name is missing or invalid"
    tool = tool.strip()
    if tool not in allowed_tools:
        return None, "tool name is not allowed: " + tool
    out["tool"] = tool
    out["args"] = args
    out.setdefault("reason", "LLM requested tool call.")
    return out, None
...
def _is_final_state_decisive_negative(item):
    if not isinstance(item, dict):
        return False
    if item.get("finalStateDecisive") is True or item.get("final") is True:
        return True
    if (
        item.get("stale") is True
        or item.get("diagnosticOnly") is True
        or item.get("recovered") is True
    ):
        return False
    kind = str(
        item.get("kind") or item.get("evidenceKind") or item.get("type") or ""
    ).lower()
    strength = str(item.get("validationStrength") or "").lower()
    provenance = str(
        item.get("evidenceProvenance")
        or item.get("provenance")
        or item.get("source")
        or ""
    ).lower()
    category = str(item.get("category") or item.get("deterministicLabel") or "").lower()
    if kind in {"diagnostic", "source_inspection"}:
        return False
    return (
        strength == "failure"
        or "final" in category
        or provenance in {"command_output", "tool_observation", "file_content"}
    )
...
def deterministic_fallback_result(det, warning):
    ...
    active = [
        x
        for x in (cats.get("activeFailures") or [])
        if _is_final_state_decisive_negative(x)
    ]
    if da.get("missingDeliverables") or active:
        ...
        conf = 0.78
        action = "reject"
        reason = "Deterministic fallback: decisive missing deliverable or active failure evidence remained after LLM/tool-loop failure."
    else:
        result = None
        verdict = "error"
        conf = 0.0
        action = "inspect_manually"
        reason = "Deterministic fallback is failure-only; no decisive negative evidence was available after LLM/tool-loop failure."
    out = dict(det)
    risks = list(out.get("riskFlags") or [])
    risks.append(warning)
    warnings = list(out.get("llmProtocolWarnings") or []) + [
        warning,
        "Deterministic fallback success is disabled; ambiguous positive evidence requires manual inspection.",
    ]
    out.update(
        {
            "result": result,
            ...
            "fallbackReason": "llm_error_after_tool_loop",
            "fallbackPolicy": "failure_only",
            "warnings": warnings,
            "llmProtocolWarnings": warnings,
        }
    )
...
def select_verifier_backend(
    workspace=".villani-ops", backend_name=None, base_url=None, model=None, api_key=None
):
    if base_url and model:
        key = (
            api_key
            or os.getenv("VILLANI_OPS_VERIFIER_API_KEY")
            or os.getenv("OPENAI_API_KEY")
            or ("dummy" if _is_local(base_url) else "")
        )
        return {
            "name": "direct",
            "backend": "direct",
    ...
    if (
        (model or os.getenv("VILLANI_OPS_VERIFIER_MODEL"))
        and not backend_name
        and not base_url
    ):
        bu = os.getenv("VILLANI_OPS_VERIFIER_BASE_URL") or "http://127.0.0.1:1234/v1"
        mo = model or os.getenv("VILLANI_OPS_VERIFIER_MODEL")
        key = (
            api_key
            or os.getenv("VILLANI_OPS_VERIFIER_API_KEY")
            or os.getenv("OPENAI_API_KEY")
            or ("dummy" if _is_local(bu) else "")
        )
        return {
            "name": "direct",
            "backend": "direct",
    ...
    if backend_name:
        ...
        if not b:
            raise VerifierConfigurationError(
                f"missing verifier backend: {backend_name}"
            )
        if not b.enabled:
            raise VerifierConfigurationError(
                f"verifier backend disabled: {backend_name}"
            )
    else:
        elig = [b for b in backs.values() if b.enabled and (set(b.roles) & ROLES)]
        if not elig:
            raise VerifierConfigurationError("missing verifier backend or model config")
        b = sorted(
            elig,
            key=lambda x: (
                -x.capability_score,
                x.output_cost_per_million,
                x.input_cost_per_million,
                x.name,
            ),
        )[0]
    bu = base_url or b.base_url or "http://127.0.0.1:1234/v1"
    mo = model or b.model
    if not mo or not bu:
    ...
    try:
        ...
        if trace is not None:
            trace.append_jsonl(
                ...
                {
                    "index": getattr(trace, "llm_call_count", 0),
...
def _chat(cfg, messages, timeout, trace=None):
    return (
        _chat_message(cfg, messages, timeout, trace=trace, use_tools=False) or {}
    ).get("content", "")


def _json_object_from_mixed(s):
    if not s:
        raise VerifierSchemaError("empty response")
...
def _parse(s):
    ...
    if obj.get("type") == "tool_call":
        if obj.get("name") == "verifier_read_tool" and isinstance(
            obj.get("arguments"), dict
        ):
            return {"type": "tool_call", **obj["arguments"]}
        return obj
    ...
    obj["criticalRequirementEvidenceRefs"] = (
        [str(x) for x in refs] if isinstance(refs, list) else []
    )
    ...
    if not isinstance(obj.get("warnings"), list):
        obj["warnings"] = ["invalid_warnings_shape"]
    ...
    if match is None:
        ...
        for ref, entry in match.items():
            if not isinstance(entry, dict):
                obj["warnings"].append(
                    "invalid_critical_requirement_evidence_match_item"
                )
                continue
            ...
            cleaned_match[str(ref)] = {
                "matchesCriticalRequirement": entry.get("matchesCriticalRequirement")
                is True,
    ...
    for k in [
        ...
        obj.setdefault(k, [] if k != "reason" else "")
...
def _repair(cfg, bad, timeout, trace=None):
    msg = [
        ...
        {
            "role": "user",
            "content": "Repair this invalid verifier response to required final_verdict schema. "
            + _schema_text()
    ...
    if trace is not None:
        for m in msg:
            trace.append_jsonl(
                "llm_messages.jsonl",
                {
                    "index": getattr(trace, "msg_count", 0),
                    "role": m["role"],
                    "name": None,
                    "content": m["content"],
                    "createdAt": __import__("datetime")
            ...
            trace.msg_count = getattr(trace, "msg_count", 0) + 1
...
def _iter_evidence_items(det):
    cats = (
        (det or {}).get("evidenceByCategory", {})
        or (det or {}).get("evidence", {})
        or {}
    )
...
def _artifact_existence_only(det):
    ...
    return (
        da.get("artifactExistenceOnly") is True
        or (det or {}).get("artifactExistenceOnly") is True
    )


def _evidence_proves_critical_coverage(category, item, det):
    if (
        item.get("criticalRequirementCoverageProven") is True
        or item.get("coversCriticalRequirement") is True
    ):
        return True
    if (
        item.get("criticalRequirementCoverageProven") is False
        or item.get("coversCriticalRequirement") is False
    ):
        return False
    ...
    if not item.get("evidenceKind"):
        if str(category) in {
            ...
            kind = (
                "runtime_observation"
                if raw_kind in {"runtime_trace", "behavioral_trace"}
                else "validation"
            )
        elif str(category) == "deliverableEvidence" and (
            _artifact_existence_only(det)
            or raw_kind in {"content_check", "final_state_check"}
        ):
            kind = "artifact_check"
    if not item.get("evidenceProvenance") and provenance in {"", "commands"}:
        provenance = (
            "command_output"
            if str(category)
            in {"finalEndToEndValidation", "testValidation", "serviceValidation"}
            else provenance
        )
    if (
        not item.get("evidenceProvenance")
        and str(category) == "deliverableEvidence"
        and kind == "artifact_check"
    ):
...
def prove_critical_requirement_coverage(det, verdict):
    ...
    for ref in valid:
        ...
        if match.get("matchesCriticalRequirement") is not True:
            _warn_once(
                warnings, "critical_requirement_evidence_match_not_same_condition"
            )
            continue
        if limitations:
            _warn_once(
                warnings, "critical_requirement_evidence_match_has_material_limitations"
            )
            continue
...
def _make_disagreement(kind, summary, evidence=None):
    return {
        ...
        "summary": summary,
        "evidenceIds": [
            str((e or {}).get("id") or (e or {}).get("kind") or i)
            for i, e in enumerate(evidence or [])
        ],
        "effect": "risk_flag_only",
    }
...
def calibrate(det, verdict, trace=None, cfg=None, timeout=30):
    ...
    cats = det.get("evidenceByCategory", {}) or {}
    validations = (
        cats.get("finalEndToEndValidation", [])
        + cats.get("testValidation", [])
        + cats.get("serviceValidation", [])
    )
    strong = [
        v
        for v in validations
        if isinstance(v, dict) and v.get("validationStrength") == "strong"
    ]
    ...
    if verdict["result"] == 1 and active:
        disagreements.append(
            _make_disagreement(
                "active_failure_disagreement",
                "Deterministic collector found candidate failures, but the LLM judged the run successful.",
                active,
            )
        )
        verdict.setdefault("riskFlags", []).append(
            "Deterministic disagreement: evidence collector found unresolved-looking failures, but the LLM judged them non-blocking."
        )
        rules.append("add_deterministic_disagreement_risk")
    if verdict["result"] == 0 and (strong or cats.get("finalEndToEndValidation")):
        disagreements.append(
            _make_disagreement(
                "validation_signal_disagreement",
                "Deterministic collector found candidate validation signals, but the LLM judged the run unsuccessful.",
                strong or cats.get("finalEndToEndValidation", []),
            )
        )
        verdict.setdefault("riskFlags", []).append(
            "Deterministic disagreement: evidence collector found validation-looking signals, but the LLM judged them insufficient for the task contract."
        )
        rules.append("add_deterministic_disagreement_risk")
    da = det.get("deliverableAssessment") or {}
    if (
        verdict["result"] == 1
        and da.get("requiredDeliverables")
        and not (cats.get("deliverableEvidence") or strong)
    ):
        disagreements.append(
            _make_disagreement(
                "missing_deterministic_deliverable_label",
                "Deterministic deliverable labels are missing despite an LLM success judgement.",
                [],
            )
        )
        verdict.setdefault("riskFlags", []).append(
            "Deterministic deliverable evidence labels are missing; LLM success remains authoritative and should be audited if unexpected."
        )
    if verdict["result"] == 0 and cats.get("deliverableEvidence") and strong:
        disagreements.append(
            _make_disagreement(
                "deliverable_validation_disagreement",
                "Deterministic deliverable and validation signals exist, but the LLM judged the task contract unmet.",
                cats.get("deliverableEvidence", []) + strong,
            )
        )
        verdict.setdefault("riskFlags", []).append(
            "Deterministic deliverable and validation signals exist, but the LLM judged the task contract unmet."
        )
    if verdict["result"] == 1 and conf_before > 0.9:
        verdict["confidence"] = 0.9
        verdict.setdefault("riskFlags", []).append("Success confidence capped at 0.9.")
        rules.append("cap_success_confidence")
    verdict["criticalRequirementCoverageProven"] = prove_critical_requirement_coverage(
        det, verdict
    )
    ...
    if (
        disagreements
        and verdict.get("recommendedAction") == "accept"
        and not (
            verdict.get("result") == 1
            and verdict.get("criticalRequirementCoverageProven") is True
    ...
    cal = {
        "schemaVersion": "villani-ops-verifier-calibration-v2",
        "resultMutationAllowed": False,
        "rawLlmVerdict": raw,
        "finalResult": {
            k: final.get(k)
            for k in ["result", "verdict", "confidence", "recommendedAction", "reason"]
        },
        "resultChanged": False,
        "confidenceChanged": float(final.get("confidence", 0) or 0) != conf_before,
        "recommendedActionChanged": final.get("recommendedAction") != action_before,
        "riskFlagsAdded": [
            x for x in final.get("riskFlags", []) if x not in risk_before
        ],
        "deterministicDisagreements": disagreements,
        "contradictions": [d["summary"] for d in disagreements],
        "uncertaintyNotes": notes,
        "rulesApplied": rules,
        "auditAdjudication": {
            "enabled": False,
            "note": "Result-changing adjudication is disabled; deterministic disagreements are audit/risk only.",
        },
    }
    if trace is not None:
        trace.write_json("calibration.json", cal)
...
def llm_result(
    ...
    tools = VerifierTools(
        run, det.get("repoDir"), max_tool_result_chars, max_read_lines
    )
    ...
    if payload_chars > 60000:
        det.setdefault("warnings", []).append(
            "verifier_initial_payload_exceeds_safe_threshold"
        )
    messages = [
    ...
    while True:
        if time.monotonic() > deadline:
            raise VerifierLlmError("tool loop timeout")
        force = (
            {"type": "function", "function": {"name": "verifier_final_verdict"}}
        ...
        try:
            msg = _chat_message(
                ...
                trace=trace,
                use_tools=native_supported,
                force_tool=force,
            )
        except Exception as e:
            if native_supported and any(
                x in str(e).lower()
                for x in ["tool", "tools", "tool_choice", "function"]
            ):
                native_supported = False
                protocol = "legacy_json_fallback"
                protocol_warnings.append(
                    "Backend rejected native tool calls; falling back to legacy JSON protocol."
                )
                if trace is not None:
                    trace.append_jsonl(
                        "llm_protocol.jsonl",
                        {"protocol": protocol, "warning": protocol_warnings[-1]},
                    )
                continue
            if "400" in str(e) or "bad request" in str(e).lower():
                fb = deterministic_fallback_result(
                    det,
                    "LLM verifier HTTP/tool-loop failure; returned conservative deterministic fallback: "
                    + str(e),
                )
                if fb is not None:
                    return fb
        ...
        if trace is not None:
            ...
            if msg.get("reasoning_content"):
                trace.append_jsonl(
                    "llm_reasoning_content.jsonl",
                    {
                        "content": msg.get("reasoning_content"),
                        "chars": len(str(msg.get("reasoning_content"))),
                    },
                )
        if tc:
            call = tc[0]
            ...
            if cname == "verifier_read_tool":
                ...
                try:
                    ...
                    if not protocol_retried:
                        ...
                        messages.append(
                            {
                                "role": "user",
                                "content": "Your verifier_final_verdict arguments were invalid: "
                                + str(e)
                                + ". Return exactly one valid structured tool call.",
                            }
                        )
                        continue
                    raise
                ...
                if not protocol_retried:
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": "Unknown verifier tool call. Return exactly one valid structured tool call: verifier_read_tool or verifier_final_verdict.",
                        }
                    )
                    continue
            ...
            if not content.strip():
                if msg.get("reasoning_content") and not protocol_retried:
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": "Return your action using the structured tool call. Do not put the answer in reasoning_content.",
                        }
                    )
                    continue
                if native_supported and not empty_retried:
                    empty_retried = True
                    calls = max_tool_calls + 1
                    messages.append(
                        {
                            "role": "user",
                            "content": "Empty response received. Return the final judgement using verifier_final_verdict.",
                        }
                    )
                    continue
            ...
            try:
                ...
                if trace is not None:
                    trace.append_jsonl(
                        "llm_protocol.jsonl", {"protocol": "legacy_json_fallback"}
                    )
            except VerifierSchemaError:
                try:
                    obj = _repair(
                        cfg, content, max(1, deadline - time.monotonic()), trace=trace
                    )
                except Exception as e:
                    raise VerifierSchemaError(f"invalid JSON after repair: {e}")
        if obj.get("type") == "tool_call":
            if calls >= max_tool_calls:
                messages.append(
                    {
                        "role": "user",
                        "content": "Maximum tool calls reached. Return the final judgement using verifier_final_verdict.",
                    }
                )
                calls = max_tool_calls + 1
                continue
            ...
            if err:
                protocol_warnings.append(
                    "Rejected malformed verifier tool call: " + err
                )
                if not protocol_retried:
                    protocol_retried = True
                    messages.append(
                        {
                            "role": "user",
                            "content": "Your verifier_read_tool call was malformed: "
                            + err
                            + ". Return a valid verifier_final_verdict or a verifier_read_tool call with a non-empty allowed tool name.",
                        }
                    )
                    continue
                fb = deterministic_fallback_result(
                    det, "Malformed LLM tool call could not be normalized: " + err
                )
                if fb is not None:
                    return fb
            ...
            err = None
            try:
                raw_res = tools.dispatch(name, args)
            except Exception as e:
            ...
            if key in tool_result_cache:
                res = compact_tool_result_text(
                    "Repeated identical tool call; using cached compact summary from earlier result.\n"
                    + tool_result_cache[key],
                    600,
                )
            else:
                res = compact_tool_result_text(
                    raw_res, min(max_tool_result_chars, 2200)
                )
            ...
            if trace is not None:
                trace.append_jsonl(
                    ...
                    {
                        "index": idx,
                        "llmTool": "verifier_read_tool" if native_supported else None,
                        "verifierTool": name,
                        "tool": name,
                        "args": args,
                        ...
                        .datetime.now(__import__("datetime").timezone.utc)
                        .isoformat(),
                        "completedAt": __import__("datetime")
                        .datetime.now(__import__("datetime").timezone.utc)
                        .isoformat(),
                        "durationMs": int((time.time() - start) * 1000),
                        "status": status,
                        "resultChars": len(res),
                        "truncated": len(res) >= max_tool_result_chars,
                        "error": err,
                        "reason": obj.get("reason") or "LLM requested tool call.",
                    },
                )
                trace.append_jsonl(
        ...
        if trace is not None:
            trace.write_json(
                ...
                {"protocol": protocol, "rawText": content, "parsed": obj},
            )
            parsed = {
                "schemaVersion": "villani-ops-verifier-llm-verdict-v1",
                **{
                    k: obj.get(k)
                    for k in [
                        "result",
        ...
        obj.setdefault("riskFlags", [])
        obj["riskFlags"] += protocol_warnings
        obj = calibrate(
            det, obj, trace=trace, cfg=cfg, timeout=max(1, deadline - time.monotonic())
        )
        break
    det.update(
        {
            ...
            "reason": obj["reason"],
            "criticalRequirement": obj.get("criticalRequirement", ""),
            "directEvidenceForCriticalRequirement": obj.get(
                "directEvidenceForCriticalRequirement", ""
            ),
            "criticalRequirementCovered": obj.get("criticalRequirementCovered") is True,
            "criticalRequirementEvidenceRefs": obj.get(
                "criticalRequirementEvidenceRefs"
            )
            or [],
            "criticalRequirementEvidenceMatch": obj.get(
                "criticalRequirementEvidenceMatch"
            )
            or {},
            "criticalRequirementCoverageProven": obj.get(
                "criticalRequirementCoverageProven"
            )
            is True,
            "resultSource": obj.get("resultSource"),
            "postProcessingChangedResult": obj.get("postProcessingChangedResult"),
            "deliverableAssessment": obj.get(
                "deliverableAssessment", det.get("deliverableAssessment")
            ),
            "constraintAssessment": obj.get(
                "constraintAssessment", det.get("constraintAssessment")
            ),
            "requirementResults": obj.get(
                "requirementResults", det["requirementResults"]
            ),
            "successEvidence": obj.get("successEvidence", det["successEvidence"]),
            "failureEvidence": obj.get("failureEvidence", det["failureEvidence"]),
            "recoveredFailures": obj.get("recoveredFailures", det["recoveredFailures"]),
            "missingEvidence": obj.get("missingEvidence", det["missingEvidence"]),
            ...
            "calibration": obj.get("_calibration", {}),
            "verifier": {
                "mode": "llm_tool_loop",
                "backend": cfg["backend"],
                "model": cfg["model"],
...
def finalize_verifier_result(raw_llm_verdict, processed_verdict, trace_info=None):
    ...
    if final.get("result") != raw_llm_verdict.get("result") or final.get(
        "verdict"
    ) != raw_llm_verdict.get("verdict"):
...
def validate_final_result_consistency(result):
    ...
    if result.get("result") == 1:
        ...
        if any(
            p in reason
            for p in ["run failed", "did not solve", "unsatisfied", "blocking failure"]
        ) and not any(p in reason for p in ["despite", "although"]):
            result["reason"] = (
                "The verifier accepted the run based on deliverable-linked evidence; stale contradictory success/failure wording was replaced."
            )
            flags.append(
                "Final consistency replaced stale failure reason under success verdict."
            )
        ...
        if any(
            p in reason
            for p in ["run succeeded", "successfully solved", "accepted because"]
        ) and not any(p in reason for p in ["not ", "despite", "although"]):
            result["reason"] = (
                "The verifier rejected the run based on unresolved blocking or constraint evidence; stale success wording was replaced."
            )
            flags.append(
                "Final consistency replaced stale success reason under failure verdict."
            )
    if (
        result.get("result") == 1
        and result.get("recommendedAction") == "accept"
        and (
            result.get("criticalRequirementCovered") is not True
