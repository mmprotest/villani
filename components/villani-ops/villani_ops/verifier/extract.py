...
PASS_RE = re.compile(
    r"\b(PASS|test[s]? passed|all tests passed|all correctness tests passed|successful|succeeded|checks? passed|assertions? passed|verification complete)\b",
    re.I,
)
...
PATH_CAND_RE = re.compile(
    r'(?:https?://[^\s"\'<>]+|(?:\.?\.?/)?[A-Za-z0-9_.-]+(?:/[A-Za-z0-9_.-]+)+|[A-Za-z0-9_.-]+\.(?:py|R|r|stan|ics|tex|txt|csv|json|html|js|ts|md|yml|yaml|sh|c|h|sql|db|sqlite|xml))'
)
    ...
    if text:
        ...
        for p in parts:
            if len(p) > 8:
                req.append(
                    RequirementCheck(
                        hashlib.sha1(p.lower().encode()).hexdigest()[:10], p
                    )
                )
    if len(req) < 3:
        return [
            RequirementCheck("task_objective_addressed", "Task objective addressed"),
            RequirementCheck("final_validation_present", "Final validation present"),
            RequirementCheck("no_blocking_failures", "No blocking failures remain"),
        ]
    return req
...
def _basename(p):
    return _path_name(p).rstrip("/").split("/")[-1]


def _ext(p):
    b = _basename(p)
    return os.path.splitext(b)[1]


def _accept_path(p, known=None):
    p = _path_name(p)
    if not p or re.fullmatch(r"/?\d+\)?", p) or re.search(r"\(-?\d+/\d+\)?$", p):
        return False
    if re.fullmatch(r"/\d+", p):
        return False
    if URL_RE.fullmatch(p):
        return True
    if _ext(p) in KNOWN_EXT or _ext(p).lower() in {e.lower() for e in KNOWN_EXT}:
        return True
    if p.startswith("/app/"):
        return True
    if p.startswith(("./", "../")):
    ...
    if len(comps) >= 2 and any(re.search(r"[A-Za-z]", c) for c in comps):
        return True
...
def _tool_name(t):
    return (
        t.toolName
        or (t.raw or {}).get("tool_name")
        or (t.raw or {}).get("toolName")
        or (t.raw or {}).get("name")
        or ""
    ).lower()
...
def _extract_paths_from_obj(obj, known=None, scan_text=True):
    ...
    if isinstance(obj, dict):
        for k in [
            ...
            if obj.get(k):
                paths.append(_path_name(str(obj.get(k))))
    if scan_text:
        for m in PATH_CAND_RE.findall(_as_text(obj)):
            paths.append(_path_name(m))
    out = []
    for p in paths:
        if _accept_path(p, known) and p not in out:
            out.append(p)
    return out
...
def extract_deliverables(objective, run=None):
    ...
    for p in files:
        ext = _ext(p)
        p.lower()
        if ext.lower() in OUTPUT_EXT or re.search(
            r"(must|create|write|generate|output)\s+[^.]{0,80}"
            + re.escape(_basename(p).lower()),
            text.lower(),
        ):
            spec.required_output_files.append(p)
        else:
            spec.required_edited_files.append(p)
        spec.required_files.append(p)
    for pat, dst in [
        (r"\b(?:must|should)\s+install\s+([A-Za-z0-9_.+-]+)", spec.required_binaries),
        (r"\b(?:command|run)\s+`?([^`\n]+)`?", spec.required_commands),
    ]:
        for m in re.findall(pat, text, re.I):
            dst.append(m.strip())
    for fn in re.findall(
        r"\bdef\s+([A-Za-z_]\w*)|\bfunction\s+([A-Za-z_]\w*)|`([A-Za-z_]\w*)\(`", text
    ):
    ...
    for m in re.findall(
        ...
        spec.negative_constraints.append(m.strip())
    for m in re.findall(
        r"(?i)(only edit\s+[^.\n]+|only replace\s+[^.\n]+|using\s+synonyms\.txt|allowed\s+synonyms[^.\n]*)",
        text,
    ):
        spec.allowed_edit_constraints.append(m.strip())
    if run:
        for t in run.toolCalls:
            ...
            for p in _extract_paths_from_obj(
                _tool_args(t), spec.required_files, scan_text=False
            ):
                if p not in spec.required_files and any(
                    _basename(p) == _basename(x) for x in spec.required_files
                ):
                    spec.required_files.append(p)
    for attr in spec.__dataclass_fields__:
        vals = []
        [vals.append(x) for x in getattr(spec, attr) if x and x not in vals]
        setattr(spec, attr, vals)
    return spec
...
def _looks_deliverable(path, spec):
    ...
    if any(
        b == _basename(x).lower() or path == x
        for x in spec.required_files
        + spec.required_output_files
        + spec.required_edited_files
        + spec.required_generated_artifacts
    ):
        return True
    return bool(
        re.search(
            r"(output|answer|solution|result|meeting_scheduled|posterior_.*|.*_mean)\.(json|txt|csv|ics|html)$",
            b,
        )
    )
...
def _ics_struct(content):
    s = (content or "").upper()
    return {
        "kind": "ics_deliverable_structure",
        "hasVcalendar": "BEGIN:VCALENDAR" in s,
        "hasVevent": "BEGIN:VEVENT" in s,
        "hasSummary": "SUMMARY" in s,
        "hasDtstart": "DTSTART" in s,
        "hasDtend": "DTEND" in s,
        "attendeeCount": s.count("ATTENDEE"),
        "hasEndVevent": "END:VEVENT" in s,
        "hasEndVcalendar": "END:VCALENDAR" in s,
    }


def _ics_ok(preview):
    st = _ics_struct(preview)
    return (
        st["hasVcalendar"]
        and st["hasVevent"]
        and st["hasSummary"]
...
def tool_call_evidence(run, spec):
    ...
    for t in run.toolCalls:
        name = _tool_name(t)
        ok = _tool_success(t)
        paths = _extract_paths_from_obj(
            _tool_args(t), spec.required_files, scan_text=False
        ) or _extract_paths_from_obj(t.raw, spec.required_files, scan_text=False)
        order = t.turnIndex if t.turnIndex is not None else t.index
        tid = t.toolCallId
        if not ok:
            failures.append(
                EvidenceItem(
                    ...
                    "tool_calls",
                    "medium",
                    f"tool[{tid}] {t.toolName} failed: {t.error or t.resultSummary}",
                    tid,
                    t.turnIndex,
                    t.startedAt,
                    order,
            ...
            for p in paths or ["unknown file"]:
                ...
                mutations.append(
                    EvidenceItem(
                        ...
                        "tool_calls",
                        "high",
                        f"{(t.toolName or name)} tool created or updated {p}"
                        + (f" :: {prev[:120]}" if prev else ""),
                        tid,
                        t.turnIndex,
                        t.startedAt,
                        order,
                ...
                if _looks_deliverable(p, spec):
                    ...
                    if p.lower().endswith(".ics"):
                        struct = _ics_struct(prev)
                        strength = "strong" if _ics_ok(prev) else "medium"
                        extra = " :: " + json.dumps({"path": p, **struct})
                    deliverables.append(
                        EvidenceItem(
                            "deliverable_write",
                            "tool_calls",
                            "high",
                            f"Successful tool call created or updated required deliverable {p}{extra}",
                            tid,
                            t.turnIndex,
                            t.startedAt,
                            order,
            ...
            for p in paths or ["unknown file"]:
                inspections.append(
                    EvidenceItem(
                        ...
                        path=p,
                        toolCallId=tid,
                        deliverableLinked=_looks_deliverable(p, spec),
                        deliverableLinks=[p] if _looks_deliverable(p, spec) else [],
                    )
                )
...
def extract_evidence(run):
    ...
    for m in run.missingArtifacts:
        txt = (
            "No validations.jsonl artifact was present."
            if m == "validations.jsonl"
            else f"Missing artifact: {m}"
        )
        ...
        if p.ok:
            mutations.append(
                EvidenceItem(
                    ...
                    "patches",
                    "medium",
                    f"Patch applied successfully to {p.filePath}",
                    order=p.index,
                    path=p.filePath,
                )
            )
            ...
            for p in obj.get("changed_files") or obj.get("changedFiles") or []:
                mutations.append(
                    EvidenceItem(
                        ...
                        src,
                        "medium",
                        f"{src}.changed_files reports changed file {p}",
                        path=p,
                    )
                )
    for c in run.commands:
        ...
        if c.exitCode not in (None, 0) or FAIL_RE.search(blob):
            failures.append(
                EvidenceItem(
                    "failure_signal",
                    "commands",
                    "high" if is_validation_command(cmd) else "medium",
                    text,
                    c.toolCallId,
                    timestamp=c.ts,
                    order=c.index,
        ...
        if (
            (c.exitCode == 0 and is_validation_command(cmd))
            or PASS_RE.search(blob)
            or ("git clone" in cmd and c.exitCode == 0)
            or ("git push" in cmd and c.exitCode == 0)
        ):
            ev = EvidenceItem(
                "validation" if is_validation_command(cmd) else "success_signal",
                "commands",
                "high" if is_validation_command(cmd) else "medium",
                text,
                c.toolCallId,
    ...
    tm, ti, td, tf = tool_call_evidence(run, spec)
    mutations += tm
    success += td
    failures += tf
