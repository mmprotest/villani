...
def collect_relevant_file_snippets(
    repo_path: Path,
    task: str,
    tree: list[str],
    ...
    max_total_bytes: int = 30000,
) -> list[RelevantFileSnippet]:
    repo = Path(repo_path)
    task_l = task.lower()
    toks = _tokens(task)
    ...
    def add(rel, score, reason):
        rel = rel.replace("\\", "/")
        if rel not in tree or is_skipped_repo_file(rel):
            return
    ...
    for rel in tree:
        ...
        if r in mentions or low in {m.lower() for m in mentions}:
            add(r, 100, "path mentioned in task text")
        elif base in basenames:
            add(r, 90, "basename mentioned in task text")
        elif r in likely or base in {Path(x).name for x in likely}:
            add(r, 85, "likely file from classification metadata")
        overlap = len(toks & set(re.split(r"[^a-z0-9_]+", low)))
        if overlap:
            add(r, 20 + overlap, f"path contains {overlap} task token(s)")
    ...
    if re.search(r"\b(failing|failed|pytest|tests?)\b", task_l):
        for rel in tree:
            low = rel.lower()
            if (
                "test" in Path(low).name or "/tests/" in "/" + low
            ) and not is_skipped_repo_file(rel):
                overlap = len(toks & set(re.split(r"[^a-z0-9_]+", low)))
    ...
    mentioned_test_bases = {
        Path(r).name.lower() for _, r, _ in candidates if "test" in Path(r).name.lower()
    }
    source_bases = {b.replace("test_", "", 1) for b in mentioned_test_bases} | {
        b.replace("_test", "") for b in mentioned_test_bases
    }
    for rel in tree:
        base = Path(rel).name.lower()
        low = rel.lower()
        if (
            base in source_bases
            and "/test" not in "/" + low
            and not is_skipped_repo_file(rel)
        ):
    ...
    for rel in ordered:
        ...
        excerpt = _read_excerpt(
            repo, rel, min(max_bytes_per_file, max_total_bytes - total)
        )
        if excerpt is None:
            continue
