...
class ExecutionPolicy(Protocol):
    ...
    def select_backend(
        ...
        *,
        ...
        prior_results: list[NodeResult] | None = None,
    ) -> BackendSelection: ...


def enabled_backends(
    backends: Mapping[str, BackendConfig],
) -> list[tuple[str, BackendConfig]]:
    vals = [(n, b) for n, b in backends.items() if getattr(b, "enabled", True)]
    if not vals:
        raise ValueError("No enabled backends configured")
    return vals
...
def required_role_for_node(node: OrchestrationNode) -> str:
    if node.kind == "code":
        return "coding"
    if node.kind in {"review", "final_review"}:
        return "review"
    if node.kind in {"classify"}:
        return "classification"
    if node.kind in {"investigate"}:
        return "investigation"
    if node.kind in {"plan", "decompose"}:
        return "policy"
    if node.kind in {"select", "verify", "integration_validate"}:
        return "selection"
    if node.kind in {"integrate", "integration_repair"}:
        return "coding"
    return node.kind


def filter_backends_for_role(
    backends: Mapping[str, BackendConfig], role: str
) -> dict[str, BackendConfig]:
    vals = {
        n: b
        for n, b in backends.items()
        if getattr(b, "enabled", True) and role in (getattr(b, "roles", []) or [])
    }
    return vals


def select_backend_for_role(
    role: str,
    policy: str,
    registry: Mapping[str, BackendConfig],
    task_context: TaskContext | None = None,
    *,
    preferred: str = "highest",
) -> tuple[str, BackendConfig, str]:
    requested_role = role
    ...
    if preferred == "lowest":
        ...
        n, b = ordered[len(ordered) // 2]
    else:
        n, b = sorted(candidates.items(), key=lambda x: (-x[1].capability_score, x[0]))[
            0
        ]
    if fallback_role:
        ...
        reason = (
            f"Filtered by required role {role} before {preferred} capability selection."
        )
    return n, b, reason


def sort_by_capability(
    backends: Mapping[str, BackendConfig],
) -> list[tuple[str, BackendConfig]]:
    return sorted(
        enabled_backends(backends), key=lambda x: (x[1].capability_score, x[0])
    )


def highest_capability(backends):
    return sorted(
        enabled_backends(backends), key=lambda x: (-x[1].capability_score, x[0])
    )[0]


def lowest_capability(backends):
    return sort_by_capability(backends)[0]


def middle_capability(backends):
    vals = sort_by_capability(backends)
    return vals[len(vals) // 2]


def role_highest(node, backends, task_context=None):
    return select_backend_for_role(
        required_role_for_node(node),
        "policy",
        backends,
        task_context,
        preferred="highest",
...
def _strongest_signal(values: list[str], order: list[str]) -> str:
    ...
    return max(values, key=lambda v: ranks.get(v, 0)) if values else order[0]


def estimate_node_difficulty(
    node: OrchestrationNode, task_context: TaskContext
) -> tuple[str, str, float]:
    diffs = [node.difficulty]
    risks = [node.risk]
    confs = []
    for src in [
        task_context.classification,
        task_context.investigation,
        task_context.plan,
        task_context.decomposition,
    ]:
        if not src:
            continue
        ...
        if isinstance(src.get("confidence"), (int, float)):
            confs.append(float(src["confidence"]))
        if (
            src is task_context.investigation
            and isinstance(src.get("relevant_files"), list)
            and len(src["relevant_files"]) >= 6
        ):
            diffs.append("hard")
    if node.kind in {"review", "select"} and "easy" not in diffs:
        diffs.append("medium")
    if node.kind in {"code", "verify", "select"} and any(
        "blocker" in str(x).lower() for x in (task_context.plan or {}).get("risks", [])
    ):
        risks.append("high")
    if (
        task_context.decomposition
        and len(task_context.decomposition.get("subtasks") or []) > 2
    ):
        diffs.append("hard")
    ...
    confidence = (
        ...
        else (min(confs) if confs else task_context.confidence)
    )
