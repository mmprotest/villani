...
class ReviewResult(BaseModel):
    ...
    recommended_action: Literal[
        "accept", "retry_same_backend", "escalate", "ask_human", "fail"
    ] = "fail"
...
def normalize_review_payload(raw: dict) -> dict:
    ...
    if dk in pass_map:
        ...
        decision = "fail"
    elif dk in uncertain_map:
        decision = "uncertain"
    elif "passed" in data:
        decision = "pass" if _bool(data.get("passed")) else "fail"
    else:
        decision = "uncertain"
    data["decision"] = decision
    action_map = {
        "accept_attempt": "accept",
        "accept": "accept",
        "approve": "accept",
        "approved": "accept",
        "use_this": "accept",
        "ship": "accept",
        "retry": "retry_same_backend",
        "try_again": "retry_same_backend",
        "retry_same_backend": "retry_same_backend",
        "rerun": "retry_same_backend",
        "escalate": "escalate",
        "use_stronger_model": "escalate",
        "stronger_model": "escalate",
        "next_backend": "escalate",
        "ask_human": "ask_human",
        "human": "ask_human",
        "human_review": "ask_human",
        "manual_review": "ask_human",
    ...
    data["recommended_action"] = (
        action_map.get(ak)
        or ({"pass": "accept", "fail": "fail", "uncertain": "ask_human"}[decision])
    )
    ...
    if ev is None:
        ...
        ev = [
            json.dumps(ev, sort_keys=True) if isinstance(ev, (dict, list)) else str(ev)
        ]
    data["evidence"] = ev
...
class LLMReviewer:
    ...
    def review(
    ...
    ) -> tuple[ReviewResult, LLMCallResult]:
        ...
        ctx = {
            ...
            "coding_backend": coding_backend.redacted_dict(),
            "attempt": attempt,
        }
        ...
        try:
            result = self.client.complete_json(
                backend,
                SYSTEM,
                USER.format(context=json.dumps(ctx, indent=2)[:60000]),
                "ReviewResult",
        ...
        try:
            ...
            setattr(e, "llm_result", result)
            setattr(e, "schema_name", "ReviewResult")
            setattr(e, "backend", backend)
            setattr(e, "normalized_payload", normalized)
            raise
        review.reviewer_backend = backend.name
        if backend_override is not None:
            review.assigned_backend = {"name": backend.name, "model": backend.model}
        if out_path:
            Path(out_path).write_text(review.model_dump_json(indent=2))
