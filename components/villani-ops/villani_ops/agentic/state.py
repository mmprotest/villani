...
def _mentions_unproven_critical_behaviour(items: list[str]) -> bool:
    text = " ".join(str(x).lower() for x in items)
    return any(t in text for t in UNVERIFIED_EVIDENCE_TERMS) and any(
        t in text for t in CRITICAL_BEHAVIOUR_TERMS
    )
...
class CandidateAttemptState(BaseModel):
    ...
    runner_telemetry: dict = Field(default_factory=dict)
    review_status: Literal[
        "not_run", "passed", "failed", "unavailable", "malformed", "provider_error"
    ] = "not_run"
...
class CandidateEvidencePacket(BaseModel):
    ...
    final_changed_file_summaries: list[ChangedFileEvidence] = Field(
...
class CandidateRiskReview(BaseModel):
    ...
    review_quality: Literal[
        "model_full", "model_compact", "model_minimal", "deterministic_fallback"
    ...
    recommendation: Literal[
        "strong_accept", "accept", "weak_accept", "reject", "uncertain"
    ]
    rationale: str
...
class PairwiseCandidateComparison(BaseModel):
    ...
    candidate_b: str
    material_differences: list[str] = Field(default_factory=list)
    a_evidence_advantages: list[str] = Field(default_factory=list)
    b_evidence_advantages: list[str] = Field(default_factory=list)
    a_likely_failures: list[str] = Field(default_factory=list)
    b_likely_failures: list[str] = Field(default_factory=list)
    winner: Literal["candidate_a", "candidate_b", "tie", "neither"]
    confidence: float
    comparison_quality: Literal[
        "model_full", "model_compact", "model_minimal", "deterministic_fallback"
    ] = "deterministic_fallback"
...
class CandidateAgreementSummary(BaseModel):
    ...
    consensus_type: Literal[
        "same_patch", "same_answer", "same_strategy", "mixed", "none"
    ]
    agreeing_candidates: list[str] = Field(default_factory=list)
    material_differences: list[str] = Field(default_factory=list)
...
class SubtaskState(BaseModel):
    ...
    attempts: list[CandidateAttemptState] = Field(default_factory=list)
...
def _depends_on_failed(
    sid: str, by: dict, failed: set[str], seen: set[str] | None = None
) -> bool:
    seen = seen or set()
    if sid in seen:
        return False
    ...
    if not st:
        return False
    return any(
        d in failed or _depends_on_failed(d, by, failed, seen) for d in st.dependencies
    )
...
def detect_decomposition_deadlock(state: "OpsRunState") -> DecompositionDeadlock | None:
    ...
    pending = sorted(
        s.subtask_id for s in state.subtasks if s.status in {"pending", "running"}
    )
    blocked = sorted(
        s.subtask_id
        ...
        or (
            ...
            and _depends_on_failed(s.subtask_id, by, failed)
        )
    )
    ready = [
        s.subtask_id
    ...
    exhausted = any(
        s.status == "failed"
        and len(s.attempts) >= max(1, int(state.candidate_attempts or 1))
        for s in state.subtasks
    )
    incomplete = any(
        s.status not in {"accepted", "skipped"} for s in state.subtasks
    ) or bool(blocked)
    if (blocked or incomplete) and not ready and exhausted:
        reasons = ["required_subtask_failed"]
        if blocked:
            reasons += ["dependency_failed", "blocked_dependents_exist"]
        if exhausted:
            reasons.append("subtask_attempts_exhausted")
        if not ready:
            reasons.append("no_ready_subtasks_remaining")
        return DecompositionDeadlock(
            deadlocked=True,
            reason=",".join(dict.fromkeys(reasons)),
            failed_subtasks=sorted(failed),
            blocked_subtasks=blocked,
            accepted_subtasks=accepted,
            pending_subtasks=pending,
            can_continue_subtasks=False,
        )
    return None
...
class OpsRunState(BaseModel):
    ...
    candidate_execution_mode: Literal["unknown", "sequential", "parallel"] = "unknown"
    attempts_requested: int | None = None
    attempts_started: int = 0
    stopped_early: bool = False
    stop_reason: str | None = None
    tournament_candidates_launched: int = 0
    tournament_candidates_completed: int = 0
    tournament_parallelism_used: int = 0
    tournament_phase: Literal[
        "not_started",
        "launching_candidates",
        "candidates_complete",
    ...
    candidate_evidence_packets: dict[str, CandidateEvidencePacket] = Field(
        default_factory=dict
    )
    candidate_risk_reviews: dict[str, CandidateRiskReview] = Field(default_factory=dict)
    pairwise_comparisons: list[PairwiseCandidateComparison] = Field(
        default_factory=list
    )
    tournament_ranking: TournamentRanking | None = None
    candidate_agreement_summary: CandidateAgreementSummary | None = None
    selection_basis: (
        Literal[
            "validated_acceptance",
            "evidence_based_tournament_selection",
            "best_effort_tournament_selection",
            "failed",
            "inconclusive",
        ]
        | None
    ) = None
    candidate_attempts_requested: int | None = None
    candidate_attempts_launched: int = 0
    candidate_launch_limit_reason: str | None = None
    reserve_finalization_seconds: int = 60
    finalization_guard_seconds: int = 60
    tournament_budget_policy: Literal["off", "guarded", "planned"] = "off"
    reserve_pairwise_seconds: int = 180
    reserve_ranking_seconds: int = 30
    reserve_review_seconds: int = 60
    max_candidate_review_seconds: int = 240
    per_candidate_review_timeout_seconds: int = 30
    per_pairwise_comparison_timeout_seconds: int = 30
    tournament_evaluation_deadline_seconds: int = 120
    max_review_retries: int = 2
    max_malformed_review_retries: int = 2
    max_pairwise_retries: int = 2
    candidate_generation_deadline: float | None = None
    decomposition_requested: bool = False
    decomposition_validated: bool = False
    ...
    decomposed_execution_status: Literal[
        "not_started", "running", "completed", "blocked", "failed"
    ] = "not_started"
    ...
    fallback_execution_path: Literal[
        "none", "parallel_candidates_after_decomposition_deadlock"
    ] = "none"
    fallback_reason: str | None = None
    fallback_used: bool = False
    fallback_from_execution_path: str | None = None
    fallback_started_at: str | None = None
    best_partial_attempt_id: str | None = None
    partial_progress: dict | None = None
    candidates: list[CandidateAttemptState] = Field(default_factory=list)
    ...
    backend_assessments: dict[str, dict] = Field(default_factory=dict)
    runner_assessment: dict = Field(default_factory=dict)
    adaptive_context: dict = Field(default_factory=dict)
    integration: dict | None = None
    decomposition_integration_worktree: str | None = None
    integration_base_revision: str | None = None
    accepted_patch_application_status: dict[str, dict] = Field(default_factory=dict)
    reviews: list[dict] = Field(default_factory=list)
    repo_validation_results: list[dict] = Field(default_factory=list)
    ...
    execution_concurrency: dict = Field(default_factory=dict)
    candidate_concurrency: dict = Field(default_factory=dict)
    subtask_concurrency: dict = Field(default_factory=dict)
    ...
    last_invalid_tool_input_hash: str | None = None
    repeat_invalid_count: int = 0
    ...
    def allowed_next_actions(self) -> list[str]:
        ...
        if not self.investigation:
            ...
            return a
        if not self.plan:
            a.append("ops_submit_plan")
            return a
        if self.orchestrator == "adaptive" and self.execution_path == "unknown":
            if (self.plan or {}).get("execution_path") == "candidate_tournament":
                a.append("ops_launch_tournament_candidates")
            else:
                a.append("ops_select_execution_path")
            return a
        if self.decomposition_requested and not self.decomposition:
            a.append("ops_submit_decomposition")
            return a
        if self.decomposition and not self.decomposition_validated:
            a.append("ops_validate_decomposition")
            return a
        if self.execution_path == "unknown":
            a.append("ops_select_execution_path")
            return a
        if self.execution_path == "candidate_tournament":
            ...
            if self.tournament_ranking is None:
                a.append("ops_evaluate_tournament")
                return list(dict.fromkeys(a))
            ...
            if self.tournament_ranking.selected_candidate_id:
                a += ["ops_select_winner", "ops_finalize_run"]
                return list(dict.fromkeys(a))
        ...
        if self.execution_path == "single_task":
            ...
            for c in self.candidates:
                try:
                    ...
                    if is_attempt_acceptance_eligible(c, state=self)[0]:
                        eligible.append(c)
                except Exception:
                    pass
                complete = c.status in {
                    "completed",
                    "failed",
                    "reviewed",
                    "rejected",
                ...
                if complete and (
                    o is None
                    or o.validation_snapshot_id != val_snap
                    or o.review_snapshot_id != rev_snap
                ):
            ...
            if eligible:
                ...
                return list(dict.fromkeys(a))
            if needs_validation:
                a.append("ops_run_validation")
                return list(dict.fromkeys(a))
            if needs_review:
                a.append("ops_review_attempt")
                return list(dict.fromkeys(a))
            if needs_observation:
                a.append("ops_observe_completed_attempt")
                return list(dict.fromkeys(a))
            if len(self.candidates) < budget:
                a += ["ops_run_next_candidate_attempt"]
                return list(dict.fromkeys(a))
            a += ["ops_select_winner", "ops_finalize_run"]
            return list(dict.fromkeys(a))
        if self.execution_path == "parallel_candidates" and not self.candidates:
            a.append("ops_launch_candidates")
            return a
        if (
            self.fallback_execution_path
            ...
            for c in complete:
                try:
                    ...
                    pass
                if c.patch_path and c.changed_files and not c.validation:
                    needs_validation.append(c)
                if (
                    c.patch_path
                    and c.changed_files
                    and not c.review
                    and not c.failure_reason
                ):
                    needs_review.append(c)
                o = obs_by_id.get(c.attempt_id)
                val_snap = f"{c.validation_status}:{len(c.validation_results or [])}"
            ...
            if len(complete) < budget:
                a.append("ops_run_next_fallback_candidate_attempt")
                return list(dict.fromkeys(a))
            a.append("ops_finalize_run")
            return list(dict.fromkeys(a))
        if (
            self.execution_path == "decomposed_subtasks"
            and self.decomposed_execution_status in {"blocked", "failed"}
        ):
            if not self.candidates:
                a.append("ops_start_candidate_fallback")
            a += ["ops_select_winner", "ops_finalize_run"]
            return list(dict.fromkeys(a))
        if self.execution_path == "decomposed_subtasks":
            if any(s.status == "pending" for s in self.subtasks):
                a += ["ops_run_next_subtask_attempt"]
                return list(dict.fromkeys(a))
            if (
                all(s.status in {"accepted", "skipped"} for s in self.subtasks)
                and not self.integration
            ):
                a.append("ops_integrate_subtasks")
                return a
            if (
                self.integration
                and (self.integration.get("validation") or {}).get("passed") is False
            ):
                a.append("ops_run_next_integration_repair_attempt")
                return list(dict.fromkeys(a))
        a += [
            "ops_review_attempt",
            "ops_run_validation",
    ...
    def load(cls, path: Path) -> "OpsRunState":
        ...
        if not text.strip():
            ...
            raise ValueError(
                "state.json is empty or corrupted, likely due a previous interrupted/failed write. Check runtime_events.jsonl and transcript.json."
            )
