...
class ToolCallingLLMClient:
    def create_message(
        ...
        if not base or not model:
            raise ValueError(
                "agentic orchestrator requires a configured backend with tool-calling support or OpenAI-compatible chat completions"
            )
        key = getattr(backend, "api_key", None) or os.getenv(
            getattr(backend, "api_key_env", None) or "", ""
        )
        msgs = [
            {
                "role": "system",
