    ...
    for pat in patterns or DEFAULT_PATCH_EXCLUDES:
        ...
        if q.endswith("/**"):
            ...
            if p == base or p.startswith(base + "/"):
                return True
        elif q.startswith("**/") and fnmatch.fnmatch(p, q):
            return True
        elif "/" not in q and (
            p == q or p.startswith(q + "/") or fnmatch.fnmatch(Path(p).name, q)
        ):
            return True
...
def is_scratch_artifact_path(path: str) -> bool:
    ...
    name = Path(p).name
    return any(
        fnmatch.fnmatch(p, pat) or fnmatch.fnmatch(name, pat)
        for pat in SCRATCH_ARTIFACT_PATTERNS
    )


def clean_untracked_scratch_artifacts(worktree_path: Path) -> list[str]:
    status = _run(
        ["git", "status", "--porcelain", "--untracked-files=all"], worktree_path
    )
...
def is_git_compatible_patch(patch_path: str | Path | None) -> bool:
    if not patch_path:
        return False
    try:
        text = Path(patch_path).read_text(encoding="utf-8", errors="replace").lstrip()
    except Exception:
        return False
    return (
...
def clean_runner_artifacts_from_worktree(worktree_path: Path) -> None:
    ...
    for p in (
        list(worktree_path.rglob("*.pyc"))
        + list(worktree_path.rglob("*.pyo"))
        + list(worktree_path.rglob("*.log"))
    ):
...
def capture_git_patch(
    worktree_path: Path, patch_path: Path, *, exclude_patterns: list[str] | None = None
    ...
    try:
        ...
        return GitPatchCaptureResult(
            patch_path=str(patch_path) if patch_bytes.strip() else None,
            ...
            diff_stat=stat.stdout or None,
            name_status=rows,
        )
    except Exception as e:
        try:
            write_text_utf8(patch_path, "")
        except Exception:
            pass
        return GitPatchCaptureResult(
            patch_path=None,
