...
UsageSource = Literal[
    "provider_response", "runner_result", "debug_trace", "unavailable", "estimated"
]
...
def extract_usage_tokens(response: Any) -> dict[str, int | None] | None:
    ...
    if not isinstance(data, dict):
        data = getattr(response, "raw_response", None) or getattr(
            response, "usage", None
        )
...
def calculate_usage_cost(
    input_tokens: int | None, output_tokens: int | None, backend: Any | None
) -> CostResult:
    inp_price = (
        getattr(backend, "input_cost_per_million", None)
        if backend is not None
        else None
    )
    out_price = (
        getattr(backend, "output_cost_per_million", None)
        if backend is not None
        else None
    )
    input_cost = (
        (input_tokens / 1_000_000 * inp_price)
        if input_tokens is not None and inp_price is not None
        else None
    )
    output_cost = (
        (output_tokens / 1_000_000 * out_price)
        if output_tokens is not None and out_price is not None
        else None
    )
    total_cost = (
    ...
    return CostResult(
        ...
        input_cost_per_million=inp_price,
        output_cost_per_million=out_price,
    )
...
def usage_record_from_response(
    *,
    ...
    if tokens is None and isinstance(getattr(response, "usage", None), dict):
        tokens = extract_usage_tokens(getattr(response, "usage"))
    source = usage_source if tokens is not None else "unavailable"
    cost = calculate_usage_cost(
        tokens.get("input_tokens") if tokens else None,
        tokens.get("output_tokens") if tokens else None,
        backend,
    )
    return UsageRecord(
        run_id=run_id,
        ...
        model=getattr(response, "model", None) or getattr(backend, "model", None),
        ...
        input_cost_per_million=cost.input_cost_per_million,
        output_cost_per_million=cost.output_cost_per_million,
        ...
        if tokens
        else (unavailable_reason or "provider_response_missing_usage"),
    )


def usage_record_from_runner(
    *,
    run_id: str,
    phase: str,
    role: str,
    ...
    inp = _maybe_int(getattr(result, "input_tokens", None))
    out = _maybe_int(getattr(result, "output_tokens", None))
    total = _maybe_int(getattr(result, "total_tokens", None))
    status = getattr(result, "token_accounting_status", None)
    has_usage = (
        (inp is not None and inp > 0)
        or (out is not None and out > 0)
        or (total is not None and total > 0)
        or status in {"verified", "summary_only", "mismatch"}
    )
    if total is None and (inp is not None or out is not None):
        total = (inp or 0) + (out or 0)
    ...
    return UsageRecord(
        ...
        input_tokens=inp if has_usage else None,
        output_tokens=out if has_usage else None,
        total_tokens=total if has_usage else None,
        input_cost_per_million=cost.input_cost_per_million,
        output_cost_per_million=cost.output_cost_per_million,
        input_cost=cost.input_cost if has_usage else None,
        output_cost=cost.output_cost if has_usage else None,
        total_cost=cost.total_cost if has_usage else None,
        usage_source="runner_result" if has_usage else "unavailable",
        unavailable_reason=None if has_usage else "runner_result_missing_usage",
    )
...
class UsageRecorder:
    def __init__(self, run_dir: Path, run_id: str):
        ...
        self.run_id = run_id
        self.path = self.run_dir / "usage.jsonl"
        self.run_dir.mkdir(parents=True, exist_ok=True)
        self._lock = threading.Lock()
        self.records: list[UsageRecord] = []
        self._summary_write_interval_seconds = 3.0
        self._last_summary_write_at = 0.0

    def record(self, usage: UsageRecord) -> None:
        if usage.run_id is None:
            usage.run_id = self.run_id
        line = (
            json.dumps(usage.model_dump(mode="json"), ensure_ascii=False, default=str)
            + "\n"
        )
        with self._lock:
            ...
            now = time.monotonic()
            if (
                now - self._last_summary_write_at
                >= self._summary_write_interval_seconds
            ):
                try:
    ...
    def _summarize_records(self, records: list[UsageRecord]) -> UsageSummary:
        ...
        for r in records:
            _add(s, r)
            for key, val, attr in [
                ("by_role", r.role, "by_role"),
                ("by_backend", r.backend_name, "by_backend"),
                ("by_model", r.model, "by_model"),
                ("by_attempt", r.attempt_id, "by_attempt"),
                ("by_phase", r.phase, "by_phase"),
            ]:
                if val:
                    d = getattr(s, attr)
                    d.setdefault(str(val), UsageBucket())
                    _add(d[str(val)], r)
        return s
    ...
    def _write_artifacts_unlocked(self) -> None:
        ...
        _atomic_json(
            self.run_dir / "usage.json", {"records": records, "summary": summary}
        )
...
def _add(b: UsageBucket, r: UsageRecord) -> None:
    b.calls_count += 1
    if r.usage_source == "unavailable":
        b.unavailable_calls_count += 1
    if r.estimated:
        b.estimated_calls_count += 1
    b.input_tokens += r.input_tokens or 0
    b.output_tokens += r.output_tokens or 0
    b.total_tokens += r.total_tokens or 0
    b.input_cost += r.input_cost or 0.0
    b.output_cost += r.output_cost or 0.0
    b.total_cost += r.total_cost or 0.0
