...
def write_markdown_report(
    run_dir, task, policy_or_strategy, attempts, decision, wall_time: float
):
    p = Path(run_dir) / "report.md"
    strategy = decision.execution_strategy or (
        policy_or_strategy.model_dump(mode="json")
        if hasattr(policy_or_strategy, "model_dump")
        else {}
    )
    ...
    rankings = (
        "\n".join(
            f"- {r.get('expected_cost_rank')}. {r.get('backend')}: capability {r.get('capability_score')}, capability gap {r.get('capability_gap')}, base p_solve {r.get('base_solve_probability')}, shape adjustment {r.get('shape_adjustment')}, final p_solve {r.get('final_solve_probability')}, estimated ${r.get('estimated_attempt_cost')} — {r.get('rank_reason')}"
            for r in (strategy.get("backend_rankings") or [])
        )
        or "- none"
    )
    planned_attempts = (
    ...
    human_override_blockers = (
        "\n".join(
            f"- {b}" for b in (getattr(decision, "human_override_blockers", []) or [])
        )
        or "- none"
    )
    ...
    any(
        "deterministic fallback policy" in str(w)
        for w in (strategy.get("warnings") or [])
    )
    next(
        (
            str(w)
            for w in (strategy.get("warnings") or [])
            if "deterministic fallback policy" in str(w)
        ),
        "",
    )
    changed = (
        "\n".join(f"- {f}" for a in attempts for f in a.get("changed_files", []))
        or "- none"
    )
    telemetry = (
        "\n\n".join(
