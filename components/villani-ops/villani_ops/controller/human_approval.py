...
class HumanApprovalProvider(Protocol):
    def request_approval(
        self, context: HumanApprovalPromptContext
    ) -> HumanApprovalResult: ...
...
class NonInteractiveHumanApprovalProvider:
    def request_approval(
        ...
        return HumanApprovalResult(
            requested=True,
            request_reasons=context.request_reasons,
            prompted=False,
            skipped_reason="non_interactive",
            decision="skipped",
            shown_evidence=_shown(context),
        )
...
class TerminalHumanApprovalProvider:
    def request_approval(
        ...
        print(
            f"Human approval requested for {context.attempt_id} ({context.backend_name}/{context.backend_model})"
        )
        ...
        for _ in range(3):
            decision = (
                input("Decision [accept/reject/retry/escalate/fail]: ").strip().lower()
            )
        ...
        return HumanApprovalResult(
            ...
            valid_override=_valid(decision, context),
            shown_evidence=_shown(context),
...
class TestHumanApprovalProvider:
    ...
    def request_approval(
        ...
        return HumanApprovalResult(
            ...
            reason=self.reason,
            approved_by="test",
            valid_override=_valid(self.decision, context),
            shown_evidence=_shown(context),
        )
...
def _shown(c: HumanApprovalPromptContext) -> dict[str, Any]:
    return {
        ...
        "reviewer_summary": c.review_summary,
        "reviewer_decision": c.review_decision,
        "reviewer_issues": c.review_issues,
...
def _valid(decision: str, c: HumanApprovalPromptContext) -> bool:
    return (
        decision == "accept"
        and bool(c.request_reasons)
        and bool(c.patch_path)
        and bool(c.changed_files)
    )
