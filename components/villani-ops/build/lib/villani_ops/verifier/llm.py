...
def select_verifier_backend(workspace='.villani-ops', backend_name=None, base_url=None, model=None, api_key=None):
    if base_url and model:
        key=api_key or os.getenv('VILLANI_OPS_VERIFIER_API_KEY') or os.getenv('OPENAI_API_KEY') or ('dummy' if _is_local(base_url) else '')
    ...
    if (model or os.getenv('VILLANI_OPS_VERIFIER_MODEL')) and not backend_name and not base_url:
        ...
        key=api_key or os.getenv('VILLANI_OPS_VERIFIER_API_KEY') or os.getenv('OPENAI_API_KEY') or ('dummy' if _is_local(bu) else '')
...
def _chat_message(cfg,messages,timeout, trace=None, use_tools=True, force_tool=None):
    ...
    try:
        ...
        r=httpx.post(cfg['baseUrl'].rstrip('/')+'/chat/completions',headers=headers,json=payload,timeout=timeout)
...
def calibrate(det, verdict, trace=None, cfg=None, timeout=30):
    ...
    return final
def llm_result(run, det, workspace='.villani-ops', backend=None, base_url=None, model=None, api_key=None, timeout_seconds=180, max_tool_calls=12, max_tool_result_chars=12000, max_read_lines=160, trace=None):
    cfg=select_verifier_backend(workspace,backend,base_url,model,api_key=api_key); tools=VerifierTools(run,det.get('repoDir'),max_tool_result_chars,max_read_lines)
