...
const value = (input, missing = "Unknown") => input === null || input === undefined || input === ""
    ? missing
    : String(input);
const json = (input) => escapeHtml(JSON.stringify(input, null, 2) ?? "Unknown");
const money = (input) => input === null ? "Unknown" : `USD ${input.toFixed(5)}`;
const tokens = (input) => input === null ? "Unknown" : input.toLocaleString("en-US");
const statusGlyph = (status) => {
    const normalized = String(status ?? "unknown").toLowerCase();
    if (/fail|error/.test(normalized))
        return "×";
    if (/reject|exhaust/.test(normalized))
        return "⊘";
    if (/run|start|pending/.test(normalized))
        return "◉";
    if (/complete|succeed|accept|selected/.test(normalized))
        return "●";
    return "○";
};
const eventName = (event) => event.name ?? event.title ?? event.type ?? "unknown_event";
const eventRole = (event) => event.body?.command_role ??
    event.attributes?.command_role ??
    event.body?.role ??
    "—";
const eventAuthority = (event) => event.body?.verification_authority ??
...
function classification(snapshot) {
...
function files(snapshot, artifacts) {
    return `<section class="panel villani-panel" id="file-activity" data-testid="file-activity"><div class="panel-head"><div><h2>File activity</h2><p>Writes by attempt and the files materialized from the selected patch only.</p></div></div><div class="file-activity-grid"><div><h4>Writes by attempt</h4>${Object.entries(snapshot.attempt_changed_files)
        .map(([attempt, paths]) => `<article class="attempt-file-row"><strong>${escapeHtml(attempt)}</strong><span>${paths.map(escapeHtml).join(", ") || "No changed files"}</span></article>`)
...
function redaction(snapshot) {
    const visible = snapshot.redaction_status !== null ||
...
export function renderConnectedReplay(detail, events = [], artifacts = []) {
