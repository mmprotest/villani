...
const text = (value, missing = "Not captured") => value === null || value === undefined ? missing : String(value);
const number = (value, missing = "Unknown") => value === null || value === undefined ? missing : String(value);
const usd = (value, currency = "USD") => value === null || value === undefined
    ? "Unknown"
...
const duration = (value) => value === null || value === undefined
    ...
    : value < 1000
        ? `${value}ms`
        : `${value / 1000}s`;
const json = (value) => escapeHtml(truncateText(JSON.stringify(value, null, 2) ?? "Not captured"));
const artifactList = (paths) => `<ul class="artifact-list">${Object.entries(paths)
    .map(([label, artifact]) => `<li><b>${escapeHtml(label)}</b><span class="mono">${escapeHtml(text(artifact))}</span></li>`)
    .join("")}</ul>`;
function classificationAudit(run) {
    const metadata = (run.classification?.metadata ?? {});
    const raw = metadata.raw_classification ?? run.classification ?? null;
    const effective = metadata.effective_classification ?? run.classification ?? null;
    ...
    const rows = adjustments
        .map((item) => item)
        .map((adjustment) => `<li>${escapeHtml(text(adjustment.field))}: ${escapeHtml(text(adjustment.before))} → ${escapeHtml(text(adjustment.after))}<span>${escapeHtml(text(adjustment.rule_id))} / ${escapeHtml(text(adjustment.reason))} / ${escapeHtml(text(adjustment.authority))}</span></li>`)
        .join("");
...
function policyDetails(run) {
    ...
    return `<section class="panel villani-panel"><div class="panel-head"><div><h2>Policy decisions</h2><p>Every backend alternative and rejection reason, in decision sequence.</p></div></div>${run.policyDecisions
        .map((decision) => `<article class="policy-decision" data-decision-sequence="${decision.decision_sequence}"><h3>#${decision.decision_sequence} ${escapeHtml(decision.action)} · ${escapeHtml(decision.policy_version)}</h3><p>${escapeHtml(decision.reason)}</p><dl class="compact-facts"><div><dt>Required capability</dt><dd>${escapeHtml(number(decision.metadata.required_capability_score))}</dd></div><div><dt>Rule</dt><dd>${escapeHtml(text(decision.metadata.required_capability_rule))}</dd></div><div><dt>Chosen backend</dt><dd>${escapeHtml(text(decision.chosen_backend))}</dd></div><div><dt>Attempt</dt><dd>${escapeHtml(text(decision.attempt_id))}</dd></div></dl><div class="table-wrap"><table class="evidence-table policy-table"><thead><tr><th>Backend / model</th><th>Capability</th><th>Estimated cost</th><th>Accounting</th><th>Eligible</th><th>Rejection reasons</th></tr></thead><tbody>${decision.considered_backends
        .map((alternative) => `<tr><td><b>${escapeHtml(alternative.backend_name)}</b><br>${escapeHtml(text(alternative.model))}</td><td>${escapeHtml(number(alternative.capability_score))}</td><td>${escapeHtml(usd(alternative.estimated_cost_usd, run.aggregate?.currency))}</td><td>${escapeHtml(alternative.cost_accounting_status)}</td><td>${alternative.eligible ? "Yes" : "No"}</td><td>${escapeHtml(alternative.rejection_reasons.join("; ") || "None")}</td></tr>`)
        .join("")}</tbody></table></div><details><summary>Budget before and projected after</summary><pre>${json({ before: decision.budget_before, after: decision.budget_after })}</pre></details></article>`)
        .join("")}</section>`;
}
function candidateComparison(run) {
    const verificationByAttempt = new Map(run.verifications.map((verification) => [
        verification.attempt_id,
        verification,
    ]));
    const rankingByAttempt = new Map((run.selection?.rankings ?? []).map((ranking) => [
        ranking.attempt_id,
        ranking,
    ]));
    ...
    return `<section class="panel villani-panel"><div class="panel-head"><div><h2>Candidate comparison</h2><p>Acceptance evidence and deterministic ranking from the canonical bundle.</p></div></div><div class="table-wrap"><table class="evidence-table candidate-table"><thead><tr><th>Attempt</th><th>Backend / model</th><th>Eligible</th><th>Verifier outcome</th><th>Confidence</th><th>Critical coverage</th><th>Tests / evidence</th><th>Risk flags</th><th>Tokens</th><th>Duration</th><th>Cost</th><th>Selection reason</th></tr></thead><tbody>${run.attempts
        ...
        const passed = verification?.requirement_results.filter((result) => result.outcome === "passed").length ?? 0;
        ...
        const evidence = [
            ...
            .join("; ");
        const tokens = snapshot.input_tokens !== null && snapshot.output_tokens !== null
            ? snapshot.input_tokens + snapshot.output_tokens
            : null;
        const selectionReason = ranking?.reason ??
        ...
        .join("")}</tbody></table></div><details><summary>Candidate evidence matrix</summary><pre>${json(run.candidateEvidenceMatrix)}</pre></details><details><summary>Deterministic selection</summary><pre>${json(run.selection)}</pre></details></section>`;
}
function attemptDetails(run) {
    return `<section class="panel villani-panel"><div class="panel-head"><div><h2>Attempt evidence</h2><p>Captured runner output, patch, trace, telemetry, usage, cost components, and artifact paths.</p></div></div>${run.attempts
        .map((attempt) => {
        const snapshot = attempt.snapshot;
        const verification = run.verifications.find((item) => item.attempt_id === snapshot.attempt_id);
        const traceRecords = [
        ...
        const toolCalls = traceRecords.filter((item) => String(item.event_type ?? item.type ?? "").includes("tool"));
        const commands = traceRecords.filter((item) => String(item.event_type ?? item.type ?? "").includes("command"));
...
function canonicalEvidenceSummary(run) {
    const summary = run.runSummary;
    if (!summary)
        return `<section class="panel villani-panel" data-testid="canonical-run-summary"><h2>Canonical evidence summary</h2><p>Not captured in this legacy run bundle.</p></section>`;
    return `<section class="panel villani-panel" data-testid="canonical-run-summary"><div class="panel-head"><div><h2>Canonical evidence summary</h2><p>The same projection consumed by CLI, Web, and durable reports.</p></div></div><dl class="compact-facts"><div><dt>Repository checks</dt><dd>passed ${escapeHtml(number(summary.checks.passed))}, failed ${escapeHtml(number(summary.checks.failed))}, not run ${escapeHtml(number(summary.checks.not_run))}, unavailable ${escapeHtml(number(summary.checks.unavailable))}</dd></div><div><dt>Focused probes</dt><dd>passed ${escapeHtml(number(summary.focused_probes.passed))}, failed ${escapeHtml(number(summary.focused_probes.failed))}, not run ${escapeHtml(number(summary.focused_probes.not_run))}, unavailable ${escapeHtml(number(summary.focused_probes.unavailable))}</dd></div><div><dt>Requirements</dt><dd>proved ${escapeHtml(number(summary.requirements.proved))}, not proved ${escapeHtml(number(summary.requirements.not_proved))}</dd></div><div><dt>Accounting</dt><dd>${summary.accounting.known ? escapeHtml(usd(summary.accounting.total_cost, summary.accounting.currency ?? undefined)) : `Unknown (${escapeHtml(summary.accounting.accounting_status)})`}</dd></div><div><dt>Final acceptance</dt><dd>${summary.acceptance.decision ? "Accepted" : "Not accepted"} · ${escapeHtml(summary.acceptance.reason_code)}</dd></div></dl></section>`;
}
function costAudit(run) {
...
export function villaniRunDetails(run) {
    if (run.corruptReason)
        return "";
    return `<div class="villani-details"><div id="evidence-panel" data-testid="evidence-panel">${aggregateDetails(run)}${canonicalEvidenceSummary(run)}</div>${costAudit(run)}${classificationAudit(run)}${policyDetails(run)}<div id="candidate-comparison" data-testid="candidate-comparison">${candidateComparison(run)}</div><div id="file-activity" data-testid="file-activity">${attemptDetails(run)}</div></div>`;
}
