...
const usd = (value) => value === null || value === undefined ? "Unknown" : `$${value.toFixed(2)}`;
...
function policyDetails(run) {
    ...
    return `<section class="panel villani-panel"><div class="panel-head"><div><h2>Policy decisions</h2><p>Every backend alternative and rejection reason, in decision sequence.</p></div></div>${run.policyDecisions
        ...
        .map((alternative) => `<tr><td><b>${escapeHtml(alternative.backend_name)}</b><br>${escapeHtml(text(alternative.model))}</td><td>${escapeHtml(number(alternative.capability_score))}</td><td>${escapeHtml(usd(alternative.estimated_cost_usd))}</td><td>${escapeHtml(alternative.cost_accounting_status)}</td><td>${alternative.eligible ? "Yes" : "No"}</td><td>${escapeHtml(alternative.rejection_reasons.join("; ") || "None")}</td></tr>`)
