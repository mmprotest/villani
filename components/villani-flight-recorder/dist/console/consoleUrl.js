...
const endpointPath = () => path.join(process.env.VILLANI_HOME || path.join(os.homedir(), ".villani"), "agentd", "endpoint.json");
...
export async function villaniConsoleUrl(relative = "/console") {
    let document;
    try {
    ...
    const endpoint = document && typeof document === "object" && "endpoint" in document
    ...
    try {
        ...
        throw new Error("Villani Service endpoint is invalid. Run: villani service restart");
    }
...
export function consoleRedirectHtml(url) {
    const safe = url
        .replaceAll("&", "&amp;")
        .replaceAll('"', "&quot;")
        .replaceAll("<", "&lt;");
