...
export function assertOutsideVillaniRunsRoot(runsRoot, ...outputs) {
    ...
    for (const output of outputs) {
        ...
        if (candidate === root || candidate.startsWith(`${root}${path.sep}`))
            throw new Error(`Flight Recorder output must be outside the canonical runs root: ${candidate}`);
    }
}
export async function launchVillaniRun(options, dependencies = {}) {
    ...
    if (options.open !== false)
        await (dependencies.open ?? openBrowser)(file);
    return file;
}
