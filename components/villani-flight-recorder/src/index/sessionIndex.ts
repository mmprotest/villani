...
import {
  fingerprintVillaniRun,
  villaniSessionFields,
} from "./villaniRunIndex.js";
...
async function fp(file: string) {
  ...
  if (st.isDirectory()) return fingerprintVillaniRun(file);
    ...
    for (const ad of tryAdapters) {
      ...
      try {
        ...
        sessions.push({
          ...
          inputTokenCount:
            parsed.tokenUsage?.inputTokens ?? tokenUsage?.inputTokens,
          outputTokenCount:
            parsed.tokenUsage?.outputTokens ?? tokenUsage?.outputTokens,
          cacheTokenCount:
            tokenUsage &&
          ...
          ...villaniSessionFields(parsed as unknown as ParsedSession),
        });
