...
import type { ParsedSession } from "../providers/types.js";
import type { SessionRecord } from "./sessionTypes.js";

export interface VillaniRunFingerprint {
  sizeBytes: number;
  modifiedAt: string;
  mtimeMs: number;
  hash: string;
...
export async function fingerprintVillaniRun(
  runDirectory: string,
): Promise<VillaniRunFingerprint> {
...
export function villaniSessionFields(
  ...
  return {
    ...
    outcome:
      manifest?.final_state === "COMPLETED"
        ? "success"
