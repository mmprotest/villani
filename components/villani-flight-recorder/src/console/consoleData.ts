...
import type { FlightEvent, ParsedSession } from "../providers/types.js";
...
const state = (record: SessionRecord): string =>
  text(record.state ?? record.outcome) ?? "unknown";

export function consoleHistoryFromIndex(
  index: SessionIndex,
): ConsoleHistoryEntry[] {
...
function flightEvent(event: FlightEvent, runId: string): RunEvent {
  return {
...
function localRunDetail(session: ParsedSession): RunDetail {
    ...
    (data?.attempts ?? []).map((attempt) => {
      ...
      return [
        attempt.snapshot.attempt_id,
        {
          ...
          total_tokens:
            attempt.snapshot.input_tokens === null ||
          ...
          file_write_count: number(
            object(attempt.runnerTelemetry).file_write_count,
          ),
  ...
  const verification = data?.verifications.find(
    (item) => item.attempt_id === manifest?.selected_attempt_id,
  );
  ...
  const rawClassification = data?.classification
    ...
    : null;
  const first =
    manifest?.created_at ?? session.startedAt ?? new Date(0).toISOString();
  const last =
  ...
  return {
    ...
    effective_classification:
      classificationMetadata.effective_classification &&
    ...
    classification_adjustments: Array.isArray(
      classificationMetadata.adjustments,
    )
      ? (classificationMetadata.adjustments as Record<string, unknown>[])
      : [],
    policy_version: policy?.policy_version ?? null,
    policy_decisions: data?.policyDecisions as unknown as Record<
      string,
      unknown
    >[],
    ...
    escalation_count: (data?.policyDecisions ?? []).filter(
    ...
    total_tokens:
      manifest?.total_input_tokens === null ||
      manifest?.total_output_tokens === null
        ...
        : manifest?.total_input_tokens !== undefined &&
          ...
          : null,
    coding_cost_usd:
      manifest?.stage_metrics?.coding?.cost ??
    ...
    terminal_reason:
      runState?.failure?.message ?? manifest?.final_state ?? null,
    candidate_outcomes: candidateOutcomes,
    selection_reason: data?.selection?.reason ?? null,
    selection_rankings: data?.selection?.rankings as unknown as Record<
      string,
      unknown
    >[],
    ...
    verification_authority:
      text(
        verificationMetadata.authority_source ??
          verificationMetadata.verification_authority,
      ) ?? null,
...
function cleanRedaction<T>(value: T): T {
  const result = redactDeep(value) as T & { redactionReport?: unknown };
  if (result && typeof result === "object") delete result.redactionReport;
  return result;
}

function importedReplay(
  ...
  const files = [
    ...new Set(
      record.changedFiles ?? session.events.map((event) => event.path),
    ),
  ]
    .filter(
...
function localReplay(session: ParsedSession): ConsoleReplaySnapshot {
  const detail = localRunDetail(session);
  const result = consoleReplayFromRunDetail(
    detail,
    session.events.map((event) => flightEvent(event, detail.id)),
    [],
    "LOCAL",
  );
  const data = session.villani;
  const extraLogs: ConsoleReplayLog[] = [];
...
export async function consoleIndex(
  options: {
    indexDir?: string;
    refresh?: boolean;
    roots?: string[];
...
export async function consoleReplay(options: {
  ...
  if (!record) throw new Error(`indexed session ${options.id} was not found`);
