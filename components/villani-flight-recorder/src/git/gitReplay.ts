...
async function git(args: string[], cwd = process.cwd()) {
  return (
    await runChildProcess("git", args, {
      cwd,
      timeoutMs: 10_000,
...
function flags(files: string, patch: string) {
  const names = files.split(/\r?\n/).map((l) => l.split(/\s+/).pop() ?? "");
  return [
    names.some((f) =>
      /package.json|lock|Cargo.toml|requirements|pyproject|go.mod|go.sum/.test(
        f,
      ),
...
export async function buildGitReplay(
  from: string,
  to: string,
  cwd = process.cwd(),
  ...
  for (const sha of shas) {
    ...
    events.push(
      event(
        ...
        "git_commit",
        `Commit ${subject}`,
        { sha: full, author, timestamp, files },
        {
          timestamp,
  ...
  events.push(
    event(
      ...
      {},
      {
        ...
        summary:
          "Git-only replay: reasoning, tool calls, failed attempts, approvals, and shell commands are unknown unless committed.",
      },
    ),
  );
  return {
